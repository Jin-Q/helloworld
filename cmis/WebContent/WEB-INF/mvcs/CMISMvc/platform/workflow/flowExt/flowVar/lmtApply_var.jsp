...
<style type="text/css">
.emp_field_text_input2 {width: 150px;}
</style>
...
KeyedCollection kCollApp = dao.queryDetail("LmtApply", serno, connection);
...
<script type="text/javascript">
	...
	/*    var recordCount = LmtAppDetailsList._obj.recordCount;//取总记录数
	    ...
	    for(var i=0;i<recordCount;i++){
			var limit_code = LmtAppDetailsList._obj.data[i].org_limit_code._getValue();
			alert(limit_code);       
			...
			//WfiBizVarRecord.crd_lmt@+serno+'@'+limit_code._setValue(crd_amt);
	    }
	    return;*/
   ...
   /* modified by yangzy 2014/12/01  需求:XD140925064,生活贷需求开发  end */
	...
	<DIV>
		<emp:gridLayout id="bizGroup" title="业务概况" maxColumn="2" >
			<emp:select id="LmtApply.app_type" label="申请类型" dictname="STD_ZB_APP_TYPE" readonly="true" />
			<emp:select id="LmtApply.biz_type" label="授信业务类型" dictname="STD_ZB_BIZ_TYPE" defvalue="01" readonly="true"/>
			<emp:text id="LmtApply.cus_id" label="客户码" readonly="true"/>
			<emp:text id="LmtApply.cus_id_displayname" label="客户名称" cssElementClass="emp_field_text_long_readonly" colSpan="2" readonly="true" /> 
			<emp:select id="LmtApply.lrisk_type" label="低风险业务类型" dictname="STD_ZB_LRISK_TYPE" readonly="true" defvalue="20"/>
			<emp:select id="LmtApply.cur_type" label="授信币种" dictname="STD_ZX_CUR_TYPE" defvalue="CNY" readonly="true"/>
			<emp:text id="LmtApply.crd_totl_amt" label="授信总额" maxlength="18" dataType="Currency" readonly="true" defvalue="0" cssElementClass="emp_currency_text_readonly" />
			<emp:text id="LmtApply.crd_cir_amt" label="循环授信敞口" maxlength="18" dataType="Currency" defvalue="0.00" cssElementClass="emp_currency_text_readonly" readonly="true"/>
			<emp:text id="LmtApply.crd_one_amt" label="一次性授信敞口" maxlength="18" dataType="Currency" defvalue="0.00" cssElementClass="emp_currency_text_readonly" readonly="true"/>
		</emp:gridLayout>
	</DIV>
	</emp:form>
