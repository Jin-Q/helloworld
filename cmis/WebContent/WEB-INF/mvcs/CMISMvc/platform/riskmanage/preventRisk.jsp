	...
	function goto(className){
		...
		var handleSuccess = function(o){
			if(o.responseText !== undefined) {
				...
				var nextUrl = jsonstr.nextUrl;
				if(flag == "success"){
					var url1 = EMPTools.encodeURI(nextUrl);
					url1=url1+'&EMP_SID='+EMP_SID;
					EMPTools.openWindow(url1,'newwindow','height=700, width=1200, top=80, left=80, toolbar=no, menubar=no, scrollbars=yes, resizable=yes, location=no, status=no');
				}else {
					alert("获取连接配置异常!");
				}
			}
		...
		var url = '<emp:url action="doUrlDual.do"/>?className='+className+'&serno='+serno+'&rd='+Math.random();
		...
		for(int i=0;i<icoll.size();i++) {
			...
			if("2".equals(passState)&&"2".equals(riskLevel)){
				if(linkUrl != null && linkUrl.trim().length() > 0){
					...
					out.println("<td ><span style=\"color: red;\">"+kcoll.get("item_desc")+"</span></td>");
				}
				String id = "img"+i;
				...
				if(linkUrl != null && linkUrl.trim().length() > 0){
					...
					out.println("<td ><span style=\"color: #A6A600;\">"+kcoll.get("item_desc")+"</span></td>");
				}
				String id = "img"+i;
				String name = "warn"+i;
