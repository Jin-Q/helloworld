...
<script type="text/javascript">
	...
	function changeScence(data){
		...
		var scene_id = data;
		var url = '<emp:url action="getPrdPvRiskSceneChangeUpdatePage.do"/>?prevent_id='+prevent_id+'&wfid='+flowId+'&scene_id='+scene_id;
	...
	function doSave(){
		...
		for(var i=0;i<sceneListValue.length;i++){
			...
			if(sceneValue.is_check._getValue() == 1){//选中状态
				if(sceneValue.risk_level._getValue() == null || sceneValue.risk_level._getValue() == 0){
					alert("请选择第"+(i+1)+"跳拦截项的拦截类型,或者重置其选中状态！");
					return;
				}
		...
		var handleSuccess = function(o){
			if(o.responseText !== undefined) {
				...
				if(flag == "success"){
					alert("保存成功！");
					var url = '<emp:url action="getPrdPvRiskSceneChangeUpdatePage.do"/>?prevent_id='+prevent_id+'&wfid='+wfid+'&scene_id='+scene_id;
					url = EMPTools.encodeURI(url);
					window.location = url;
				}else {
					alert("异步发生异常！");
		...
		var url='<emp:url action="updatePrdPreventRiskPrdPvRiskSceneRecord.do"/>?'+param;
		url = EMPTools.encodeURI(url);
...
<body class="page_content">
	<emp:tabGroup id="PrdPreventRisk" mainTab="base_tab">
		<emp:form id="submitForm" action="updatePrdPreventRiskPrdPvRiskSceneRecord.do" method="POST">
