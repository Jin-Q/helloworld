...
<script type="text/javascript">
	...
	function doDelPrevent(){
		...
		if(data.length == 0){
			...
			if(confirm("是否确认要移除该拦截项，移除该拦截项会同步移除项下流程中的拦截项！")){
				var relArr = "";
				//组装多记录选择返回参数
...
<body class="page_content">
	<emp:tabGroup id="PrdPreventRisk_tabs" mainTab="base_tab">
		<emp:tab label="风险拦截方案" id="base_tab">
			<emp:form id="submitForm" action="updatePrdPreventRiskRecord.do" method="POST">
				<emp:gridLayout id="PrdPreventRiskGroup" title="风险拦截方案" maxColumn="2">
					...
					<emp:select id="PrdPreventRisk.used_flow" label="是否适用流程" required="true" readonly="true" dictname="STD_ZX_YES_NO" />
				...
				<div id="tempButton" style="display:${param.optype};padding-top:10px;border-top:1px solid #c2c2c2;" >
				  	<emp:button id="connPrevent" label="引入拦截项"/>
				  	<emp:button id="delPrevent" label="移除拦截项"/>
				</div>
		...
		%>
		<emp:tab id="PrdPvRiskScene_tab" label="风险拦截场景（适用流程）" url="queryPrdPreventRiskPrdPvRiskSceneList.do" reqParams="PrdPreventRisk.prevent_id=$PrdPreventRisk.prevent_id;" needFlush="true" initial="false" />
		<% 
			}else {
		%>
		<emp:tab id="PrdPvRiskScene_tab" label="风险拦截场景(无流程)" url="queryPrdPvRiskSceneList.do" reqParams="prevent_id=$PrdPreventRisk.prevent_id;" needFlush="true" initial="false" />
		<% 
			}
		%>
	</emp:tabGroup>
