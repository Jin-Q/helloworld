			...
			if(o.responseText !== undefined) {
				...
				if(flag=="新增成功"){
					if(confirm("增加成功！是否继续操作？")){
						...
						var url = '<emp:url action="getCusIndivFamLbyAddPage.do"/>&'+paramStr+"&EditFlag="+EditFlag;
...
<body class="page_content">
	<emp:form id="submitForm" action="addCusIndivFamLbyRecord.do" method="POST">
		<emp:gridLayout id="CusIndivFamLbyGroup" title="负债情况" maxColumn="2">
			...
			<emp:select id="CusIndivFamLby.indiv_debt_cur" label="负债币种" required="true" dictname="STD_ZX_CUR_TYPE" defvalue="CNY"/>
			<emp:text id="CusIndivFamLby.indiv_debt_amt" label="负债金额(元)" maxlength="18" required="true" colSpan="2" dataType="Currency"/>
			...
			<emp:text id="CusIndivFamLby.input_id" label="登记人" maxlength="20" required="false" hidden="true" defvalue="$currentUserId"/>
			<emp:text id="CusIndivFamLby.input_br_id" label="登记机构" maxlength="20" required="false" hidden="true" defvalue="$organNo"/>
			<emp:date id="CusIndivFamLby.input_date" label="登记日期" required="false" hidden="true" defvalue="$OPENDAY"/>
		...
		<div align="center">
			<br>
			<emp:button id="addCusIndivFamLby" label="保存"/>
