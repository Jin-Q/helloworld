...
<script type="text/javascript">
	...
	function doGetUpdateCusScaleApplyPage() {
		var paramStr = CusScaleApplyList._obj.getParamStr(['serno']);
		if (paramStr != null) {
			var status = CusScaleApplyList._obj.getParamValue('approve_status');
			if(status != '000' && status != '992' && status!= '991'&& status!= '993'){
			    alert("该记录已提交审批！");
			    return ;
			}
			var url = '<emp:url action="getCusScaleApplyUpdatePage.do"/>?'+paramStr;
	...
	function doGetAddCusScaleApplyPage() {
		var url = '<emp:url action="getCusScaleApplyAddPage.do"/>';
		url = EMPTools.encodeURI(url);
		window.location = url;
	};
	
	function doDeleteCusScaleApply() {
		var paramStr = CusScaleApplyList._obj.getParamStr(['serno']);
		if (paramStr != null) {
			var status = CusScaleApplyList._obj.getParamValue('approve_status');
			if(status != '000'){
			...
			if(confirm("是否确认要删除？")){
				var url = '<emp:url action="deleteCusScaleApplyRecord.do"/>?'+paramStr;
				url = EMPTools.encodeURI(url);
	...
	function doSumbitCusCognizApply(){
		var paramStr = CusScaleApplyList._obj.getParamValue(['serno']);
		if (paramStr != null) {
			var _status = CusScaleApplyList._obj.getParamValue(['approve_status']);
			var cus_id = CusScaleApplyList._obj.getParamValue(['cus_id']);//客户码
			var cus_name = CusScaleApplyList._obj.getParamValue(['cus_id_displayname']);//客户名称
			WfiJoin.table_name._setValue("CusScaleApply");
			WfiJoin.pk_col._setValue("serno");
			WfiJoin.pk_value._setValue(paramStr);
			WfiJoin.wfi_status._setValue(_status);
			WfiJoin.status_name._setValue("approve_status");
			WfiJoin.appl_type._setValue("004");  //流程申请类型，对应字典项[ZB_BIZ_CATE]，对应流程标识：LMT_APP_FLOW
			WfiJoin.cus_id._setValue(cus_id);
			WfiJoin.cus_name._setValue(cus_name);
			WfiJoin.prd_name._setValue("条线认定申请");
			initWFSubmit(false);
		}else {
			alert('请先选择一条记录！');
		}
...
<body class="page_content">
	...
	<div align="left">
		<emp:button id="getAddCusScaleApplyPage" label="新增" op="add"/>
		<emp:button id="getUpdateCusScaleApplyPage" label="修改" op="update"/>
		<emp:button id="deleteCusScaleApply" label="删除" op="remove"/>
		<emp:button id="viewCusScaleApply" label="查看" op="view"/>
		<emp:button id="sumbitCusCognizApply" label="提交"  op ="startFlow"/>
	</div>

	<emp:table icollName="CusScaleApplyList" pageMode="true" url="pageCusScaleApplyQuery.do">
