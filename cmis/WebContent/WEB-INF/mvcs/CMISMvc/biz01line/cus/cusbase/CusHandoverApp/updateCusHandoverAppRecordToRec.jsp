...
<script type="text/javascript">
	...
	function doReturn(){
		var paramStr = "CusHandoverApp.approve_status=20";
		var url = '<emp:url action="queryCusHandoverAppListRec.do"/>?'+paramStr;
		url = EMPTools.encodeURI(url);
		window.location = url;
	}
	function doCusHandoverApp() {
		var form = document.getElementById('submitForm');
		var paramStr = "status=20"+"&serno="+CusHandoverApp.serno._obj.element.value;
		var url = '<emp:url action="updateCusHandoverAppStatusRec.do"/>?'+paramStr;
		url = EMPTools.encodeURI(url);
		CusHandoverApp._toForm(form);
		//CusHandoverLstList._toForm(form);
		form.action=url;
		form.submit();
...
<body class="page_content" onload="doOnLoad()">
	...
	<emp:form id="submitForm" action="" method="POST">
		...
		<emp:gridLayout id="CusHandoverAppGroup" title="客户移交申请" maxColumn="2">
			...
			<emp:select id="CusHandoverApp.org_type" label="接收机构与移出机构关系" required="true" dictname="STD_ZB_ORG_TYPE" colSpan="2"/>
			...
			<emp:select id="CusHandoverApp.status" label="状态" required="false" dictname="STD_ZB_HAND_STATUS" hidden="true" defvalue="000"/>
...
<form  method="POST" action="#" id="queryForm" >
	...
	<emp:table icollName="CusHandoverLstList" pageMode="true" url="pageCusHandoverLstQuery.do" reqParams="CusHandoverLst.serno=$CusHandoverApp.serno;">
		...
		<div align="center">
			<br>
			<emp:button id="cusHandoverApp" label="接收" op="update"/>
