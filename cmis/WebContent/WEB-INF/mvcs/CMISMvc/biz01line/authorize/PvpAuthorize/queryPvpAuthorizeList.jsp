...
<script type="text/javascript">
	...
	function doAuthorize(){
       ...
       var paramStr = PvpAuthorizeList._obj.getParamStr(['tran_serno']);
		if (paramStr != null) {
			...
			var tran_id = PvpAuthorizeList._obj.getParamValue(['tran_id']);
		    var service_code = tran_id.substr(0,tran_id.length-2);
		    var sence_code = tran_id.substr(tran_id.length-2,tran_id.length);
			if(tran_id=="0200200001201"){//授权交易为担保品出入库特殊处理
				var url = '<emp:url action="clientTrade4EsbMulti.do"/>?'+paramStr+'&service_code='+service_code+'&sence_code='+sence_code;
				}else{
					var url = '<emp:url action="clientTrade4Esb.do"/>?'+paramStr+'&service_code='+service_code+'&sence_code='+sence_code;
				}
			...
			var handleSuccess = function(o){
				...
				if(o.responseText !== undefined) {
					...
					var retMsg = jsonstr.RetInf;
					if(flag == "success"){
						alert("授权成功！");
						window.location.reload();
					}else {
						doQuery();
						alert("授权失败！原因:" + retMsg);
	...
	function doViewPvpAuthorize() {
		var paramStr = PvpAuthorizeList._obj.getParamStr(['tran_serno']);
		if (paramStr != null) {
			var url = '<emp:url action="getPvpAuthorizeViewPage.do"/>?'+paramStr+'&flag=authorize&biz_type='+'<%=biz_type%>';		
			url = EMPTools.encodeURI(url); 
			window.location = url;
		} else {
			alert('请先选择一条记录！');
	...
	//风险拦截
    function interRisk(){
	   var serno = PvpAuthorizeList._obj.getSelectedData()[0].tran_serno._getValue();
	   var _applType="";
	   var _modelId="PvpAuthorize";
	   var _pkVal=serno;
	   var _preventIdLst="FFFA278600A518FCA409BC8019FA600E";
...
<body class="page_content">
	...
	<div align="left">
		<emp:button id="authorize" label="授权" op="add"/>
		<emp:button id="viewPvpAuthorize" label="查看" op="view"/>
	</div>

	<emp:table icollName="PvpAuthorizeList" pageMode="true" url="pagePvpAuthorizeQuery.do" reqParams="biz_type=${context.biz_type}">
