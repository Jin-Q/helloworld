...
<script type="text/javascript">
	...
	function refreshQryParam() {
		QryTemplet_tabs.tabs.QryParam_tab.refresh();
	};
	function refreshQryResult() {
		QryTemplet_tabs.tabs.QryResult_tab.refresh();
	};

	/*--user code begin--*/
	function returnOrganLevel(organLevel){
		QryTemplet.organlevel._setValue(organLevel);
	}			
	function doReturn(){
...
<body class="page_content" >
	<emp:form id="submitForm" action="updateQryTempletRecord.do" method="POST">
		<emp:gridLayout id="QryTempletGroup" title="查询信息配置模板" maxColumn="2">
			...
			<emp:text id="QryTemplet.temp_name" label="查询名称" maxlength="200" required="true" colSpan="2" cssElementClass="emp_input2" />
		...
		<div class="note" > <span class="note_tips"></>注意:SQL关键字请使用小写形式</span><br>
...
9.	系统当前登录人角色ID：roles（可能存在多个角色信息） " required="false" readonly="true"/>
		...
		<div align=center>
			<emp:button id="preSubmit" label="修改" op="update"/>
			<emp:button id="reset" label="重置"/>
			<emp:button id="return" label="返回"/>
		</div>
	</emp:form>
	<emp:tabGroup id="QryTemplet_tabs" mainTab="QryParam_tab">
		<emp:tab id="QryParam_tab" label="查询条件参数配置表" url="queryQryTempletQryParamList.do" reqParams="QryTemplet.temp_no=$QryTemplet.temp_no;" initial="true"/>
				
		<emp:tab id="QryResult_tab" label="查询返回值配置表" url="queryQryTempletQryResultList.do" reqParams="QryTemplet.temp_no=$QryTemplet.temp_no;"/>
	</emp:tabGroup>
</body>
</html>
</emp:page>
