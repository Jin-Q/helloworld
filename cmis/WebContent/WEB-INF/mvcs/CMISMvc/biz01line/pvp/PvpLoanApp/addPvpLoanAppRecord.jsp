...
<script type="text/javascript">
	...
	function doSelect(){
		...
		if (data != null && data.length !=0) {
			...
			var url = '<emp:url action="queryIsConfPvp.do"/>?cont_no='+cont_no;
	...
	function doSave(){
		...
		var handleSuccess = function(o){
			if(o.responseText !== undefined) {
				...
				if(flag == "success"){
					url = '<emp:url action="getPvpLoanAppUpdatePage.do"/>?menuId=queryPvpLoanAppList&serno='+serno+'&biz_type=${context.biz_type}';
					url = EMPTools.encodeURI(url);
					window.location = url;
				}else {
					//added by yangzy 20140829 放款改造  begin
		...
		var url = '<emp:url action="addPvpLoanAppRecord.do"/>?cont_no='+cont_no;
		url = EMPTools.encodeURI(url);
		var postData = YAHOO.util.Connect.setForm(form);	
		var obj1 = YAHOO.util.Connect.asyncRequest('POST',url, callback,postData) 
	}
...
<emp:form id="submitForm" action="" method="POST">
	<div  class='emp_gridlayout_title'>新增出账  </div> 
		...
		<emp:table icollName="CtrLoanContList" pageMode="true" url="pageAddPvpLoanApp.do">
		   ...
		   <emp:text id="in_acct_br_id" label="入账机构" required="false" readonly="true" hidden="true"/>
		   <emp:text id="input_br_id" label="登记机构" maxlength="20" required="false" defvalue="${context.organNo}" hidden="true"/>
		   <emp:date id="input_date" label="登记日期" required="false" defvalue="${context.OPENDAY}" hidden="true"/>
		   <emp:select id="approve_status" label="审批状态" required="false" dictname="WF_APP_STATUS" defvalue="000" hidden="true"/>
		</emp:table>
		<div align="center">
			<br>
