...
<script type="text/javascript">
	...
	function doGetUpdatePvpLoanAppPage() {
		...
		if (paramStr != null) {
			var url = '<emp:url action="getPvpAssetTransUpdatePage.do"/>?'+paramStr;
	...
	function doViewPvpLoanApp() {
		...
		if (paramStr != null) {
			var url = '<emp:url action="getPvpAssetTransViewPage.do"/>?flag=assettrans&'+paramStr;
			url = EMPTools.encodeURI(url);
			//window.location = url;
			var param = 'height=700, width=1200, top=80, left=80, toolbar=no, menubar=no, scrollbars=yes, resizable=yes, location=no, status=no';
			window.open(url,'newWindow',param);
	...
	function doGetAddPvpLoanAppPage() {
		var url = '<emp:url action="getPvpAssetTransAddPage.do"/>?menuId=xzzclzcz';
		url = EMPTools.encodeURI(url);
		window.location = url;
	};
	...
	function doDeletePvpLoanApp() {
		...
		if (paramStr != null){ 
			...
			if(confirm("是否确认要删除？")){
				...
				var url = '<emp:url action="deletePvpAssetTransRecord.do"/>?'+paramStr;
				url = EMPTools.encodeURI(url); 
...
<body class="page_content">
	...
	<emp:table icollName="PvpLoanAppList" pageMode="true" url="pagePvpAssetTransQuery.do">
		...
		<emp:text id="cus_id_displayname" label="交易对手行名" />
