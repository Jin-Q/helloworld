...
<script type="text/javascript">
	...
	function doSelect(){
		var data = CtrAssetTransContList._obj.getSelectedData();
		if (data != null && data.length !=0) {
			...
			var url = '<emp:url action="queryIsConfPvpForAssetTrans.do"/>?menuId=zclzczsq&cont_no='+cont_no;
	...
	function doSave(){
		...
		var handleSuccess = function(o){
			if(o.responseText !== undefined) {
				...
				if(flag == "success"){
					url = '<emp:url action="getPvpAssetTransUpdatePage.do"/>?op=update&cont_no='+cont_no+'&serno='+serno;
					url = EMPTools.encodeURI(url);
					window.location = url;
				}else {
					alert("异步请求出错！");
		...
		var url = '<emp:url action="addPvpAssetTransRecord.do"/>?cont_no='+cont_no;
		url = EMPTools.encodeURI(url);
		var postData = YAHOO.util.Connect.setForm(form);	
		var obj1 = YAHOO.util.Connect.asyncRequest('POST',url, callback,postData) 
	}
...
<body class="page_content">
	...
	<emp:form id="submitForm" action="" method="POST"></emp:form>
		<emp:table icollName="CtrAssetTransContList" pageMode="true" url="pageAddPvpLoanApp.do">
		   ...
		   <emp:text id="cont_status" label="项目状态" dictname="STD_ZB_CTRLOANCONT_TYPE" />
