...
<script type="text/javascript">
	...
	function doGetUpdatePvpLoanAppPage() {
		...
		if (paramStr != null) {
			var url = '<emp:url action="getPvpAssetstrsfUpdatePage.do"/>?'+paramStr;
	...
	function doViewPvpLoanApp() {
		...
		if (paramStr != null) {
			var url = '<emp:url action="getPvpAssetstrsfViewPage.do"/>?flag=assetstrs&'+paramStr;
			url = EMPTools.encodeURI(url);
			//window.location = url;
	...
	function doGetAddPvpLoanAppPage() {
		var url = '<emp:url action="getPvpAssetstrsfAddPage.do"/>?menuId=addQueryPvpAssetstrsfPage';
		url = EMPTools.encodeURI(url);
		window.location = url;
	};
	...
	function doDeletePvpLoanApp() {
		...
		if (paramStr != null){ 
			...
			if(confirm("是否确认要删除？")){
				...
				var url = '<emp:url action="deletePvpAssetstrsfRecord.do"/>?'+paramStr;
				url = EMPTools.encodeURI(url); 
...
//------生成授权信息------
	...
	//-----------通过产品编号查询产品配置中使用流程类型----------
	function doGetApplyTypeByPrdId(){
	...
	function doSubmitWF(apply_type){
		...
		var cus_name = PvpLoanAppList._obj.getSelectedData()[0].toorg_name._getValue();
...
<body class="page_content">
	...
	<div align="left">
		...
		<emp:button id="getApplyTypeByPrdId" label="提交" op="submit"/>
	...
	<emp:table icollName="PvpLoanAppList" pageMode="true" url="pagePvpAssetstrsfQuery.do">
