...
<script type="text/javascript">
	...
	function doViewPvpLoanApp() {
		...
		if (paramStr != null) {
			var url = '<emp:url action="getPvpRpddscantViewPage.do"/>?&showButton=Y&'+paramStr;
			url = EMPTools.encodeURI(url);
			//window.location = url;
			var param = 'height=700, width=1200, top=80, left=80, toolbar=no, menubar=no, scrollbars=yes, resizable=yes, location=no, status=no';
			window.open(url,'newWindow',param);
	...
	function doGetAddPvpLoanAppPage() {
		var url = '<emp:url action="getPvpRpddscantAddPage.do"/>?menuId=addPvpRpddscant';
		url = EMPTools.encodeURI(url);
		window.location = url;
	};
...
//------生成授权信息------
	...
	function doGetApplyTypeByPrdId(){
		/**modified by lisj 2015-2-12 修复提交操作无选择项报错BUG，于2015-2-12 上线 begin**/
		var paramStr = PvpLoanAppList._obj.getParamStr(['serno']);
		if(paramStr != null){
		...
		/**modified by lisj 2015-2-12 修复提交操作无选择项报错BUG，于2015-2-12 上线 end**/
	};
	//-----------提交流程----------
...
<body class="page_content">
	...
	<emp:table icollName="PvpLoanAppList" pageMode="true" url="pagePvpRpddscantQuery.do">
