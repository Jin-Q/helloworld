...
<script type="text/javascript">
	function doReturn1() { 
        ...
        }else if('<%=flag%>'=="assettrans"){
        	var url = '<emp:url action="queryPvpAssetTransList.do"/>';
        }else if('<%=flag%>'=="assetpro"){
        	var url = '<emp:url action="queryPvpAssetProList.do"/>';
        }else{
...
<body class="page_content" >
		 ...
		 <emp:tab label="出账信息" id="base_tab" needFlush="true" initial="true" >
          <emp:gridLayout id="CtrLoanContGroup" maxColumn="2" title="出账基本信息">
			...
			<emp:text id="PvpLoanApp.cus_id_displayname" label="交易对手行名"  required="false" colSpan="2" readonly="true" cssElementClass="emp_input2"/>
		...
		<emp:tab label="资产证券化项目信息" id="subTab" url="getCtrAssetProContViewPage.do?cont_no=${context.PvpLoanApp.cont_no}&menuIdTab=zczqhxmcx&op=view&flag=notHave" initial="false" needFlush="true"/>
	</emp:tabGroup>
	<div align="center">
	<br>
	    <%if(!"assetpro".equals(flag)){ %>
