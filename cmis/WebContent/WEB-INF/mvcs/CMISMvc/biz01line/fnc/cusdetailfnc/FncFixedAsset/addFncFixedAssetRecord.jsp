...
<script type="text/javascript">
	...
	function returnCus(data){
		FncFixedAsset.cus_id._setValue(data.cus_id._getValue());
		FncFixedAsset.cus_name._setValue(data.cus_name._getValue());
	};
	function doReturn() {
...
<body class="page_content">
	...
	<emp:form id="submitForm" action="addFncFixedAssetRecord.do" method="POST">
		<emp:gridLayout id="FncFixedAssetGroup" maxColumn="2" title="主要固定资产明细表">
			<emp:pop id="FncFixedAsset.cus_id" label="客户码" url="selectCusInfoPop.do?type=com" returnMethod="returnCus" required="true" />
			<emp:text id="FncFixedAsset.cus_name" label="客户姓名" maxlength="80" required="true" />
			...
			<emp:text id="FncFixedAsset.input_date" label="登记日期" maxlength="10" required="false" defvalue="$OPENDAY" readonly="true"/>
				<!-- 下面是真实值字段 -->	
		<emp:text id="FncFixedAsset.last_upd_id" label="更新人" defvalue="${context.currentUserId}" hidden="true"/>
			<emp:text id="FncFixedAsset.last_upd_date" label="更新日期" defvalue="$OPENDAY" hidden="true"/>
