...
function returnCus(data){
	FncOtherPayable.cus_id._setValue(data.cus_id._getValue());
	FncOtherPayable.cus_name._setValue(data.cus_name._getValue());
}
function checkZero(data){
	var x=data.length;
	if(x !=6){
 		alert("年月的位数必须为6位,格式为YYYYMM！");
 		FncOtherPayable.fnc_ym._setValue("");
	}
	else{
		var nn;
		nn=new Date();
...
<body class="page_content">
	...
	<emp:form id="submitForm" action="addFncOtherPayableRecord.do" method="POST">
