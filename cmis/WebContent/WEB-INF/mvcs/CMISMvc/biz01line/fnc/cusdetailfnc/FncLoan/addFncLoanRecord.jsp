...
function returnCus(data){
	FncLoan.cus_id._setValue(data.cus_id._getValue());
	FncLoan.cus_name._setValue(data.cus_name._getValue());
}
function checkmoney(){
...
<body class="page_content">
	...
	<emp:form id="submitForm" action="addFncLoanRecord.do" method="POST">
		...
		<emp:gridLayout id="FncLoanGroup" title="主要借款明细" maxColumn="2">
			...
			<emp:date  id="FncLoan.input_date" label="登记日期" defvalue="$OPENDAY" readonly="true" required="true" />
				...
				<!-- 下面是真实值字段 -->	
				<emp:text id="FncLoan.input_id" label="登记人" maxlength="20" required="false" defvalue="$actorname" hidden="true" />
			    <emp:text id="FncLoan.input_br_id" label="登记机构" maxlength="20" required="false"  defvalue="$organName" hidden="true" />
			  	<emp:text id="FncLoan.last_upd_id" label="更新人" defvalue="${context.currentUserId}" hidden="true"/>
			<emp:text id="FncLoant.last_upd_date" label="更新日期" defvalue="$OPENDAY" hidden="true"/>
