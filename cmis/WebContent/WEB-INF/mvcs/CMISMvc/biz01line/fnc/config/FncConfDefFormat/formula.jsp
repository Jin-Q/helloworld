...
<%  
	 ...
	 //String gs = new String(request.getParameter("gs").getBytes("ISO-8859-1"));
...
<script type="text/javascript">
	...
	function changeItems(){
		var fnc_conf_typ = document.all["sel_type"].value ;		
	    var paramStr ="fnc_conf_typ="+fnc_conf_typ;  
    ...
    <tr>
      <td width="475" valign="top" bgcolor="#FFFFFF"><br>
        ...
        <table width="99%" border="1" align="center" cellpadding="5" cellspacing="0">
         <tr>
            <td width="36%">引用报表</td>
            <td width="64%"><select name="sel_type" id="select" onchange="changeItems()">
                <option value="01">[01]资产负债表</option>
                <option value="02">[02]损益表</option>
                <option value="03">[03]现金流量</option>
                <option value="04">[04]财务指标</option>
				<option value="05">[05]所有者权益变动表</option>
				<option value="06">[06]财务简表</option>
              </select>
              </td>    
          </tr>
          <tr> 
              <td width="36%">引用指标 </td>
               <td width="64%"><emp:select id="item_id" label="项目编号" required="false" cssElementClass="_select"/></td>
          </tr> 
          <tr>
            <td width="36%">引用数据</td>
            <td width="64%"><select name="sel_datasrc" id="sel_datasrc" >
                <option value="1">[1]期初数</option>
                <option value="2">[2]期末数</option>
                <option value="1">[1]上年同期数</option>
                <option value="2">[2]本年累计数</option>
                <option value="1">[1]金额</option>
              </select></td>
          </tr>
          <tr> 
               <td><div align="center"> 
                <input name="b_insert" type="button" id="b_insert2" value="插入" onClick="insertOther()">
               </div></td>
          </tr>
        </table></td>
      <td width="167" valign="top" bgcolor="#FFFFFF"><table width="165" height="308" border="0" align="center" cellpadding="0" cellspacing="0">
          ...
          <tr>
            <td><p align="center">
                <input name="b_max" type="button"  onClick="inputStr('MAX()')" value="MAX">
                <input name="b_min" type="button" id="b_min2" onClick="inputStr('MIN()')" value="MIN">
                <input name="b_int" type="button" id="b_int2" onClick="inputStr('INT()')" value="INT">
              </p></td>
          </tr>
          <tr>
            <td> <div align="center">
                <input name="s_rank" type="button" id="s_rank" onClick="inputStr('@RANK()')" value="RANK">
                <input name="s_zone" type="button" id="s_zone" onClick="inputStr('@ZONE()')" value="ZONE">
              </div></td>
          </tr>
          <tr>
...
function insertOther(){
	var string;
  	string="{"+document.all["sel_type"].options.item(document.all["sel_type"].selectedIndex).text
  	+"."+item_id._obj.element.value
  	+"."+document.all["sel_datasrc"].options.item(document.all["sel_datasrc"].selectedIndex).text+"}";
	inputStr(string);
}
...
function check()
{
	...
	var leftCount   = getFlagCount(formula,"{");
	var rightCount  = getFlagCount(formula,"}");
...
function getFlagCount(str,flag){
	   var count=0;
