...
<script>
	...
	function doView() {
		...
		if (paramStr != null) {
			...
			var url = '<emp:url action="fncStat_view.do"/>&'+paramStr+"&EditFlag="+editFlag+"&isSmp=isSmp";
	...
	function doView2() {
		...
		if (paramStr != null) {
			...
			var editFlag = '${context.EditFlag}';
			var url = '<emp:url action="fncStat.do"/>&'+paramStr+"&EditFlag="+editFlag+"&isSmp=isSmp";
			url = EMP.util.Tools.encodeURI(url);
			window.location = url;
		} else {
			alert('请先选择一条记录！');
	...
	//	var flag = '${context.flag}';
		var editFlag = '${context.EditFlag}';
		var url = '<emp:url action="getFncStatBaseAddPage.do"/>?cus_id='+"<%=cus_id%>&EditFlag="+editFlag+"&isSmp=isSmp";
		url = EMP.util.Tools.encodeURI(url);
		window.location = url;
	};
				...
				var handleSuccess = function(o){
					if(o.responseText !== undefined) {
						...
						if(flag=="success"){
							...
							var url = '<emp:url action="queryFncStatBaseSmpList.do"/>&FncStatBase.cus_id='+cus_id+"&EditFlag="+editFlag+"&isSmp=isSmp";
							url = EMPTools.encodeURI(url);
							window.location = url;
				 	  	}else {
					 		alert(flag);
...
//	String flag=(String)request.getSession().getAttribute("buttonFlag");
	...
	<emp:table icollName="FncStatBaseList" pageMode="true" url="pageFncStatBaseQuery.do?FncStatBase.cus_id=${context.FncStatBase.cus_id}&EditFlag=${context.EditFlag}&isSmp=${context.isSmp}">
