...
//String opdate = (String)context.getDataValue("OPENDAY");
//是否是财务简表
...
<script type="text/javascript">
	...
	function doOnLoad() {
		if('${param.flag}' == 'WF'){
			var cus_id = window.parent.IqpMeLoanApp.cus_id._getValue();
			var cus_name = window.parent.IqpMeLoanApp.cus_name._getValue();
			FncStatBase.cus_id._setValue(cus_id);
			FncStatBase.cus_name._setValue(cus_name);
			FncStatBase.cus_id._obj._renderReadonly(true);
			FncStatBase.cus_name._obj._renderReadonly(true);
		}
		//原来的做法
		EMPTools.addEvent(FncStatBase.stat_prd_style._obj.element, "change", doChange, FncStatBase.stat_prd_style);
		//EMPTools.addEvent(FncStatBase.stat_prd_year._obj.element, "changey", doChange, FncStatBase.stat_prd_year);
		//新的做法
	};

/*	function getYYYYMM(obj){
		var prd=obj.value;
		var stat_prd= FncStatBase.stat_prd._getValue();
		var prdy=prd.substring(0,4);
		var prdm=prd.substring(5,7);
		//FncStatBase.stat_prd_date._setValue(prdy+prdm);
		FncStatBase.stat_prd._setValue(prdy+prdm);
		//doSelectedStatPrd();
		return  FncStatBase.stat_prd._obj.element.focus();
	}*/
	
	function doAddFncStatBase(){
		//return alert(FncStatBase.stat_prd._obj.element.value+"!!!!!!");
		 //doSelectedStatPrd();
		...
		var handleSuccess = function(o){
			if(o.responseText !== undefined) {
				...
				if(flag=="exist"){
					...
					return;
			/*		if(state_flg !='222299992'){
						if(confirm("该客户财报信息已经存在！\n点【确定】将进入维护财报信息！\n点【取消】将进入列表页面！")){
							var paramStr="cus_id="+cus_id+"&stat_prd_style="+stat_prd_style
							+"&stat_prd="+stat_prd+"&state_flg="+state_flg
							+"&stat_style="+stat_style+"&EditFlag="+editFlag;
							var url = '<emp:url action="fncStat.do"/>&'+paramStr;
						...
						alert("该客户财报信息已经存在且状态为'完成',无法进行新增操作！");
					}*/
			    	...
			    	if('${param.flag}' == 'WF'){
			    		...
			    		/*var stat_prd_style = FncStatBase.stat_prd_style._obj.element.value;
						var stat_prd =  FncStatBase.stat_prd._obj.element.value;
						var state_flg =  FncStatBase.state_flg._obj.element.value;
						var stat_style =  FncStatBase.stat_style._obj.element.value;*/
			    		var paramStr="cus_id="+cus_id+"&stat_prd_style="+stat_prd_style
						+"&stat_prd="+stat_prd+"&state_flg="+state_flg
						+"&stat_style="+stat_style+"&EditFlag="+editFlag+"&fnc_type="+fnc_type;				    		
			    		var url = '<emp:url action="fncStat.do"/>&'+paramStr;
				    }
					url = EMPTools.encodeURI(url);
					window.location = url;
	...
	function doReturn() {
		if(window.parent && window.parent.refreshFnc){
			...
			var isSmp = '<%=isSmp%>';
			var paramStr="FncStatBase.cus_id="+FncStatBase.cus_id._obj.element.value+"&EditFlag=<%=editFlag%>";
			var url = '<emp:url action="queryFncStatBaseSmpList.do"/>&'+paramStr+"&isSmp=isSmp";
			if(isSmp==null||isSmp==''||isSmp=='null'){
				url = '<emp:url action="queryFncStatBaseList.do"/>&'+paramStr;
			}
...
/*	
	function idRupervise(data){
		var managerId=data.cus_id._getValue();
		var managerIdName=data.cus_name._getValue();
        var url = '<emp:url action="CheckCusId.do"/>&cus_id='+managerId;
        var handleSuccess = function(o){
			if(o.responseText != undefined){
				try {
					var jsonstr = eval("("+o.responseText+")");
				...
				var flag = jsonstr.flag;
				if(flag =='2') {
				  if(managerId!=null && managerIdName!=null){
					  FncStatBase.cus_id._setValue(managerId);
					  FncStatBase.cus_name._setValue(managerIdName);
					  FncStatBase.stat_style._setValue('2');
					  FncStatBase.stat_style._obj._renderReadonly(true);
			         }else if(flag =='3'){
						  FncStatBase.cus_id._setValue(managerId);
						  FncStatBase.cus_name._setValue(managerIdName);
						  FncStatBase.stat_style._setValue('1');
						  FncStatBase.stat_style._obj._renderReadonly(true);
					}else{
						FncStatBase.cus_id._setValue(managerId);
					    FncStatBase.cus_name._setValue(managerIdName);
...
<div id="FncStatBaseGroup" class="emp_group_div">
	<emp:gridLayout id="FncStatBaseGroup" maxColumn="2" title="公司客户报表">
			<emp:text id="FncStatBase.cus_id" label="客户码"  required="true" colSpan="2" defvalue="<%=cus_id %>" hidden="true"/>
			<emp:select id="FncStatBase.fnc_type" label="财务报表类型" dictname="STD_ZB_FIN_REP_TYPE" required="true"/>
