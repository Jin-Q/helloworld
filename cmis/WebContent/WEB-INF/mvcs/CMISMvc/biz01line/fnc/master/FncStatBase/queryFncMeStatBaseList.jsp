...
//			}		
		 ...
		 if(style_value=="4"){
			...
			for(var i=1;i<=2;i++){
				...
				select.options[i].value = ""+curYear+(i+11);
	...
	function doGetAddFncStatBasePage() {
		...
		var url = '<emp:url action="getFncStatBaseAddPage.do"/>?flag='+flag;
		url = EMP.util.Tools.encodeURI(url);
		window.location = url;
	};
	...
	<form id="improtForm" action="<emp:url action='importReport.do'/>" method="POST" enctype="multipart/form-data">
		<emp:text id="FncStatBaseTmp.cus_id" label="客户码"  hidden="true"/>
		<emp:text id="FncStatBaseTmp.cus_name" label="客户名称"  hidden="true"/>
 	 ...
 	 <div align="left" style="display:${param.flg eq 'query'? '' : 'none'}">
 	    <emp:button id="getAddFncStatBasePage" label="新增" op="add"/>
	    <!-- emp:button id="getUpdateFncStatBasePage" label="修改"/-->
	    <emp:button id="deleteFncStatBase" label="删除" op="delete"/> 
	    <emp:button id="view" label="查看" op="view"/> 
	    <emp:button id="view2" label="修改" op="update"/> 
	     <emp:button id="importReport" label="导入EXCEL" op="importRp"/>
	     <emp:button id="exportReport" label="导出EXCEL" op="exportRp"/>
    </div>
 	<div align="left" style="display:${param.flag eq 'WF'? '' : 'none'}">
		 <!-- <emp:button id="getAddFncStatBasePage" label="新增" op="add"/> -->
	    <!-- emp:button id="getUpdateFncStatBasePage" label="修改"/-->
	    <emp:button id="deleteFncStatBase" label="删除" op="delete"/> 
	    <emp:button id="view" label="查看" op="view"/> 
	...
	<emp:table icollName="FncStatBaseList" pageMode="true" url="pageFncStatBaseQuery.do">
	   ...
	   <emp:text id="cus_name" label="客户名称" hidden="false"/>
		...
		<emp:select id="stat_prd" label="报表期间"  required="true" onchange="linkChangeStatPrd()"/>
