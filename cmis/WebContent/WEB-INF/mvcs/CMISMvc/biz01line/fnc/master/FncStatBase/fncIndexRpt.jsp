<%@page language="java" contentType="text/html; charset=UTF-8"%>
<%@taglib uri="/WEB-INF/emp.tld" prefix="emp" %>
<emp:page>
<html>
<head>
<title>企业财务报表</title>

<jsp:include page="/include.jsp" />
<link href="<emp:file fileName='styles/emp/rpt.css'/>" rel="stylesheet" type="text/css" />
<script src="<emp:file fileName='scripts/emp/rpt.js'/>" type="text/javascript" language="javascript"></script>

<script type="text/javascript">
	
	function doOnLoad() {
		var flg = document.getElementById('state_flg').value;
		//var tempFlg = flg.substring(0,3);//获得资产，损益表的状态,是否为22
		var fncStatBsFlg = flg.substring(0,1);//资产负债表状态
		var fncStatIsFlg = flg.substring(1,2);//损益表状态
		var fncStatCfsFlg = flg.substring(2,3);//现金流量表状态
		var fncStatSlFlg = flg.substring(5,6);//财务简表状态
	
	/*	if(fncStatSlFlg!=9){
			if(fncStatSlFlg!=2){
				alert("请先完成【财务简表】报表信息！");
				window.parent.rptGroup.tabs['cwjb']._clickLink();
				return;
			}
		}*/
		
		if(fncStatBsFlg!=2){
			alert("请先完成【资产负债表】报表信息！");
			window.parent.rptGroup.tabs['zcfzb']._clickLink();
			return;
		}
		if(fncStatIsFlg!=2){
			alert("请先完成【损益表】报表信息！");
			window.parent.rptGroup.tabs['syb']._clickLink();
			return;
		}
		if(fncStatCfsFlg!=2){
			alert("请先完成【现金流量表】报表信息！");
			window.parent.rptGroup.tabs['xjllb']._clickLink();
			return;
		}

		var str = document.getElementById('errList').value;
	    if(str!=""){
	    	alert(str);
	    }
		
	};
	function doTempAddFncIndexRpt(){
		var form = document.getElementById('submitForm');
		var action = '<emp:url action="tempAddFncIndexRpt.do"/>';
		
		var result = rptstyle._checkAll();
		if(result){
			rptstyle._toForm(form)
			form.action = action;
			form.submit();
		}
	};
	
	function doAddFncIndexRpt(){
		var form = document.getElementById('submitForm');
		var action = '<emp:url action="addFncIndexRpt.do"/>';
		var result = rptstyle._checkAll();
		if(result){
			rptstyle._toForm(form)
			form.action = action;
			form.submit();
		}
	};
	
	function doUpdateFncIndexRpt(){
		var form = document.getElementById('submitForm');
		var action = '<emp:url action="updateFncIndexRpt.do"/>';
		
		var result = rptstyle._checkAll();
		if(result){
			rptstyle._toForm(form)
			form.action = action;
			form.submit();
		}
	};
    
    function doCheckButton(){
    	var flg = state_flg._getValue();
		var str = flg.substring(0,5);
		if(str == 2222){
			document.getElementById("div1").style.display = "none";
		}else{
			document.getElementById("div1").style.display = "block";
		}
    };

    function doCheckButton(){
    	var cus_id = document.getElementById('cus_id').value;
		var stat_prd_style = document.getElementById('stat_prd_style').value;
		var stat_prd = document.getElementById('stat_prd').value;
		var stat_style = document.getElementById('stat_style').value;
		var fnc_type = document.getElementById('fnc_type').value;
		
		var param = "cus_id="+cus_id+"&stat_prd_style="+stat_prd_style+"&stat_prd="+stat_prd
		+"&fnc_conf_typ=04&stat_style="+stat_style+"&fnc_type="+fnc_type;
        
    	var stockURL = '<emp:url action="queryFncIndexRptEdit.do"/>&'+param;
		stockURL = EMPTools.encodeURI(stockURL);
		window.location = stockURL;
    	
    }
</script>
</head>
<body  class="page_content" onload="doOnLoad()">

	<form id="submitForm" action="#" method="POST">
		<emp:text id="cus_id" label="客户码" hidden="true"/>
		<emp:text id="stat_prd_style" label="报表周期类型" hidden="true"/>
		<emp:text id="stat_prd" label="报表期间" hidden="true"/>
		<emp:text id="fnc_conf_data_col" label="数据列数" hidden="true"/>
		<emp:text id="fnc_name" label="报表名称"  hidden="true"/>
		<emp:text id="style_id" label="报表样式编号"  hidden="true"/>
		<emp:text id="state_flg" label="状态"  hidden="true"/> 
		
		<emp:text id="fnc_conf_typ" label="报表类型"  hidden="true"/>
		<emp:text id="stat_style" label="报表口径" hidden="true"/>
		<emp:text id="errList" label="公式校验信息"  hidden="true"/>
		<emp:text id="fnc_type" label="财报类型"  hidden="true"/>
	</form>
	
	<emp:rpt id="rptstyle" cusComRptId="fncstatbase" editFlag="editFlag"/>
	
	<div align="center">
		<emp:button id="checkButton" label="编辑"/>&nbsp;&nbsp;
   	</div>
	</body>
</html>
</emp:page>

