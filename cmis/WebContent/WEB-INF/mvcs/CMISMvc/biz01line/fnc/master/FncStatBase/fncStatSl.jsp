...
<script type="text/javascript">
	...
	function getParam(){
		...
		"&fnc_conf_data_col="+fnc_conf_data_col+"&fnc_name="+fnc_name+"&style_id="+style_id+"&state_flg="+state_flg+"&fnc_conf_typ=06&fnc_type="+fnc_type;
		return param;
	}
	
	function doTempAddFncStatSl(){

		var form = document.getElementById('submitForm');
		var action = '<emp:url action="tempAddFncStatSl.do"/>';
	...
	function doAddFncStatSl(){
		var form = document.getElementById('submitForm');
		var action = '<emp:url action="addFncStatSl.do"/>';
		var result = rptstyle._checkAll();
		if(result){
			rptstyle._toForm(form)
			form.action = action;
	...
	function doUpdateFncStatSl(){

		var form = document.getElementById('submitForm');
		var action = '<emp:url action="addOrupdateFncStatSl.do"/>';
    ...
    function doCheckButton(){
		...
		var fnc_type = document.getElementById('fnc_type').value;
		var param = "cus_id="+cus_id+"&stat_prd_style="+stat_prd_style+"&stat_prd="+stat_prd
		+"&fnc_conf_typ=06&stat_style="+stat_style+"&fnc_type="+fnc_type;
        
    	var stockURL = '<emp:url action="queryFncStatSlEdit.do"/>&'+param;
