...
//			}		
	...
	function doView() {
		...
		if (paramStr != null) {
			var url = '<emp:url action="cusFncStat_view.do"/>&'+paramStr;
	...
	function doGetAddFncStatBasePage() {
		var url = '<emp:url action="getCusFncStatBaseAddPage.do"/>';
		url = EMP.util.Tools.encodeURI(url);
		window.location = url;
	};
	...
	<form id="improtForm" name="improtForm" action="<emp:url action='importReport.do?EditFlag=cusFnc'/>"  method="POST" enctype="multipart/form-data">
 ...
 <div align="left">
		...
		<emp:button id="deleteFncStatBase" label="删除" op="remove"/> 
		<emp:button id="view" label="查看" op="view"/> 
		<emp:button id="view2" label="修改" op="update"/> 
		<emp:button id="importReport" label="导入EXCEL" locked="false" op="importReport"/>
		<emp:button id="exportReport" label="导出EXCEL" locked="false" op="exportReport"/>
		<emp:button id="downLoadTmplate" label="下载模板" locked="false" op="downLoadTmplate"/>
