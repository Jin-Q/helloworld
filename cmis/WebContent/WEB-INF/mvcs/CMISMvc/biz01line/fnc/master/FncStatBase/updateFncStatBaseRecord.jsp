...
<script type="text/javascript">
	...
	function doOnLoad() {
		//console.log(FncStatBase.stat_is_audit);
		checkStatChange(FncStatBase.stat_is_audit._obj.value);
		checkAdjtChange(FncStatBase.stat_is_adjt._obj.value);	
	}
	...
	function checkStatChange(value){	
		...
		if(value==1){
			FncStatBase.stat_adt_entr._obj._renderReadonly(false);
			FncStatBase.stat_adt_entr._obj._renderRequired(true);
			FncStatBase.stat_adt_conc._obj._renderReadonly(false);
	...
	function checkAdjtChange(value){	
		...
		if(value==1){
			FncStatBase.stat_adj_rsn._obj._renderReadonly(false);
...
<body class="page_content" onload="doOnLoad()" >
	<form id="submitForm" action="<emp:url action='updateFncStatBaseRecord.do'/>" method="POST">
		...
		<emp:gridLayout id="FncStatBaseGroup" maxColumn="2" title="公司客户报表">
			...
			<emp:text id="FncStatBase.state_flg" label="状态" maxlength="9" required="true" readonly="true" />
			<emp:select id="FncStatBase.stat_is_nrpt" label="是否新准则报表" required="true" dictname="STD_ZX_YES_NO" />
			<emp:select id="FncStatBase.stat_style" label="报表口径" required="true" dictname="STD_ZB_FNC_STYLE" readonly="true"/>
			<emp:select id="FncStatBase.stat_is_audit" label="是否经过审计"  required="true" dictname="STD_ZX_YES_NO" onchange="checkStatChange(this.value)"/>
			<emp:select id="FncStatBase.stat_is_adjt" label="是否经过调整" required="true" dictname="STD_ZX_YES_NO" onchange="checkAdjtChange(this.value)"/>
			<emp:text id="FncStatBase.stat_adt_entr" label="审计单位" maxlength="60" required="false" cssElementClass="emp_field_text_input_0"/>
			<emp:text id="FncStatBase.input_date" label="登记日期" maxlength="10" required="false" hidden="true" />
			<emp:textarea id="FncStatBase.stat_adt_conc" label="审计结论" maxlength="200" required="false" />
			<emp:text id="FncStatBase.last_upd_date" label="更新时间" maxlength="10" required="false" hidden="true" />
			<emp:textarea id="FncStatBase.stat_adj_rsn" label="财务报表调整原因" maxlength="200" required="false"/>
	...
	<div align="center">
		<br>
		<emp:button id="updateFncStatBase" label="保存"/>
