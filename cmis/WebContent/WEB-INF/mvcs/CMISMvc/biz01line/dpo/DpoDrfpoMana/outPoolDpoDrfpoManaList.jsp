			...
			if(confirm("是否确认出池？")){
				var handleSuccess = function(o) {
					if (o.responseText !== undefined) {
						...
						if("success" == flag){
							alert("所选择汇票记录已成功出池记账！");
							window.location.reload();
						/* added by yangzy 2014/12/11 应收账款类出池校验，在池是否覆盖敞口 start */
						}else if(flag == "error"){
							alert("[在池票据总金额+保证金金额]不能覆盖池担保占用敞口，不能出池！");
							window.location.reload();
				...
				var url = '<emp:url action="doSureOutPool.do"/>?porder_no='+encodeURI(porder_no)+'&drfpo_no='+encodeURI(drfpo_no);
...
<body class="page_content">
	...
	<div class='emp_gridlayout_title'>已出池票据列表&nbsp;</div>
	...
	<div align="left">
		...
		<emp:button id="sureOutPool" label="确认出池" op=""/>
		<emp:button id="sureOutPool_p" label="出池单打印" op=""/>
	</div>
</body>
</html>
</emp:page>
