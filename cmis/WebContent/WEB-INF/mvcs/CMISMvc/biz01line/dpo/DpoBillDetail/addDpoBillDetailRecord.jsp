...
<script type="text/javascript">
	...
	function doAdd(){
		if(!IqpBillDetailInfo._checkAll()){
			...
			var handleSuccess = function(o){
				if(o.responseText !== undefined) {
					...
					if(flag == "success"){
						...
						window.close();
						<%}else{%>
						var url = '<emp:url action="queryDpoBillDetailList.do"/>?drfpo_no=<%=drfpo_no%>';
						url = EMPTools.encodeURI(url);
						window.location=url;
						<%}%>
					}else {
			...
			var url = '<emp:url action="addDpoBillDetailRecord.do"/>?drfpo_no=<%=drfpo_no%>';
			url = EMPTools.encodeURI(url);
	...
	function getPorderMsg(){
		...
		var handleSuccess = function(o){
			if(o.responseText !== undefined) {
				...
				}else if(flag == "9999"){//不存在票据信息可以进
					//alert(msg);
					unReadOnly();
				}else if(flag =="0001"){//已存在的票据信息处于其他池中，不能在此票据池中进行新增
					alert(msg);
	      		 	 if(msg=="该笔票据已存在于本票据池中"){
	      		 		if(confirm("是否需要跳转到修改页面？")){
		      		 		var porder_no = IqpBillDetailInfo.porder_no._getValue();
	      		 			var url = '<emp:url action="getDpoBillDetailUpdatePage.do"/>?porder_no='+porder_no+'&drfpo_no=<%=drfpo_no%>';
	      					url = EMPTools.encodeURI(url);
	      					window.location = url;
		      		 	}else{
		...
		var url = '<emp:url action="getPorderMsgByPorderNoDrfpo.do"/>?porder_no='+porderno+'&drfpo_no=<%=drfpo_no%>';
		url = EMPTools.encodeURI(url);
		var obj1 = YAHOO.util.Connect.asyncRequest('POST',url, callback,null) 
	...
	function doReset(){
		IqpBillDetailInfo.bill_type._setValue("");
		IqpBillDetailInfo.porder_no._setValue("");
		IqpBillDetailInfo.porder_curr._setValue("");
		IqpBillDetailInfo.drft_amt._setValue("");
		IqpBillDetailInfo.porder_addr._setValue("");
		IqpBillDetailInfo.is_ebill._setValue("");
		IqpBillDetailInfo.bill_isse_date._setValue("");
		IqpBillDetailInfo.porder_end_date._setValue("");
		IqpBillDetailInfo.utakeover_sign._setValue("");
		IqpBillDetailInfo.tcont_no._setValue("");
		IqpBillDetailInfo.tcont_amt._setValue("");
		IqpBillDetailInfo.tcont_content._setValue("");
		IqpBillDetailInfo.drwr_org_code._setValue("");
		IqpBillDetailInfo.isse_name._setValue("");
		IqpBillDetailInfo.daorg_no._setValue("");
		IqpBillDetailInfo.daorg_name._setValue("");
		IqpBillDetailInfo.daorg_acct._setValue("");
		IqpBillDetailInfo.pyee_name._setValue("");
		IqpBillDetailInfo.paorg_no._setValue("");
		IqpBillDetailInfo.paorg_name._setValue("");
		IqpBillDetailInfo.paorg_acct_no._setValue("");
		...
		IqpBillDetailInfo.status._setValue("");
	}
	function doReturn(){
		<%if("tab".equals(tab)){%>
		window.close();
...
<body class="page_content" onload="doOnLoad();">
		<emp:form id="submitForm" action="addDpoBillDetailRecord.do" method="POST">
			<emp:gridLayout id="DpoBillDetailGroup" title="票据基本信息" maxColumn="2">
				<emp:text id="IqpBillDetailInfo.porder_no" label="汇票号码" hidden="false" onchange="getPorderMsg();" colSpan="2" maxlength="40" readonly="false" required="true" />
				...
				<emp:select id="IqpBillDetailInfo.status" label="票据状态" readonly="true" required="true" dictname="STD_ZB_DRFT_STATUS" defvalue="01"/>
			</emp:gridLayout>
