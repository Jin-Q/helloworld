	...
	function doSave(){ 
		...
		var handleSuccess = function(o){
			if(o.responseText !== undefined) {
				...
				if(flag == "success"){   
					alert("保存成功!");
					var url = '<emp:url action="getIqpCreditChangeAppUpdatePage.do"/>?serno='+serno+'&op=update';  
					url = EMPTools.encodeURI(url);      
					window.location = url; 
				}else {
					alert("异步请求出错！");
	   ...
	   <emp:tab label="信用证修改申请基本信息" id="base_tab" needFlush="true" initial="true" >  
		...
		<emp:gridLayout id="IqpCreditChangeAppGroup" title="信用证修改信息" maxColumn="2">
			...
			<emp:select id="IqpCreditChangeApp.new_security_cur_type" label="修改后保证金币种" defvalue='${context.IqpCreditChangeApp.security_cur_type}' onchange="getHLByCurr4Security()" readonly="true" required="true" dictname="STD_ZX_CUR_TYPE" />
		   	<!-- modified by yangzy 2015/07/15 需求：XD150407026， 存量外币业务取当时时点汇率 start-->
		   	<emp:text id="IqpCreditChangeApp.new_security_exchange_rate" label="保证金汇率" defvalue='${context.IqpCreditChangeApp.new_exchange_rate_security}' maxlength="16" readonly="true" required="true" />
		   	<!-- modified by yangzy 2015/07/15 需求：XD150407026， 存量外币业务取当时时点汇率 end-->
		   	<emp:text id="IqpCreditChangeApp.new_security_rate" label="修改后保证金比例" maxlength="16" readonly="false" defvalue='${context.IqpCreditChangeApp.security_rate}'  cssElementClass="emp_currency_text_readonly"  required="true" dataType="Rate" />
