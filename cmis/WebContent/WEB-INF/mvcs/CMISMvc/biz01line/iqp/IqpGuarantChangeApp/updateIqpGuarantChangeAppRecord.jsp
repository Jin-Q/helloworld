...
<script type="text/javascript">    
	...
	function doSubWfiFlow(){
		    var end_date = IqpGuarantChangeApp.end_date._getValue();
            var openDay = '${context.OPENDAY}';
		    if(end_date<openDay){
               alert("该笔业务已过期,不能提交流程!");
               return false;
    	    }
			var serno = IqpGuarantChangeApp.serno._getValue();    
			var cus_id = IqpGuarantChangeApp.cus_id._getValue(); 
			var cus_id_displayname = IqpGuarantChangeApp.cus_id_displayname._getValue();
			var approve_status = IqpGuarantChangeApp.approve_status._getValue(); 
			...
			WfiJoin.amt._setValue(IqpGuarantChangeApp.new_cont_amt._getValue());
			initWFSubmit(false); 	 
	};
...
<body class="page_content" onload="onload()">
	<emp:form id="submitForm" action="updateIqpGuarantChangeAppRecord.do" method="POST">
	<emp:tabGroup mainTab="base_tab" id="mainTab" >  
	   <emp:tab label="保函修改申请基本信息" id="base_tab" needFlush="true" initial="true" >
		...
		<emp:gridLayout id="IqpGuarantChangeAppGroup" title="保函修改信息" maxColumn="2">
			...
			<emp:select id="IqpGuarantChangeApp.new_guarant_mode" label="保函类型" required="true" dictname="STD_ZB_GUARANT_TYPE" readonly="false"/>  
