...
<script type="text/javascript">
	...
	if (data != null && data.length !=0) {
		...
		var bill_no = data[0].bill_no._getValue(); 
		if(cont_no == "" || cont_no == null || bill_no == "" || bill_no == null){
			alert("检查此条数据合同编号和借据号!");
			return; 
		}
		...
		var url = '<emp:url action="queryIsConfIqpGuarantChange.do"/>?cont_no='+cont_no; 
	...
	function doAddIqpGuarantChangeAppPage(){
		var paramStr = AccLoanList._obj.getParamStr(['cont_no','bill_no']); 
		if (paramStr != null) {
			var url = '<emp:url action="addIqpGuarantChangeAppPage.do"/>?menuId=queryIqpGuarantChangeAppList&'+paramStr;  
			url = EMPTools.encodeURI(url);   
			window.location = url;
		} else {
			alert('请先选择一条记录！'); 
...
<div  class='emp_gridlayout_title'>新增保函修改申请  </div> 
   </emp:form>
	<emp:table icollName="AccLoanList" pageMode="true" url="pageAddIqpGuarantQuery.do">
