...
<script type="text/javascript">
	...
	function doGetUpdateIqpBillDetailPage() {
		...
		if(idx.length == 1){
			...
			var url = '<emp:url action="getIqpBillDetailUpdatePage.do"/>?menuId=queryIqpBillDetail&op=update&batch_no=${context.batch_no}&serno=${context.serno}&porder_no='+paramStr;
	...
	function doViewIqpBillDetail() {
		...
		if(idx.length == 1){
			var paramStr = IqpBillDetailList._obj.data[idx[0]]['porder_no']._getValue();  //得到单个值
			var url = '<emp:url action="getIqpBillDetailViewPage.do"/>?menuId=queryIqpBillDetail&op=view&batch_no=${context.batch_no}&serno=${context.serno}&porder_no='+paramStr;
			url = EMPTools.encodeURI(url);
			window.location = url;
		} else {
			alert('请选择一条记录，再操作！');
	...
	function doGetAddIqpBillDetailPage() {
		var url = '<emp:url action="getIqpBillDetailAddPage.do"/>?batch_no=${context.batch_no}&bill_type=${context.bill_type}';
		url = EMPTools.encodeURI(url);
		window.location = url;
	};
	...
	function doImportIqpBatch(){
		var url = '<emp:url action="importIqpBatchPage.do"/>?serno=${context.serno}';
		url = EMPTools.encodeURI(url);
		var param = 'height=700, width=1200, top=80, left=80, toolbar=no, menubar=no, scrollbars=yes, resizable=yes, location=no, status=no';
		window.open(url,'newWindow',param);
	};	
	//-------修改批次出发事件------
	function doUpdateIqpBatch(){
		var url = '<emp:url action="getIqpBatchMngUpdatePage.do"/>?batch_no=${context.batch_no}';
		url = EMPTools.encodeURI(url);
		var param = 'height=700, width=1200, top=80, left=80, toolbar=no, menubar=no, scrollbars=yes, resizable=yes, location=no, status=no';
		window.open(url,'newWindow',param);
	};	
...
%>
	<emp:table icollName="IqpBillDetailList" pageMode="true" selectType="2" url="pageIqpBillDetail4RpdQuery.do" reqParams="batch_no=${context.batch_no}">
