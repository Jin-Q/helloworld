...
<%
String instanceId1 = request.getParameter("instanceId1");
String instanceId2 = request.getParameter("instanceId2");
...
<fieldset><legend>授信流程意见信息</legend>
...
<iframe id="advice_wf" frameborder="0" width="98%" scrolling="no" src='<%=request.getContextPath()%>/getWFComment.do?instanceId=<%=instanceId1 %>&EMP_SID=<%=request.getParameter("EMP_SID")%>'></iframe>
</div>
</fieldset>


<%if(instanceId2 != null && !"".equals(instanceId2)){ %>
<br/>
<fieldset><legend>第三方授信流程意见信息</legend>
<img alt="收起" src="<%=request.getContextPath()%>/images/workflow/up.png" align="right" onclick="hiddenShow(this,'div_advice_wf_three')" onmouseover="this.style.cursor='hand'">
<div id="div_advice_wf_three">  
<iframe id="advice_wf_three" frameborder="0" width="98%" scrolling="no" src='<%=request.getContextPath()%>/getCreditWFComment.do?instanceId=<%=instanceId2 %>&EMP_SID=<%=request.getParameter("EMP_SID")%>'></iframe>
</div>
</fieldset>
<%} %>
</body>
