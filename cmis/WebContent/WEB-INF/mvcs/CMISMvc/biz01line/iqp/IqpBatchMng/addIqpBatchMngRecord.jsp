...
<script type="text/javascript">
	...
	function doAdd(){
		...
		//--------获取利率和业务类型，传参做判断条件--------
		var bizType = IqpBatchMng.biz_type._getValue();
		var rate = IqpBatchMng.rate._getValue();
		var param = "";
		if(rate != "" && rate != null){
			param = "&bizType="+bizType+"&rate="+rate;
		}
		...
		var handleSuccess = function(o){
			if(o.responseText !== undefined) {
				...
				var batchno = jsonstr.batchno;
				if(flag == "success"){
					alert("保存成功！");
					var url = '<emp:url action="getIqpActTabHelp.do"/>?op=update&batch_no='+batchno+param;
		...
		var url = '<emp:url action="addIqpBatchMngRecord.do"/>';
		url = EMPTools.encodeURI(url);
...
<body class="page_content">
	<emp:form id="submitForm" action="addIqpBatchMngRecord.do" method="POST">
			<emp:gridLayout id="IqpBatchMngGroup" title="票据批次管理新增" maxColumn="2">
				<emp:text id="IqpBatchMng.batch_no" label="批次号"  readonly="true" hidden="true" maxlength="40" colSpan="2" required="false" />
				<emp:select id="IqpBatchMng.bill_type" label="票据种类" required="true" dictname="STD_DRFT_TYPE" />
				<emp:select id="IqpBatchMng.biz_type" label="暂存用途" onblur="chageBizType();"  required="true" dictname="STD_ZB_BUSI_TYPE"/>
				...
				<emp:date id="IqpBatchMng.rebuy_date" label="回购日期" required="false" onblur="rebuyDateCheck()"  hidden="true" />
				<emp:text id="IqpBatchMng.rebuy_rate" label="回购利率" maxlength="10" required="false" dataType="Rate" hidden="true" />
				<emp:text id="IqpBatchMng.rebuy_int" label="回购利息" maxlength="18" hidden="true" defvalue="0" required="false" readonly="true" dataType="Currency" />
				<emp:text id="IqpBatchMng.due_rebuy_rate" label="逾期回购利率" maxlength="10" required="false" dataType="Rate" hidden="true" />
				...
				<emp:text id="IqpBatchMng.bill_qnt" label="票据数量" maxlength="38" required="false" defvalue="0" readonly="true" dataType="Int" />
				<emp:text id="IqpBatchMng.bill_total_amt" label="票据总金额" maxlength="18" defvalue="0" required="false" readonly="true" dataType="Currency" />
				<emp:text id="IqpBatchMng.int_amt" label="利息金额" maxlength="18"  defvalue="0" required="false" readonly="true" dataType="Currency" />
				<emp:text id="IqpBatchMng.rpay_amt" label="实付金额" maxlength="18" defvalue="0" required="false" readonly="true" dataType="Currency" />
				
				<emp:text id="IqpBatchMng.input_id" label="登记人" maxlength="40" hidden="true" defvalue="${context.currentUserId}" required="false" />
				<emp:text id="IqpBatchMng.input_br_id" label="登记机构" maxlength="20" hidden="true" defvalue="${context.organNo}" required="false" />
				<emp:date id="IqpBatchMng.input_date" label="登记日期" hidden="true" defvalue="${context.OPENDAY}" required="false" />
				<emp:select id="IqpBatchMng.status" label="状态" hidden="true" required="false" dictname="STD_BATCH_NUM_STATUS" defvalue="01"/>
