...
<% 
	...
	if(context.containsKey("takeover_date")){
		takeover_date = (String)context.getDataValue("takeover_date");
	}
%>
<emp:page>
<html>
...
<script type="text/javascript">
	...
	function onload(){
        if('<%=takeover_type%>'=="01"||'<%=takeover_type%>'=="02"){
        	...
        	IqpAssetRel.bill_no._obj._renderReadonly(true);
        	readonly();
	...
	function readonly(){
		...
		/**add by lisj 2014年11月20日 修改币种为只读 begin**/
		IqpAssetRel.cur_type._obj._renderReadonly(true);
		/**add by lisj 2014年11月20日 修改币种为只读 end**/
	};
	//返回客户信息
	function returnCus(data){
		...
		IqpAssetRel.prd_id._obj._renderReadonly(false);
			...
			var handleSuccess = function(o){
				if(o.responseText !== undefined) {
					...
					if(flag == "success"){
						...
						url = EMPTools.encodeURI(url);
						window.location = url; 
					}else if(flag =="CurTypeInConf"){
						alert("新增的资产清单与存量清单的币种不一致，请重新输入！");
						}
					else {
						alert("请重新选择客户!"); 
					}
	...
	function getBill(data){
		...
		IqpAssetRel.prd_id._setValue(data.prd_id._getValue());
		IqpAssetRel.prd_name._setValue(data.prd_id_displayname._getValue());
		...
		/**add by lisj 2014年11月20日 币种从借据中获取 begin**/
		IqpAssetRel.cur_type._setValue(data.cur_type._getValue());
		/**add by lisj 2014年11月20日 币种从借据中获取 end**/
		...
		readonly();
		checkBillNo();
		/**add by lisj 2014-12-1 资产包管理改造，校验tableModel begin**/
		checkTableModel(data.table_model._getValue());
		/**add by lisj 2014-12-1 资产包管理改造，校验tableModel end**/
	};
	
	//校验tableModel
	function checkTableModel(tableModel){
		var prdId = IqpAssetRel.prd_id._getValue();
		//垫款业务
		if(tableModel =="AccPad"){
			//到期日期
			IqpAssetRel.loan_end_date._obj._renderHidden(true);
			IqpAssetRel.loan_end_date._obj._renderRequired(false);
			//还款方式
			IqpAssetRel.repay_type_displayname._obj._renderHidden(true);
			//计息周期
			...
			//利率依据方式
			IqpAssetRel.ir_accord_type._obj._renderHidden(true);
			IqpAssetRel.ir_accord_type._obj._renderRequired(false);
			//垫款利率
			IqpAssetRel.pad_rate_y._obj._renderHidden(false);
			}
		//进口开证业务
		if(tableModel =="AccLoan" && prdId =="700021"){
			//计息周期
			IqpAssetRel.ei_type._obj._renderHidden(true);
			IqpAssetRel.ei_type._obj._renderRequired(false);
			//利率依据方式
	...
	function ir_accord_typeChange(data){
		...
		if(llyjfs == "01"){//议价利率
			...
			//IqpAssetRel.takeover_rate._obj._renderHidden(false);//转让利率
			//IqpAssetRel.takeover_int._obj._renderHidden(false);//转让利息
			/** 赋值控制 */
			if(data != "init"){
			...
			if(data != "init"){
				...
				IqpAssetRel.ir_float_rate._setValue("");//利率浮动比
				IqpAssetRel.ir_float_point._setValue("");//贷款利率浮动点数
				IqpAssetRel.overdue_float_type._setValue("0");//逾期利率浮动方式
				IqpAssetRel.overdue_rate._setValue("");//逾期利率浮动比
				IqpAssetRel.overdue_point._setValue("");//逾期利率浮动点数
				IqpAssetRel.default_float_type._setValue("0");//违约利率浮动方式
				IqpAssetRel.default_rate._setValue("");//违约利率浮动比
				IqpAssetRel.default_point._setValue("");//违约利率浮动点数
			}
			/** 必输控制 */
			...
			/** 只读控制 */
			IqpAssetRel.ir_adjust_type._obj._renderReadonly(false);//利率调整方式
			IqpAssetRel.reality_ir_y._obj._renderReadonly(true);//执行利率（年）

			IqpAssetRel.overdue_float_type._obj._renderReadonly(true);//逾期利率浮动方式
			IqpAssetRel.default_float_type._obj._renderReadonly(true);//违约利率浮动方式
...
function reality_ir_yChange(){
	//var reality_ir_y_Value = IqpAssetRel.reality_ir_y._getValue();
...
<body class="page_content" onload="onload()">
	<emp:form id="submitForm" action="addIqpAssetRelRecord.do" method="POST">   
	  <emp:tabGroup mainTab="base_tab" id="mainTab" >
		<emp:gridLayout id="IqpAssetRelGroup" title="资产信息" maxColumn="2">
			...
			<emp:pop id="IqpAssetRel.prd_id" label="产品编号" url="" returnMethod="returnPrdId" required="true" buttonLabel="选择" readonly="true" />
			...
			<emp:text id="IqpAssetRel.ruling_ir" label="基准利率（年）" hidden="true" maxlength="16" readonly="false" required="false" dataType="Rate" onchange="setRulingMounth()"/>
			...
			<emp:text id="IqpAssetRel.ir_next_adjust_term" label="下一次利率调整间隔" hidden="true" required="false" dataType="Int" />
