		...
		if(IqpRateChangeApp._checkAll()){
			...
			var handleSuccess = function(o){
				if(o.responseText !== undefined) {
					...
					if(flag == "success"){
						alert("新增成功!");
						var url = '<emp:url action="getIqpRateChangeAppUpdatePage.do"/>?serno='+serno; 
...
<body class="page_content" onload="onload()">
	...
	<emp:form id="submitForm" action="addIqpRateChangeAppRecord.do" method="POST">
		...
		<emp:gridLayout id="" maxColumn="2" title="原利率信息">
			...
			<emp:text id="IqpRateChangeApp.ir_float_point" label="贷款利率浮动点数" maxlength="10" required="false" defvalue="0" colSpan="2" readonly="true"/>
		 ...
		 <emp:gridLayout id="" maxColumn="2" title="调整后利率信息">
			...
			<emp:text id="IqpRateChangeApp.new_ruling_ir" label="基准利率（年）"  maxlength="16" required="false" dataType="Rate" readonly="true"/>
			<!--XD150520037_信贷系统利率调整修改优化 end-->
			<emp:text id="new_ruling_mounth" label="基准利率(月)"  maxlength="16" required="false" dataType="Rate4Month" readonly="true"  cssElementClass="emp_currency_text_readonly" />	
			<emp:text id="IqpRateChangeApp.new_pad_rate_y" label="垫款利率（年）"  maxlength="16" required="false" dataType="Rate" />
			<emp:select id="IqpRateChangeApp.new_ir_adjust_type" label="利率调整方式" required="false" dictname="STD_IR_ADJUST_TYPE" readonly="true"/>
		 ...
		 <emp:gridLayout id="" maxColumn="3" title="登记信息">		
		    ...
		    <emp:text id="IqpRateChangeApp.input_id_displayname" label="登记人" defvalue="${context.currentUserName}" required="true"  readonly="true"/>
			<emp:text id="IqpRateChangeApp.input_br_id_displayname" label="登记机构" defvalue="${context.organName}" required="true"  readonly="true"/>
			<emp:date id="IqpRateChangeApp.input_date" label="登记日期" required="true" readonly="true" defvalue="${context.OPENDAY}" />
			
			<emp:text id="IqpRateChangeApp.manager_br_id" label="主管机构" maxlength="20" required="false" readonly="true" hidden="true"/> 
			<emp:text id="IqpRateChangeApp.input_id" label="登记人" defvalue="${context.currentUserId}" hidden="true" maxlength="20" required="false"  readonly="true"/>
			<emp:text id="IqpRateChangeApp.input_br_id" label="登记机构" defvalue="${context.organNo}" hidden="true" maxlength="20" required="false"  readonly="true"/>
		    <emp:select id="IqpRateChangeApp.approve_status" label="申请状态" required="false" defvalue="000" hidden="true" dictname="WF_APP_STATUS" />
