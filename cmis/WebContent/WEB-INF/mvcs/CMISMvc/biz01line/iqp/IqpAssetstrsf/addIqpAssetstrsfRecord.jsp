...
<script type="text/javascript">
	...
	/*--user code begin--*/
	function doOnLoad(){
		IqpAssetstrsf.serno._setValue("");
		IqpAssetstrsf.serno._obj._renderHidden(true);
		IqpAssetstrsf.approve_status._setValue("000");//审批状态默认为 待发起
		IqpAssetstrsf.approve_status._obj._renderHidden(true);
    ...
    function getAssetMsg(data){
        ...
        IqpAssetstrsf.takeover_date._setValue(data.takeover_date._getValue());//转让日期
        ...
        IqpAssetstrsf.takeover_date._obj._renderReadonly(true);
	...
	function doSave(){
		...
		var handleSuccess = function(o){
			if(o.responseText !== undefined) {
				...
				if(flag == "success"){
					var url = '<emp:url action="getIqpAssetstrsfUpdatePage.do"/>?op=update&serno='+serno+"&asset_no="+asset_no;
					url = EMPTools.encodeURI(url);
					window.location = url;
				}else {
					alert("保存失败！");
		...
		var url = '<emp:url action="addIqpAssetstrsfRecord.do"/>';
		url = EMPTools.encodeURI(url);
...
<body class="page_content" onload="doOnLoad()">
	...
	<emp:form id="submitForm" action="addIqpAssetstrsfRecord.do" method="POST">
		
		<emp:gridLayout id="IqpAssetstrsfGroup" title="基本信息" maxColumn="2">
			<emp:text id="IqpAssetstrsf.serno" label="业务编号" maxlength="40" required="false" />
			...
			<emp:text id="IqpAssetstrsf.prd_id" label="产品编码" maxlength="6" required="false" defvalue="600020" readonly="true"/>
			...
			<emp:text id="IqpAssetstrsf.this_acct_no" label="本行账户" maxlength="40" dataType="Acct" required="false" cssElementClass="emp_field_input"/> 
			<emp:text id="IqpAssetstrsf.this_acct_name" label="本行账户名" maxlength="100" required="false" colSpan="2" cssElementClass="emp_field_text_long"/>
			<!--modified by wangj 需求编号：ED150612003 ODS系统取数需求  begin-->
			<emp:pop id="IqpAssetstrsf.acctsvcr_no" label="开户行行号" url="getPrdBankInfoPopList.do?status=1" returnMethod="getAcctsvcrNo" required="false" buttonLabel="选择" />
			<!--modified by wangj 需求编号：ED150612003 ODS系统取数需求  end-->
			<emp:text id="IqpAssetstrsf.acctsvcr_name" label="开户行行名" maxlength="100" required="false" readonly="true" colSpan="2" cssElementClass="emp_field_text_long_readonly"/>
			<emp:text id="IqpAssetstrsf.takeover_qnt" label="转让笔数" maxlength="38" required="false" dataType="Int" readonly="true"/>
			<emp:select id="IqpAssetstrsf.acct_curr" label="转让币种" required="false" defvalue="CNY" dictname="STD_ZX_CUR_TYPE"/>
			...
			<emp:date id="IqpAssetstrsf.takeover_date" label="转让日期" required="true" defvalue="${context.OPENDAY}"/>
		...
		<emp:gridLayout id="" maxColumn="3" title="登记信息">
		    		...
		    		<emp:select id="IqpAssetstrsf.flow_type" label="流程类型"  required="false" defvalue="01" dictname="STD_ZB_FLOW_TYPE" readonly="true"/>
		    		<emp:select id="IqpAssetstrsf.approve_status" label="申请状态" dictname="WF_APP_STATUS" readonly="true" required="false" hidden="true" defvalue="000"/>
		   			<emp:text id="IqpAssetstrsf.input_id_displayname" label="登记人" required="false"  readonly="true" defvalue="${context.currentUserName}"/>
					<emp:text id="IqpAssetstrsf.input_br_id_displayname" label="登记机构" required="false"  readonly="true" defvalue="${context.organName}"/>
			
					<emp:date id="IqpAssetstrsf.input_date" label="登记日期" required="false" readonly="true" defvalue="${context.OPENDAY}"/>
					<emp:text id="IqpAssetstrsf.manager_br_id" label="管理机构" hidden="true"/>
					<emp:text id="IqpAssetstrsf.input_id" label="登记人" hidden="true" maxlength="20" required="false"  readonly="true" defvalue="${context.currentUserId}"/>
					<emp:text id="IqpAssetstrsf.input_br_id" label="登记机构" hidden="true" maxlength="20" required="false"  readonly="true" defvalue="${context.organNo}"/>
				</emp:gridLayout>
