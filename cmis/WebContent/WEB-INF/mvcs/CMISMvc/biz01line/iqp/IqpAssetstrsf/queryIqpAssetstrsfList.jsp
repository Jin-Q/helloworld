...
<script type="text/javascript">
	...
	function doGetUpdateIqpAssetstrsfPage() {
		...
		var appStatus = IqpAssetstrsfList._obj.getParamValue(['approve_status']);
		if (paramStr != null) {
			var approve_status = IqpAssetstrsfList._obj.getSelectedData()[0].approve_status._getValue();
			if(approve_status == "000" || approve_status == "992" || approve_status == "993"){
				var url = '<emp:url action="getIqpAssetstrsfUpdatePage.do"/>?op=update&'+paramStr;
	...
	//-----------通过产品编号查询产品配置中使用流程类型----------
    function doSubWF(){
    	var prdId = IqpAssetstrsfList._obj.getSelectedData()[0].prd_id._getValue();
  ...
  //-----------提交流程----------
    function doSubmitWF(apply_type){
    	var serno = IqpAssetstrsfList._obj.getSelectedData()[0].serno._getValue();
    	var cus_id = IqpAssetstrsfList._obj.getSelectedData()[0].toorg_no._getValue();
    	var cus_name = IqpAssetstrsfList._obj.getSelectedData()[0].toorg_name._getValue();
    	var approve_status = IqpAssetstrsfList._obj.getSelectedData()[0].approve_status._getValue();
    	var prd_id = IqpAssetstrsfList._obj.getSelectedData()[0].prd_id._getValue();
    	var prd_name = IqpAssetstrsfList._obj.getSelectedData()[0].prd_id_displayname._getValue();
    	var takeover_total_amt = IqpAssetstrsfList._obj.getSelectedData()[0].takeover_total_amt._getValue();
    	...
    	WfiJoin.amt._setValue(takeover_total_amt);
    ...
    function doCheckAssetInfo(){
    	var paramStr = IqpAssetstrsfList._obj.getParamStr(['serno','asset_no']);
		if (paramStr != null) {
			var asset_no = IqpAssetstrsfList._obj.getParamValue(['asset_no']);
	       	var serno = IqpAssetstrsfList._obj.getParamValue(['serno']);
	       	var handleSuccess = function(o){
	       		if(o.responseText !== undefined) {
	       			try {
	       				var jsonstr = eval("("+o.responseText+")");
...
<body class="page_content">
	...
	<emp:gridLayout id="IqpAssetstrsfGroup" title="输入查询条件" maxColumn="3">
			...
			<emp:pop id="IqpAssetstrsf.toorg_name" label="交易对手行名" url="queryCusSameOrgForPopList.do?restrictUsed=false" returnMethod="returnCusId"/>
			<emp:select id="IqpAssetstrsf.approve_status" label="申请状态" dictname="WF_APP_STATUS"/>
			<emp:text id="IqpAssetstrsf.toorg_no" label="交易对手行号" hidden="true"/>
	</emp:gridLayout>
	
	<jsp:include page="/queryInclude.jsp" flush="true" />
	
	<div align="left">
		<emp:button id="getAddIqpAssetstrsfPage" label="新增" op="add"/>
		<emp:button id="getUpdateIqpAssetstrsfPage" label="修改" op="update"/>
		<emp:button id="deleteIqpAssetstrsf" label="删除" op="remove"/>
