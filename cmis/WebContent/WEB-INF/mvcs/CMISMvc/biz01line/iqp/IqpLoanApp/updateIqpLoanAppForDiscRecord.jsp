...
<script type="text/javascript">
	function doOnLoad(){
		...
		IqpLoanApp.IqpDiscApp.serno._setValue(IqpLoanApp.serno._getValue());
	...
	function doSub(data){
		...
		if(IqpLoanApp._checkAll() && IqpLoanApp.IqpDiscApp._checkAll()){
			...
			var url = '<emp:url action="updateIqpLoanAppForDiscRecord.do"/>?rd='+Math.random();
	...
	function initBillInfo(){
		...
		var is_elec_bill = IqpLoanApp.IqpDiscApp.is_elec_bill._getValue(); 
		if(is_elec_bill == '1'){
	...
	/*--user code begin--*/
	function reLoad(){
		var url = '<emp:url action="getIqpLoanAppForDiscUpdatePage.do"/>?menuId=${context.menuId}&biz_type=${context.biz_type}&serno=${context.IqpLoanApp.serno}&op=update';
		url = EMPTools.encodeURI(url);
		window.location = url;
	...
	function doReturn() {
		var url = '<emp:url action="queryIqpLoanAppList.do"/>?menuId=${context.menuId}&biz_type=${context.biz_type}';   
		url = EMPTools.encodeURI(url);
		//alert(url);
		window.location=url; 
	};
...
<body class="page_content" onload="doOnLoad()">	
  <emp:form id="submitForm" action="updateIqpLoanAppForDiscRecord.do?menuId=${context.menuId}" method="POST">
	 <emp:tabGroup mainTab="base_tab" id="mainTabs">
	  <emp:tab label="申请基本信息" id="base_tab" needFlush="true" >
		<emp:gridLayout id="IqpLoanAppGroup" maxColumn="2" title="基本信息" >
			...
			<emp:select id="IqpLoanApp.IqpDiscApp.is_elec_bill" label="是否电子票据" required="true" readonly ="true" dictname="STD_ZX_YES_NO" />
			...
			<emp:select id="IqpLoanApp.biz_type" label="业务模式" dictname="STD_BIZ_TYPE" required="false" hidden="true" onclick="show_net();controlBizType();"/>
			...
			<emp:select id="IqpLoanApp.IqpDiscApp.is_collect_stamp" label="是否收取印花税" required="false" onchange="changeStampForDis()" dictname="STD_ZX_YES_NO" readonly="true" defvalue="2"/>
			<emp:select id="IqpLoanApp.IqpDiscApp.stamp_collect_mode" label="印花税收取方式" required="false" hidden="true" readonly="false" dictname="STD_ZB_STAMP_MODE" />  
		...
		<emp:gridLayout id="" maxColumn="2" title="金额信息">
		    ...
		    <emp:text id="IqpLoanApp.apply_amount" label="票面总金额" maxlength="18" required="false" dataType="Currency" readonly="true" cssElementClass="emp_currency_text_readonly"/>
		    ...
		    <emp:text id="IqpLoanApp.risk_open_rate" label="敞口比率" maxlength="10" readonly="true" required="false" dataType="Percent" />
		...
		<emp:gridLayout id="" maxColumn="2" title="代理信息">
			...
			<emp:text id="IqpLoanApp.IqpDiscApp.agent_acct_name" label="代理人名称" maxlength="80" required="true" readonly="true"/>
			<!--modified by wangj 需求编号：ED150612003 ODS系统取数需求  begin-->
			<emp:pop id="IqpLoanApp.IqpDiscApp.agent_org_no" url="getPrdBankInfoPopList.do?restrictUsed=false&status=1" returnMethod="getOrgNo" label="代理人开户行行号"  required="true"  readonly="true"/>
			<!--modified by wangj 需求编号：ED150612003 ODS系统取数需求  end-->
			<emp:text id="IqpLoanApp.IqpDiscApp.agent_org_name" label="代理人开户行行名" maxlength="100" required="true" readonly="true"/>
		...
		<emp:gridLayout id="" maxColumn="3" title="登记信息">
		    ...
		    <emp:select id="IqpLoanApp.flow_type" label="流程类型"  required="false" hidden="true" dictname="STD_ZB_FLOW_TYPE" />
...
function doSubmitWF(apply_type){
	...
	if(biz_type == "0" || biz_type == "2" || biz_type == "1"){
