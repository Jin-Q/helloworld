...
function doOnLoad(){
	...
	*/
    ...
    if(prd_id=="100090" ||("${context.flg}" == "csgn" && prd_id =="100063") || prd_id =="400020" || prd_id =="400021"){
    	 ...
    	 IqpLoanApp.IqpLoanAppSub.marketing_br_id._setValue("02");
	 }else{
	     ...
	     IqpLoanApp.IqpLoanAppSub.marketing_br_id._setValue("");
	 }
	...
	if(IqpLoanApp._checkAll() && IqpLoanApp.IqpLoanAppSub._checkAll()){
		...
		var handleSuccess = function(o){
			if(o.responseText !== undefined) {
				...
				if(flag == "success"){
					alert("保存成功!");
					url = '<emp:url action="getIqpLoanAppUpdatePage.do"/>?menuId=${context.menuId}&op=update&serno='+serno+'&biz_type=${context.IqpLoanApp.biz_type}&flg=${context.flg}';
...
function getBill(data){
	...
	var manager_br_id = IqpLoanApp.manager_br_id._getValue();
	var supcatalog = "${context.supcatalog}"; 	
	var cus_total_amt = IqpLoanApp.cus_total_amt._getValue();
	var loan_form = IqpLoanApp.IqpLoanAppSub.loan_form._getValue();
	var is_close_loan = IqpLoanApp.IqpLoanAppSub.is_close_loan._getValue();
	var is_spe_cus = IqpLoanApp.is_spe_cus._getValue();
 	var handleSuccess = function(o){
		if(o.responseText !== undefined) {
			try {
				var jsonstr = eval("("+o.responseText+")");
	...
	var url = '<emp:url action="updateIqpLoanApp4RepayBill.do"/>?is_spe_cus='+is_spe_cus+'&bill_no='+bill_no+'&prd_id='+prd_id+'&cus_id='+cus_id+'&manager_br_id='+manager_br_id+'&supcatalog='+supcatalog+'&cus_total_amt='+cus_total_amt+'&loan_form='+loan_form+'&is_close_loan='+is_close_loan;
	url = EMPTools.encodeURI(url);
	...
	function doClean(){
		page.dataGroups.dataGroup_in_formsubmitForm.reset();
		doOnLoad();
	};
    ...
    /**add by lisj 2014-12-1 需求:XD140925064,生活贷需求开发  end**/ 
	/** add by lisj 2015-1-28 当担保方式为100%保证金时，设保证金比例默认为100%,于2015-1-29上线 begin **/
	function doChangeSerRate(){
		var ass_main = IqpLoanApp.assure_main._getValue();
		//当担保方式为100%保证金时，设保证金比例默认为100%
		if(ass_main =="500"){
			IqpLoanApp.security_rate._setValue("1");
			IqpLoanApp.security_rate._obj._renderReadonly(true);
		}
	};
	/** add by lisj 2015-1-28 当担保方式为100%保证金时，设保证金比例默认为100%,于2015-1-29上线 end **/
</script>
</head>
<body class="page_content" onload="doOnLoad()"> 
  <emp:form id="submitForm" action="addIqpLoanAppRecord.do?menuId=${context.menuId}" method="POST">
	<emp:tabGroup mainTab="base_tab" id="mainTab" >  
	  <emp:tab label="申请基本信息" id="base_tab" needFlush="false" initial="true" >
		<emp:gridLayout id="IqpLoanAppGroup" maxColumn="2" title="基本信息" >
			...
			<emp:select id="IqpLoanApp.assure_main" label="担保方式" required="true" onchange="assure_mainChange();doChangLimitInt();cleanLimitInt();doChangeCollaType();doChangeSerRate();" dictname="STD_ZB_ASSURE_MEANS" />
			...
			<emp:text id="IqpLoanApp.cus_total_amt" label="客户半年日均" maxlength="40" required="false" hidden="true" readonly="true" colSpan="2" dataType="Currency"/>
		...
		<emp:gridLayout id="" maxColumn="2" title="支付信息">
		    <emp:select id="IqpLoanApp.IqpLoanAppSub.conf_pay_type" label="是否确定支付方式" defvalue="1" readonly="true" required="true" dictname="STD_ZX_YES_NO" onclick="isShow()"/>
		...
		<emp:gridLayout id="" maxColumn="2" title="金额信息">
		    ...
		    <emp:text id="IqpLoanApp.apply_amount" label="申请金额" maxlength="18" required="true" onblur="changeRmbAmt();doChangLimitInt();changeRmbAmt4Security()" dataType="Currency" />
		    <emp:text id="IqpLoanApp.apply_rmb_amount" label="折合成人民币金额" maxlength="16" readonly="true" required="true" dataType="Currency" />
		...
		<emp:gridLayout id="" maxColumn="2" title="期限信息">
		   <emp:select id="IqpLoanApp.IqpLoanAppSub.term_type" label="期限类型" defvalue="002" required="true" dictname="STD_ZB_TERM_TYPE" onblur="cleanDate()"/>
		...
		<emp:gridLayout id="" maxColumn="2" title="利率信息">   
			...
			<emp:text id="IqpLoanApp.IqpLoanAppSub.ir_float_rate" label="利率浮动比" hidden="true" colSpan="2" maxlength="16" onchange="getRelYM();" required="false" dataType="Rate" />
			...
			<emp:text id="IqpLoanApp.IqpLoanAppSub.overdue_rate_y" label="逾期利率（年）" hidden="true" colSpan="2" onchange="caculateOverdueRate();" maxlength="16" readonly="true" required="false" dataType="Rate"/>
			
			<emp:select id="IqpLoanApp.IqpLoanAppSub.default_float_type" label="违约利率浮动方式" hidden="true" readonly="true" colSpan="2" onchange="changeDefaultFloatType();" required="false" dictname="STD_RATE_FLOAT_TYPE" />
		...
		<emp:gridLayout id="" maxColumn="3" title="登记信息">
			...
			<emp:text id="IqpLoanApp.serno" label="业务流水" hidden="true"/>
		</emp:gridLayout>
		<div align="center">
			<br>
