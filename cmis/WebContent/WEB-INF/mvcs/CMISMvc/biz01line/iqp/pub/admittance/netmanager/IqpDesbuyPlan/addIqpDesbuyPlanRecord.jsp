	...
	function doSub(){
		if(IqpDesbuyPlan._checkAll()){
			...
			var obj1 = YAHOO.util.Connect.asyncRequest('POST',form.action, callback,postData)
             ...
             //计算货物总价
	function clcTotal(){
	    var desbuy_qnt=IqpDesbuyPlan.desbuy_qnt._getValue();
	    var price=IqpDesbuyPlan.desbuy_unit_price._getValue();
	    if(desbuy_qnt!=null && desbuy_qnt!="" &&price!=null &&price!=""){
		   	 var desbuy_qntp=parseInt(IqpDesbuyPlan.desbuy_qnt._getValue());
		   	 var pricep=parseFloat(IqpDesbuyPlan.desbuy_unit_price._getValue());
		   	 var total=parseFloat(desbuy_qntp*pricep).toFixed(2);
		   	 IqpDesbuyPlan.desbuy_total._setValue(total);
	    }
	}
...
<body class="page_content">	
	<emp:form id="submitForm" action="addIqpDesbuyPlanRecord.do" method="POST">		
		<emp:gridLayout id="IqpDesbuyPlanGroup" title="年度订货计划" maxColumn="2">
			<emp:text id="IqpDesbuyPlan.desgoods_plan_no" label="订货流水号" maxlength="32" hidden="true" colSpan="2"/>
			<emp:text id="IqpDesbuyPlan.cus_id" label="客户码" maxlength="32" required="true" readonly="true"/>
			...
			<emp:text id="IqpDesbuyPlan.desbuy_qnt" label="订购数量" maxlength="10" required="true" onblur="clcTotal()"/>
			<emp:text id="IqpDesbuyPlan.desbuy_unit_price" label="订购单价" maxlength="18" required="true" dataType="Currency" onblur="clcTotal()"/>
