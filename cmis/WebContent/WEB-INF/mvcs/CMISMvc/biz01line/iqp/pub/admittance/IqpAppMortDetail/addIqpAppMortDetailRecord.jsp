...
<script type="text/javascript">
	...
	function doLoad(){
		var serno = '<%=serno%>';
		IqpAppMortDetail.serno._setValue(serno);
	}
	function doReturn(){
		window.close();
	}
...
<body class="page_content" onload="doLoad()">
	...
	<emp:form id="submitForm" action="addIqpAppMortDetailRecord.do" method="POST">
		...
		<emp:gridLayout id="IqpAppMortDetailGroup" title="押品目录准入明细" maxColumn="2">
			...
			<emp:pop id="IqpAppMortDetail.sup_catalog_no_displayname" label="上级目录" url="showCatalogManaTree.do?isMin=N&value=N" returnMethod="setCatalogPath" required="false" /> 
			...
			<emp:text id="IqpAppMortDetail.catalog_path_displayname" label="目录路径"  required="true" colSpan="2" cssElementClass="emp_field_text_readonly"  hidden="false"/>
			...
			<emp:select id="IqpAppMortDetail.status" label="状态" required="true" dictname="STD_ZB_STATUS" defvalue="1" hidden="true"/>
