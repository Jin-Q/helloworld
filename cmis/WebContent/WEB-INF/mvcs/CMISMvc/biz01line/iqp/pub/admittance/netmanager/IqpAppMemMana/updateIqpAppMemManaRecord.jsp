...
<script type="text/javascript">
    ...
    function doBack(){
    	var url = '<emp:url action="queryIqpAppMemManaList.do"/>?serno=${context.serno}'
            +"&mem_cus_id=${context.mem_cus_id}"
            +"&cus_id=${context.cus_id}"
            +"&mem_manuf_type=${context.IqpAppMemMana.mem_manuf_type}";
        url = EMPTools.encodeURI(url);
        window.location=url;
    }
	/*--user code end--*/
   ...
   <emp:tabGroup mainTab="base_tab" id="mainTab" >
     <emp:tab label="基本信息" id="base_tab" needFlush="true" initial="true" >
       <emp:form id="submitForm" action="updateIqpAppMemManaRecord.do" method="POST">
		...
		<emp:pop id="IqpAppMemMana.mem_cus_id" label="成员企业客户码" readonly="true" url="queryAllCusPop.do?cusTypCondition=cus_status='20' and BELG_LINE IN('BL100','BL200')&returnMethod=getCusInfo"  required="true" />
			<emp:text id="IqpAppMemMana.mem_cus_id_displayname" label="成员厂商名称" required="true" readonly="true"/>
			<emp:select id="IqpAppMemMana.mem_manuf_type" readonly="true" label="成员企业类别" required="true" dictname="STD_ZB_MANUF_TYPE" onchange="selectLmtType()"/>
	...
	<emp:table icollName="LmtAgrDetailsList" pageMode="false" url="">
		<emp:text id="limit_code" label="授信额度编码" />
	  ...
	  <emp:tab label="保兑仓协议" id="subTab3" url="queryIqpAppDepotAgrList.do?serno=${context.IqpAppMemMana.serno}&mem_cus_id=${context.IqpAppMemMana.mem_cus_id}&cus_id=${context.cus_id}&mem_manuf_type=${context.IqpAppMemMana.mem_manuf_type}&menuId=${context.menuId}&subMenuId=${context.subMenuId}&op=update" initial="false" needFlush="true"/>
	  <%}%>
   </emp:tabGroup>
</body>   
</html>
