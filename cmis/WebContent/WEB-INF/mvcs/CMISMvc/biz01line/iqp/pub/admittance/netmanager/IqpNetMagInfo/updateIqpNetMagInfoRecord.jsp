...
<script type="text/javascript">
	...
	function CheckDt(){
	 	...
	 	var NetDate = IqpNetMagInfo.net_build_date._getValue();
	 	var flag= CheckDate1BeforeDate2(openDay,NetDate);
	 	if(!flag){
   			alert("不能大于当前时间！");
   	    	IqpNetMagInfo.net_build_date._setValue("");
   	    	return false;
     	}
	}
...
<body class="page_content">			
   <emp:tabGroup mainTab="base_tab" id="mainTab" >
         ...
         <emp:form id="submitForm" action="updateIqpNetMagInfoRecord.do" method="POST">
		<emp:gridLayout id="IqpNetMagInfoGroup" title="客户概况分析" maxColumn="2">	   
			...
			<emp:pop id="IqpNetMagInfo.cus_id" label="核心企业客户码" url="queryAllCusPop.do?cusTypCondition=IqpNet&returnMethod=getCusInfo" required="true" readonly="true"/>
		...
		<emp:gridLayout id="LmtGroup" title="授信业务情况" maxColumn="2">
			...
			<emp:text id="IqpNetMagInfo.buyer_rebuy_amt" label="供应商回购担保额度" maxlength="18" required="true" dataType="Currency" />
		...
		<emp:gridLayout id="CoopGroup" title="业务合作方案" maxColumn="2">
			...
			<emp:date id="IqpNetMagInfo.net_build_date" label="网络建立日期" required="true" onblur="CheckDt()" readonly="true"/>
			...
			<emp:text id="IqpNetMagInfo.sale_reliterm" label="销售调剂期" maxlength="32" required="true" />
			...
			<emp:select id="IqpNetMagInfo.flow_type" label="流程类型" hidden="true" dictname="STD_ZB_FLOW_TYPE" defvalue="01"/>
