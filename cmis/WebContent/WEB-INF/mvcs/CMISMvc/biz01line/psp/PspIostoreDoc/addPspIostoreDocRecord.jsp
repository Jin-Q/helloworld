...
<script type="text/javascript">
	...
	function doload(){
		var task_id = '<%=task_id%>';
		var cus_id = '<%=cus_id%>';
		PspIostoreDoc.task_id._setValue(task_id);
		PspIostoreDoc.cus_id._setValue(cus_id);
	}
...
<body class="page_content" onload="doload()">
	...
	<emp:form id="submitForm" action="addPspIostoreDocRecord.do" method="POST">
		...
		<emp:gridLayout id="PspIostoreDocGroup" title="出入库清单" maxColumn="2">
			...
			<emp:select id="PspIostoreDoc.check_freq" label="检查频率" required="true" dictname="STD_ZB_PSP_CHECK_UNIT" defvalue="${context.check_freq}" readonly="true" colSpan="2"/>
		...
		<emp:gridLayout id="PspIostoreDocGroup" title="登记信息" maxColumn="2">
			<emp:text id="PspIostoreDoc.input_id_displayname" label="登记人" required="false" defvalue="${context.currentUserName}" readonly="true"/>
			<emp:text id="PspIostoreDoc.input_br_id_displayname" label="登记机构" required="false" defvalue="${context.organName}" readonly="true"/>
			<emp:date id="PspIostoreDoc.input_date" label="登记日期" required="false" defvalue="${context.OPENDAY}" readonly="true"/>
			
			<emp:text id="PspIostoreDoc.input_id" label="登记人" maxlength="40" required="false" hidden="true" defvalue="${context.currentUserId}"/>
			<emp:text id="PspIostoreDoc.input_br_id" label="登记机构" maxlength="20" required="false" hidden="true" defvalue="${context.organNo}"/>
