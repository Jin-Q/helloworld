...
<emp:page>
<%
  String task_create_mode = request.getParameter("task_create_mode");
%>
<html>
<head>
<title>新增页面</title>
...
<script type="text/javascript">
	...
	function doLoad(){
		var task_create_mode = '<%=task_create_mode%>';
		PspTaskCheckExpert.task_create_mode._setValue(task_create_mode);
		//doCheck();	
		if(task_create_mode == '04'){
			check_coop();
			var coop_type = PspTaskCheckExpert.coop_type._getValue();
			if(coop_type == '011'){
				var url = "<emp:url action='queryAllCusPop.do'/>?cusTypCondition=cus_type='A2' and cus_status='20'&returnMethod=returnCusCoop";
			}else{
				var url = "<emp:url action='queryLmtAgrJointPop.do'/>?returnMethod=returnCoopAgrNo&flag=3";
			}
			url = EMPTools.encodeURI(url);
			PspTaskCheckExpert.agr_no._obj.config.url = url;
		}
	}
	function check_coop(){
		var options = PspTaskCheckExpert.coop_type._obj.element.options;
	...
	function doNext(){
		...
		var handleSuccess = function(o){
			if(o.responseText !== undefined) {
				...
				if(flag == "success"){
					alert("新增成功!");
					var url = '<emp:url action="queryPspTaskCheckExpertList.do"/>?menuId=expert_task';
					url = EMPTools.encodeURI(url);
					window.location=url;
				}else {
					alert("新增失败!"); 
	...
	function returnArg(data){
		PspTaskCheckExpert.indus_no._setValue(data.agr_no._getValue());
	};
	//单一客户选择pop返回方法
	...
	function onReturnComCllName(date){
		PspTaskCheckExpert.indus_type._obj.element.value=date.id;
	...
	function checkAgrNoUrl(){
		PspTaskCheckExpert.agr_no_displayname._setValue('');
		PspTaskCheckExpert.agr_no._setValue('');
		var coop_type = PspTaskCheckExpert.coop_type._getValue();
		if(coop_type == '011'){
			var url = "<emp:url action='queryAllCusPop.do'/>?cusTypCondition=cus_type='A2' and cus_status='20'&returnMethod=returnCusCoop";
		}else{
	...
	function returnCusCoop(data){
		PspTaskCheckExpert.agr_no._setValue(data.cus_id._getValue());
		PspTaskCheckExpert.agr_no_displayname._setValue(data.cus_name._getValue());
	}
</script>
</head>
<body class="page_content" onload="doLoad()">
	
	<emp:form id="submitForm" action="addPspTaskCheckExpertRecord.do" method="POST">
		...
		<emp:gridLayout id="PspTaskCheckExpertGroup" title="专项检查任务设置" maxColumn="2">
			...
			<emp:text id="PspTaskCheckExpert.task_name" label="任务名称" maxlength="60" required="true" colSpan="2" cssElementClass="emp_field_text_input3"/>
		...
		<%if(task_create_mode.equals("00")){ //公司类客户%>
			...
			<emp:text id="PspTaskCheckExpert.cus_id_displayname" label="客户名称" readonly="true"  required="false" cssElementClass="emp_field_text_readonly"/>
			...
			<emp:checkbox id="PspTaskCheckExpert.assure_main" label="担保方式" required="false" dictname="STD_ZB_ASSURE_MEANS" layout="false" colSpan="2" />
			...
			<emp:text id="PspTaskCheckExpert.biz_type" label="业务品种编号" maxlength="40" required="false" readonly="true" cssElementClass="emp_field_text_readonly"/>
		...
		<%}else if(task_create_mode.equals("01")){ //小微类客户%>
			...
			<emp:select id="PspTaskCheckExpert.is_biz_area" label="是否属联保小组成员" required="true" dictname="STD_ZX_YES_NO" onchange="checkBizArea()"/>
			<emp:pop id="PspTaskCheckExpert.biz_area_no" label="联保协议编号" required="false" url="queryLmtAgrJointPop.do?returnMethod=returnAgrNo&flag=1"/>
			<emp:select id="PspTaskCheckExpert.is_indus" label="是否属行业成员" required="true" dictname="STD_ZX_YES_NO" onchange="checkIsIndus()"/>
			<emp:pop id="PspTaskCheckExpert.indus_no" label="行业协议编号" required="false" url="queryLmtIndusPop.do" returnMethod="returnArg"/>
			<emp:pop id="PspTaskCheckExpert.belg_branch_displayname" label="管理机构" required="false" url="querySOrgPop.do?restrictUsed=false" returnMethod="getOrgId1"/>
			<emp:pop id="PspTaskCheckExpert.belg_manager_id_displayname" label="主管客户经理" required="false" url="getValueQuerySUserPopListOp.do?restrictUsed=false" returnMethod="setconId1"/>
			<emp:pop id="PspTaskCheckExpert.cus_id" label="客户码" required="false" url="queryAllCusPop.do?cusTypCondition=cus_status='20' and cert_code not in (select cert_code from cus_blk_list where status='002' and black_level='3')&returnMethod=returnCus" />
			<emp:text id="PspTaskCheckExpert.cus_id_displayname" label="客户名称" readonly="true"  required="false" cssElementClass="emp_field_text_readonly"/>
			...
			<emp:checkbox id="PspTaskCheckExpert.assure_main" label="担保方式" required="false" dictname="STD_ZB_ASSURE_MEANS" layout="false" colSpan="2"/>
			<emp:pop id="PspTaskCheckExpert.biz_type_displayname" label="业务品种" required="false" url="showPrdTreeDetails.do?bizline=BL200" returnMethod="returnPrdId"/>
			<emp:text id="PspTaskCheckExpert.biz_type" label="业务品种编号" maxlength="40" required="false" readonly="true" cssElementClass="emp_field_text_readonly"/>
			<emp:text id="PspTaskCheckExpert.loan_amt_begin" label="贷款余额区间：从" maxlength="18" required="false" dataType="Currency" onblur="checkBegin()"/>
			<emp:text id="PspTaskCheckExpert.loan_amt_end" label="至： " maxlength="18" required="false" dataType="Currency" onblur="checkEnd()"/>
			<emp:textarea id="PspTaskCheckExpert.memo" label="备注" maxlength="200" required="false" colSpan="2" />
		...
		<%}else if(task_create_mode.equals("02")){ //个人类客户%>
			...
			<emp:select id="PspTaskCheckExpert.is_biz_area" label="是否属联保小组成员" required="true" dictname="STD_ZX_YES_NO" onchange="checkBizArea()"/>
			<emp:pop id="PspTaskCheckExpert.biz_area_no" label="联保协议编号" required="false" url="queryLmtAgrJointPop.do?returnMethod=returnAgrNo&flag=1"/>
			<emp:select id="PspTaskCheckExpert.is_biz_circle" label="是否属圈商成员" required="true" dictname="STD_ZX_YES_NO" onchange="checkBiz()"/>
			...
			<emp:pop id="PspTaskCheckExpert.indus_type_displayname" label="行业投向" required="false" url="showDicTree.do?dicTreeTypeId=STD_GB_4754-2011" returnMethod="onReturnComCllName" cssElementClass="emp_field_text_input3" colSpan="2"/>
			<emp:checkbox id="PspTaskCheckExpert.assure_main" label="担保方式" required="false" dictname="STD_ZB_ASSURE_MEANS" layout="false" colSpan="2"/>
			<emp:pop id="PspTaskCheckExpert.biz_type_displayname" label="业务品种" required="false" url="showPrdTreeDetails.do?bizline=BL300" returnMethod="returnPrdId"/>
			<emp:text id="PspTaskCheckExpert.biz_type" label="业务品种编号" maxlength="40" readonly="true" cssElementClass="emp_field_text_readonly"/>
			<emp:text id="PspTaskCheckExpert.loan_amt_begin" label="贷款余额区间：从" maxlength="18" required="false" dataType="Currency" onblur="checkBegin()"/>
			<emp:text id="PspTaskCheckExpert.loan_amt_end" label="至： " maxlength="18" required="false" dataType="Currency" onblur="checkEnd()"/>
			<emp:textarea id="PspTaskCheckExpert.memo" label="备注" maxlength="200" required="false" colSpan="2" />
		...
		<%}else if(task_create_mode.equals("03")){ //集团（关联）客户%>
			...
			<emp:text id="PspTaskCheckExpert.grp_no_displayname" label="集团名称"   required="false" readonly="true" cssElementClass="emp_field_text_readonly"/>
			<emp:pop id="PspTaskCheckExpert.belg_branch_displayname" label="管理机构" required="false" url="querySOrgPop.do?restrictUsed=false" returnMethod="getOrgId1"/>
			<emp:pop id="PspTaskCheckExpert.belg_manager_id_displayname" label="主管客户经理" required="false" url="getValueQuerySUserPopListOp.do?restrictUsed=false" returnMethod="setconId1"/>
			<emp:textarea id="PspTaskCheckExpert.memo" label="备注" maxlength="200" required="false" colSpan="2" />
		...
		<%}else if(task_create_mode.equals("04")){ //合作方客户%>
			<emp:select id="PspTaskCheckExpert.coop_type" label="合作方类型" required="true" dictname="STD_PSP_COOP_TYPE" colSpan="2" onchange="checkAgrNoUrl()"/>
			<emp:pop id="PspTaskCheckExpert.belg_branch_displayname" label="管理机构" required="false" url="querySOrgPop.do?restrictUsed=false" returnMethod="getOrgId1"/>
			<emp:pop id="PspTaskCheckExpert.belg_manager_id_displayname" label="主管客户经理" required="false" url="getValueQuerySUserPopListOp.do?restrictUsed=false" returnMethod="setconId1"/>
			<emp:pop id="PspTaskCheckExpert.agr_no" label="合作方客户码" required="false" url=""/>
		...
		<%}%>
		    <%-- 
			<emp:pop id="PspTaskCheckExpert.biz_area_no" label="联保协议编号" required="false" url="queryLmtAgrJointPop.do?condition=AND AGR_STATUS='002' AND COOP_TYPE='010' AND ADD_MONTHS(TO_DATE(END_DATE, 'yyyy-mm-dd'),6) >= (SELECT TO_DATE(OPENDAY,'yyyy-mm-dd') FROM PUB_SYS_INFO)&returnMethod=returnAgrNo"/>
			<emp:pop id="PspTaskCheckExpert.grp_no" label="集团编号" required="false" url="queryCusGrpInfoPopList.do?returnMethod=returnCusGrp"/>
			<emp:text id="PspTaskCheckExpert.grp_no_displayname" label="集团名称"   required="false" cssElementClass="emp_field_text_readonly"/>
			<emp:checkbox id="PspTaskCheckExpert.assure_main" label="担保方式" required="false" dictname="STD_ZB_ASSURE_MEANS" layout="false" />
			...
			<emp:pop id="PspTaskCheckExpert.belg_branch_displayname" label="所属支行" required="false" url="querySOrgPop.do?restrictUsed=false" returnMethod="getOrgId1"/>
			<emp:text id="PspTaskCheckExpert.belg_branch" label="所属支行编码" maxlength="60" required="false" readonly="true"/>
			<emp:pop id="PspTaskCheckExpert.belg_manager_id_displayname" label="所属客户经理" required="false" url="getValueQuerySUserPopListOp.do?restrictUsed=false" returnMethod="setconId1"/>
			<emp:text id="PspTaskCheckExpert.belg_manager_id" label="所属客户经理编号" maxlength="60" required="false" readonly="true"/>
			<emp:text id="PspTaskCheckExpert.loan_amt_begin" label="贷款金额   从：" maxlength="18" required="false" dataType="Currency" onblur="checkBegin()"/>
			<emp:text id="PspTaskCheckExpert.loan_amt_end" label="到： " maxlength="18" required="false" dataType="Currency" onblur="checkEnd()"/>
			<emp:text id="PspTaskCheckExpert.loan_amt" label="辅助版面用 " maxlength="18" required="false" dataType="Currency" hidden="true"/>
			<emp:pop id="PspTaskCheckExpert.biz_circle_no" label="圈商编号" required="false" url="queryLmtAgrBizAreaList.do?type=select"  returnMethod="setBizAreaNo"/>
			<emp:text id="PspTaskCheckExpert.biz_area_name" label="圈商名称" maxlength="60" required="false" readonly="true"/>
			<emp:select id="PspTaskCheckExpert.biz_area_type" label="圈商类型" required="false" dictname="STD_LMT_BIZ_AREA_TYPE" readonly="true"/>
			<emp:pop id="PspTaskCheckExpert.agr_no" label="合作方客户码" required="false" url="queryLmtAgrJointPop.do?condition=AND AGR_STATUS='002' AND COOP_TYPE!='010' AND ADD_MONTHS(TO_DATE(END_DATE, 'yyyy-mm-dd'),6) >= (SELECT TO_DATE(OPENDAY,'yyyy-mm-dd') FROM PUB_SYS_INFO)&returnMethod=returnCoopAgrNo&flag=3"/>
			<emp:text id="PspTaskCheckExpert.agr_no_displayname" label="合作方客户名称"   required="false" readonly="true"/>
			<emp:select id="PspTaskCheckExpert.coop_type" label="合作方类型" required="false" readonly="true" dictname="STD_ZB_COOP_TYPE"/>
			<emp:text id="PspTaskCheckExpert.indus_type" label="行业分类" maxlength="5" required="false" hidden="true"/>
			<emp:pop id="PspTaskCheckExpert.indus_type_displayname" label="行业分类" required="false" url="showDicTree.do?dicTreeTypeId=STD_GB_4754-2011" returnMethod="onReturnComCllName" cssElementClass="emp_field_text_input2"/>
			<emp:pop id="PspTaskCheckExpert.gover_fin_ter" label="政府融资平台客户码" required="false" url="queryCusGoverFinTerPop.do?restrictUsed=false" returnMethod="getGoverFinTer"/>
			<emp:text id="PspTaskCheckExpert.gover_fin_ter_displayname" label="政府融资平台客户名称"   required="false" cssElementClass="emp_field_text_readonly"/>
			<emp:pop id="PspTaskCheckExpert.task_exe_id_displayname" label="任务执行人" required="true" url="getValueQuerySUserPopListOp.do?restrictUsed=false" returnMethod="setconId" />
			<emp:pop id="PspTaskCheckExpert.task_exe_br_id_displayname" label="任务执行机构" required="true" url="querySOrgPop.do?restrictUsed=false" returnMethod="getOrgID"/>
            <emp:text id="PspTaskCheckExpert.task_exe_id" label="任务执行人" required="false" hidden="true"/>
			<emp:text id="PspTaskCheckExpert.task_exe_br_id" label="任务执行机构" required="false" hidden="true"/>
			<emp:text id="PspTaskCheckExpert.task_divis_id_displayname" label="任务分配人"   required="false" readonly="true" defvalue="$currentUserName"/>
			<emp:text id="PspTaskCheckExpert.task_divis_br_id_displayname" label="任务分配机构"   required="false" readonly="true" defvalue="$organName"/>
			<emp:text id="PspTaskCheckExpert.task_divis_id" label="任务分配人" maxlength="20" required="false" hidden="true" defvalue="$currentUserId"/>
			<emp:text id="PspTaskCheckExpert.task_divis_br_id" label="任务分配机构" maxlength="20" required="false" hidden="true" defvalue="$organNo"/>
			<emp:textarea id="PspTaskCheckExpert.memo" label="备注" maxlength="200" required="false" colSpan="2" />
			--%>
		</emp:gridLayout>
		<emp:gridLayout id="PspTaskCheckExpertGroup2" title="登记信息" maxColumn="2">	
