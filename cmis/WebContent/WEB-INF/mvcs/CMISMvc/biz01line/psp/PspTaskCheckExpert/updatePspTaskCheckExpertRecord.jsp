...
function checkBiz(){
	...
	if(is_biz_circle!=null&&is_biz_circle=='2'){
		...
		PspTaskCheckExpert.biz_circle_no._setValue('');
	}else {
...
function checkBizArea(){
	...
	if(is_biz_area!=null&&is_biz_area=='2'){
		...
		PspTaskCheckExpert.biz_area_no._setValue('');
	}else {
...
function checkIsIndus(){
	...
	if(is_indus!=null&&is_indus=='2'){
		...
		PspTaskCheckExpert.indus_no._setValue('');
	}else {
...
<body class="page_content" onload="doLoad()">
	...
	<emp:form id="submitForm" action="updatePspTaskCheckExpertRecord.do" method="POST">
		...
		<%if(task_create_mode.equals("00")){ //公司类客户%>
			...
			<emp:text id="PspTaskCheckExpert.biz_type" label="业务品种编号" maxlength="40" readonly="true" required="false" cssElementClass="emp_field_text_readonly"/>
		...
		<%}else if(task_create_mode.equals("03")){ //集团（关联）客户%>
			...
			<emp:text id="PspTaskCheckExpert.grp_no_displayname" label="集团名称" readonly="true"  required="false" cssElementClass="emp_field_text_readonly"/>
			<emp:pop id="PspTaskCheckExpert.belg_branch_displayname" label="管理机构" required="false" url="querySOrgPop.do?restrictUsed=false" returnMethod="getOrgId1"/>
			<emp:pop id="PspTaskCheckExpert.belg_manager_id_displayname" label="主管客户经理" required="false" url="getValueQuerySUserPopListOp.do?restrictUsed=false" returnMethod="setconId1"/>
			<emp:textarea id="PspTaskCheckExpert.memo" label="备注" maxlength="200" required="false" colSpan="2" />
			<emp:text id="PspTaskCheckExpert.belg_branch" label="主办机构" maxlength="60" required="false" readonly="true" hidden="true"/>
			<emp:text id="PspTaskCheckExpert.belg_manager_id" label="主办客户经理" maxlength="60" required="false" readonly="true" hidden="true"/>
		<%}else if(task_create_mode.equals("04")){ //合作方客户%>
			<emp:select id="PspTaskCheckExpert.coop_type" label="合作方类型" required="false" readonly="true" dictname="STD_PSP_COOP_TYPE" colSpan="2" onchange="checkAgrNoUrl()"/>
			<emp:pop id="PspTaskCheckExpert.belg_branch_displayname" label="管理机构" required="false" url="querySOrgPop.do?restrictUsed=false" returnMethod="getOrgId1"/>
			<emp:pop id="PspTaskCheckExpert.belg_manager_id_displayname" label="主管客户经理" required="false" url="getValueQuerySUserPopListOp.do?restrictUsed=false" returnMethod="setconId1"/>
