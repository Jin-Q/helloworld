...
<script type="text/javascript">
	...
	function convert_djjgsfyslcd(){
		if(PspSitecheckComGr.djjgsfyslcd._getValue()=="1" || PspSitecheckComGr.dzysdsfcd._getValue()=="1"){
...
<body class="page_content" onload="doload()">
	...
	<emp:form id="submitForm" action="updatePspSitecheckComGrRecordFortc.do" method="POST">
		...
		<emp:gridLayout id="PspSitecheckComGrGroup" title="抵（质）押物现场检查明细：" maxColumn="2">
			...
			<emp:radio id="PspSitecheckComGr.dzysdsfcd" label="抵（质）押物实地（物）是否被查封、冻结：" required="true" dictname="STD_ZX_YES_NO" colSpan="2" layout="false"  cssLabelClass="emp_field_label1" onclick="convert_djjgsfyslcd()" />
			<emp:radio id="PspSitecheckComGr.djjgsfyslcd" label="登记机关是否已受理查封、冻结：" required="true" dictname="STD_ZX_YES_NO" colSpan="2" layout="false"  cssLabelClass="emp_field_label1" onclick="convert_djjgsfyslcd()" />
			<emp:date id="PspSitecheckComGr.cdsj" label="查封、冻结时间"  required="true" hidden="true"/>
			<emp:textarea id="PspSitecheckComGr.cdyy" label="原因：" required="true" colSpan="2" hidden="true"/>
			<emp:text id="PspSitecheckComGr.cdsqr" label="查封、冻结申请人" maxlength="40" required="true" hidden="true"/>
