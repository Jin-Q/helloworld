...
<body class="page_content" onload="doload()">
	...
	<emp:form id="submitForm" action="updatePspSitecheckComGrRecordFortc.do" method="POST">
		...
		<emp:gridLayout id="PspSitecheckComGrGroup" title="现场检查" maxColumn="2">
			...
			<emp:select id="PspSitecheckComGr.visit_type" label="现场检查类型"  required="true"  dictname="STD_ZB_VISITGR_TYPE" defvalue="03" readonly="true"/>	
			<emp:select id="PspSitecheckComGr.cus_type" label="客户类型"  required="true"  dictname="STD_ZB_GRCUS_TYPE"  onchange="changeCusType()" readonly="true"/>	
		</emp:gridLayout>
		
		<div id="dbrgskh" style="display:none" >
		<emp:gridLayout id="PspSitecheckComGrGroup" title="公司客户" maxColumn="2">
			
			<emp:radio id="PspSitecheckComGr.dbr_jyxc" label="经营现场" required="true" dictname="STD_OFFICE_WORK_STATUS" layout="false" onclick="convert_dbr_jyxc()" disabled="true"/>
			<emp:textarea id="PspSitecheckComGr.dbr_jyxc_yysm" label="异常情况及原因说明：" required="false" hidden="true" colSpan="2" readonly="true"/>
			<emp:radio id="PspSitecheckComGr.dbr_chxc" label="存货现场" required="true" dictname="STD_OFFICE_WORK_STATUS" layout="false" onclick="convert_dbr_chxc()" disabled="true"/>
			<emp:textarea id="PspSitecheckComGr.dbr_chxc_yysm" label="异常情况及原因说明：" required="false" hidden="true" colSpan="2" readonly="true"/>
			<emp:text id="PspSitecheckComGr.dbr_bgry" label="办公场所：办公人员数量" required="true" dataType="Int" readonly="true"/>
			<emp:radio id="PspSitecheckComGr.dbr_bgcs" label="办公场所" required="true" dictname="STD_ZX_FIELD_OWNER" layout="false" disabled="true"/>	
		</emp:gridLayout>
		</div>
		
		<div id="dbrgrkh" style="display:none" >
		<emp:gridLayout id="PspSitecheckComGrGroup" title="个人客户" maxColumn="2">
			<emp:radio id="PspSitecheckComGr.bzrjssfbh" label="保证人居所是否发生变化" required="true" dictname="STD_ZX_YES_NO" layout="false" onclick="convert_bzrjssfbh()" disabled="true"/>
			<emp:textarea id="PspSitecheckComGr.bzrjssfbh_xdz" label="请录入新居所地址" required="false" hidden="true" colSpan="2" readonly="true"/>
			<emp:radio id="PspSitecheckComGr.bzrgzdwbh" label="保证人工作单位是否变化" required="true" dictname="STD_ZX_YES_NO" layout="false" onclick="convert_bzrgzdwbh()" disabled="true"/>
			<emp:textarea id="PspSitecheckComGr.bzrgzdwbh_xdw" label="请录入新的工作单位名称" required="false" hidden="true" colSpan="2" readonly="true"/>
			<emp:radio id="PspSitecheckComGr.bzrjtzcbh" label="保证人家庭资产是否变化" required="true" dictname="STD_ZX_YES_NO" layout="false" onclick="convert_bzrjtzcbh()" disabled="true"/>
			<emp:textarea id="PspSitecheckComGr.bzrjtzcbh_bhqk" label="请录入变化情况" required="false" hidden="true" colSpan="2" readonly="true"/>
			<emp:radio id="PspSitecheckComGr.bzrjtsrbh" label="保证人家庭收入是否变化" required="true" dictname="STD_ZX_YES_NO" layout="false" onclick="convert_bzrjtsrbh()" disabled="true"/>
			<emp:textarea id="PspSitecheckComGr.bzrjtsrbh_bhqk" label="请录入变化情况" required="false" hidden="true" colSpan="2" readonly="true"/>
			<emp:radio id="PspSitecheckComGr.bzrjkzkbh" label="保证人健康状况是否变化" required="true" dictname="STD_ZX_YES_NO" layout="false" onclick="convert_bzrjkzkbh()" disabled="true"/>
			<emp:textarea id="PspSitecheckComGr.bzrjkzkbh_bhqk" label="请录入变化情况" required="false" hidden="true" colSpan="2" readonly="true"/>
			<emp:textarea id="PspSitecheckComGr.dbr_qtqk" label="其他情况：" required="false" hidden="false" colSpan="2" readonly="true"/>
		</emp:gridLayout>
		</div>
