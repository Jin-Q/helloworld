						...
						for(int j=0;j<returnItemIColl.size();j++){
									...
									<% 
										if(dicIColl != null && dicIColl.size() > 0){
											for(int k=0;k<dicIColl.size();k++){
												...
												%>
													<input id='PspCheckItemResult.<%=item_id %>' name='PspCheckItemResult.<%=item_id %>' disabled="disabled" onblur="checkValueMsg(this.id,this.lang);" lang="<%=must %>" checked="checked" type='radio' value='<%=enname %>' class='emp_field_checkbox_input'/><%=cnname %>
												<%
												}else {
												%>
													<input id='PspCheckItemResult.<%=item_id %>' name='PspCheckItemResult.<%=item_id %>' disabled="disabled" onblur="checkValueMsg(this.id,this.lang);" lang="<%=must %>" type='radio' value='<%=enname %>' class='emp_field_checkbox_input'/><%=cnname %>
												<%	
												}
											}
										}
										...
										if(dicIColl != null && dicIColl.size() > 0){
											for(int k=0;k<dicIColl.size();k++){
												...
												if(defValue.trim().length() > 0 ){
													...
													if(flag){
													%>
														<input id='PspCheckItemResult.<%=item_id %>_displayname' name='PspCheckItemResult.<%=item_id %>_displayname' disabled="disabled" checked="checked" onclick="checkValue(this.id);checkValueMsg(this.id,this.lang);" lang="<%=must %>" type='checkbox'  value='<%=enname %>' class='emp_field_checkbox_input'/><%=cnname %>
													<%
													}else {
													%>
														<input id='PspCheckItemResult.<%=item_id %>_displayname' name='PspCheckItemResult.<%=item_id %>_displayname' disabled="disabled" onclick="checkValue(this.id);checkValueMsg(this.id,this.lang);" lang="<%=must %>" type='checkbox' value='<%=enname %>' class='emp_field_checkbox_input'/><%=cnname %>
													<%
													}
												}else {
												%>
							...
							<tr>
								...
								<span id='emp_field_PspCheckItemResult.<%=item_id %>' title='<%=item_name %>' type='TextArea' class='emp_field' cssErrorClass='emp_field_error' cssRequiredClass='emp_field_required' required='false' colSpan='1' readonly='false' onlyControlElement='false' value=''  maxlength='200' rendered='false'>
									<textarea id='PspCheckItemResult.<%=item_id %>' name='PspCheckItemResult.<%=item_id %>' readonly="readonly" onblur="checkValueMsg(this.id,this.lang);" lang="<%=must %>" class='emp_field_textarea_textarea'><%=defValue %></textarea>
								</span>
								<span id='MSG.<%=item_id %>'>
									<% 
									if(must){
