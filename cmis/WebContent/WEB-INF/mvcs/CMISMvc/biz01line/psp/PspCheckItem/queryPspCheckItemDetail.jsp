...
<script type="text/javascript">
	...
	function doReturn() {
		var url = '<emp:url action="queryPspCheckItemList.do"/>';
...
<body class="page_content" onload="onload()">
	...
	<emp:gridLayout id="PspCheckItemGroup" title="检查项目表" maxColumn="2">
			<emp:text id="PspCheckItem.item_id" label="项目编号" maxlength="40" required="true" />
			<emp:text id="PspCheckItem.item_name" label="项目名称" maxlength="200" required="false" />
			<emp:text id="PspCheckItem.default_value" label="缺省值" maxlength="40" required="true" />
			<emp:select id="PspCheckItem.tag_type" label="标签类型" dictname="STD_ZB_TAB_TYPE" required="false" />
			<emp:text id="PspCheckItem.tag_attr" label="标签属性" maxlength="30" required="false" />
			...
			<emp:select id="PspCheckItem.is_hidden" label="是否隐藏" required="true" dictname="STD_ZX_YES_NO" />
			<emp:select id="PspCheckItem.is_null" label="是否不为空" required="false" dictname="STD_ZX_YES_NO" />
			<emp:select id="PspCheckItem.is_judge" label="是否自动判断" required="false" dictname="STD_ZX_YES_NO" />
			<emp:text id="PspCheckItem.rule" label="业务规则" maxlength="40" required="false" />
			...
			<emp:text id="PspCheckItem.input_id" label="登记人" maxlength="20" required="false" hidden="true"/>
			<emp:date id="PspCheckItem.input_date" label="登记日期" required="false" hidden="true"/>
			<emp:text id="PspCheckItem.input_br_id" label="登记机构" maxlength="20" required="false" hidden="true"/>
	</emp:gridLayout> 
