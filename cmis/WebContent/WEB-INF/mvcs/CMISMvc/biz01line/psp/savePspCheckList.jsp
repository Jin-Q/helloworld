...
function doSave(){
	...
	for(var i=0;i<f.length;i++){
		...
		var hidFlg = false;
		if(id!= null && id != '' && id != 'undefined'&& id.indexOf('button')<0){
			var spanId = "PspCheckItemResult."+id+"_valdiv";
		...
		*/
			...
			/********根据类型控制文本域输入内容长度 add by tangzf 2014.04.18 START************/
			if(f[i].type == 'textarea' && f[i].value != null && f[i].value != '' && f[i].value != 'undefined' && f[i].value.length>1000){
				result = false;
				document.getElementById(msg).innerHTML = "<font color='red' >*输入内容超过1000字符</font>";
			}
			/******************END************************/
		}
	}
...
function checkNum(value){
	...
	if(isNaN(cvalue)){ ///如果输入的不是数字，则清空
        alert("请输入数字");
        document.getElementById(value).value = "";////清空实际值
        return;
    }
	document.getElementById(value).value = loadMoney(cvalue);
}
function loadMoney(number) {
	number = number.replace(/,/g,'');///先去所有的,号 这样就可以支持修改，或直接写的就是,号分隔的形式的数字了///
	...
	function doEvent(data,lang,value,cond,item){
		...
		if(item.indexOf('|') > 0){
			...
			for(i=0;i<itemArray.length;i++){
				showModule(flag,itemArray[i]);
			}
		}else{
			showModule(flag,item,lang);
		}		
	};
	
	function showModule(flag,item,lang){
		if(flag){
...
<body class="page_content">
	<form  id='submitForm' method='POST' action='/cmis/savePspCheckResult.do'>
		...
		<!-- modified by yezm 2015-8-14 需求编号：XD150625045 联保授信业务常规检查的担保分析需求 ,担保分析start -->
		<%if(context.containsKey("guar_cus_id")){ %>
    	<input name="guar_cus_id" type="hidden" value="<%=(String)context.getDataValue("guar_cus_id") %>" />  
    	<%}%>
    	<input name="cus_id" type="hidden" value="<%=cus_id%>" />  
		<!-- modified by yezm 2015-8-14 需求编号：XD150625045 联保授信业务常规检查的担保分析需求 ,担保分析end -->
						...
						for(int j=0;j<returnItemIColl.size();j++){
							...
							<tr>
								...
								>
									<span id='emp_field_PspCheckItemResult.<%=item_id %>' title='<%=item_name %>' type='Text' class='emp_field' cssErrorClass='emp_field_error' cssRequiredClass='emp_field_required' required='<%=must %>' hidden='true' colSpan='1' readonly='false' onlyControlElement='false' value=''  maxlength='40' rendered='false'>
										<input id='PspCheckItemResult.<%=item_id %>' name='PspCheckItemResult.<%=item_id %>' onblur="checkValueMsg(this.id,this.lang);" value='<%=defValue %>' lang="<%=must %>" maxlength='200' class='emp_field_text_input'
										<%
										if(event){
									...
									<% 
										if(dicIColl != null && dicIColl.size() > 0){
											for(int k=0;k<dicIColl.size();k++){
												...
												%>
													<input id='PspCheckItemResult.<%=item_id %>' name='PspCheckItemResult.<%=item_id %>' onblur="checkValueMsg(this.id,this.lang);" lang="<%=must %>" checked="true" type='radio' value='<%=enname %>' class='emp_field_checkbox_input'
													<%
														if(event){
															if(event_type.equals("01")){//隐藏事件
													%>
													 onclick="doEvent(this.id,this.lang,'<%=enname %>','<%=hpp_cond %>','<%=imp_item_id %>')" 
													<%
															}
														}
													%>
												...
												%>
													<input id='PspCheckItemResult.<%=item_id %>' name='PspCheckItemResult.<%=item_id %>' onblur="checkValueMsg(this.id,this.lang);" lang="<%=must %>" type='radio' value='<%=enname %>' class='emp_field_checkbox_input'
													<%
														if(event){
															if(event_type.equals("01")){//隐藏事件
													%>
										...
										if(dicIColl != null && dicIColl.size() > 0){
											for(int k=0;k<dicIColl.size();k++){
												...
												if(defValue.trim().length() > 0 ){
													...
													if(flag){
													%>
														<input id='PspCheckItemResult.<%=item_id %>_displayname' name='PspCheckItemResult.<%=item_id %>_displayname' checked="checked" onclick="checkValue(this.id);checkValueMsg(this.id,this.lang);
															<%
															if(event){
																if(event_type.equals("01")){//隐藏事件
															%>
													...
													%>
														<input id='PspCheckItemResult.<%=item_id %>_displayname' name='PspCheckItemResult.<%=item_id %>_displayname' onclick="checkValue(this.id);checkValueMsg(this.id,this.lang);
															<%
															if(event){
																if(event_type.equals("01")){//隐藏事件
															%>
							...
							<tr>
								...
								>
									<span id='emp_field_PspCheckItemResult.<%=item_id %>' title='<%=item_name %>' type='Select' class='emp_field' cssErrorClass='emp_field_error' cssRequiredClass='emp_field_required' required='<%=must %>' colSpan='1' readonly='false' onlyControlElement='false' dictname='STD_ZX_YES_NO' value=''  defMsg='-----请选择-----' rendered='false'>
										<select id='PspCheckItemResult.<%=item_id %>' name='PspCheckItemResult.<%=item_id %>' onblur="checkValueMsg(this.id,this.lang);" lang="<%=must %>" value='<%=defValue %>' class='emp_field_select_select'
											<%
												if(event){
													if(event_type.equals("01")){//隐藏事件
											%>
							...
							<tr>
								...
								<span id='emp_field_PspCheckItemResult.<%=item_id %>' title='<%=item_name %>' type='TextArea' class='emp_field' cssErrorClass='emp_field_error' cssRequiredClass='emp_field_required' required='false' colSpan='1' readonly='false' onlyControlElement='false' value=''  maxlength='200' rendered='false'>
									<textarea id='PspCheckItemResult.<%=item_id %>' name='PspCheckItemResult.<%=item_id %>' onblur="checkValueMsg(this.id,this.lang);" lang="<%=must %>" class='emp_field_textarea_textarea'
											<%
												if(event){
													if(event_type.equals("01")){//隐藏事件
											%>
							...
							//begin********金额类文本框 add by tangzf 2014.04.18*************************************
							else if(tag_type.equals("60")){//金额类文本
								...
								<tr>
									...
									>
										<span id='emp_field_PspCheckItemResult.<%=item_id %>' title='<%=item_name %>' type='Text' class='emp_field' cssErrorClass='emp_field_error' cssRequiredClass='emp_field_required' required='<%=must %>' hidden='true' colSpan='1' readonly='false' onlyControlElement='false' value=''  maxlength='40' rendered='false'>
											<input id='PspCheckItemResult.<%=item_id %>' name='PspCheckItemResult.<%=item_id %>' onblur="checkValueMsg(this.id,this.lang);checkNum(this.id);" value='<%=defValue %>' lang="<%=must %>" maxlength='18' class='emp_field_text_input'
											<%
											if(event){
												if(event_type.equals("01")){//隐藏事件
													%>
		...
		<%if(!op.equals("view")){ %>
			<div align="center">
				...
				<!-- delete by yangzy 2015/01/10 需求:XD141222090,公司小微贷后检查改造 <button id="button_return" class="btn_mouseout" onmouseover="this.className='btn_mouseover'" onmouseout="this.className='btn_mouseout'" onmousedown="this.className='btn_mousedown'" onmouseup="this.className='btn_mouseup'" onclick="doReturn()">返回列表</button>  -->
			</div>
		<%} %>
		</DIV>
