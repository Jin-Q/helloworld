...
<script type="text/javascript">
	...
	function doViewPspSitecheckComGr() {
		var paramStr = PspSitecheckComGrList._obj.getParamStr(['pk_id']);
		if (paramStr != null) {
			var url = '<emp:url action="getPspSitecheckComGrViewPage.do"/>?'+paramStr;
			url = EMPTools.encodeURI(url);
			window.location = url;
		} else {
			alert('请先选择一条记录！');
	...
	function doGetAddPspSitecheckComGrPage() {
		var task_id = '${context.task_id}';
		
		var url = '<emp:url action="getPspSitecheckComGrAddPage.do"/>?task_id='+task_id;
		url = EMPTools.encodeURI(url);
		window.location = url;
	};
	
	function doDeletePspSitecheckComGr() {
		var paramStr = PspSitecheckComGrList._obj.getParamStr(['pk_id']);
		var task_id = PspSitecheckComGrList._obj.getParamValue(['task_id']);
//		var cus_id = PspSitecheckComList._obj.getParamValue(['cus_id']);
		if (paramStr != null) {
			var choic_task_id = PspSitecheckComGrList._obj.getParamValue(['task_id']);
			if(this_task_id != choic_task_id){
				alert("只能操作本笔贷后检查任务下的记录！");
				return false;
			}
			if(confirm("是否确认要删除？")){
				...
				var url = '<emp:url action="deletePspSitecheckComGrRecord.do"/>?'+paramStr;
				url = EMPTools.encodeURI(url);
	...
	function doGetUpdatePspSitecheckComGrPage() {
		var paramStr = PspSitecheckComGrList._obj.getParamStr(['pk_id']);
		if (paramStr != null) {
			var choic_task_id = PspSitecheckComGrList._obj.getParamValue(['task_id']);
			if(this_task_id != choic_task_id){
			...
			var url = '<emp:url action="getPspSitecheckComGrUpdatePage.do"/>?'+paramStr;
			url = EMPTools.encodeURI(url);
			window.location = url;
		} else {
			alert('请先选择一条记录！');
	...
	function doXccheck(){	
		...
		if (data!=null&&data!='') {
				...
				var url = '<emp:url action="getPspSitecheckComGrAddPagedzy.do"/>?task_id='+task_id+'&cus_id='+cus_id;
				url = EMPTools.encodeURI(url);
				var param = 'height=700, width=1200, top=80, left=80, toolbar=no, menubar=no, scrollbars=yes, resizable=no, location=no, status=no';
				window.open(url,'newWindow',param);
			} else {
	...
	function doBzrxccheck(){
		...
		if (data!=null&&data!='') {
				...
				var url = '<emp:url action="getPspSitecheckComGrAddPagedbqy.do"/>?'+param+'&task_id='+task_id+'&cus_id='+cus_id;
				url = EMPTools.encodeURI(url);
				var param = 'height=700, width=1200, top=80, left=80, toolbar=no, menubar=no, scrollbars=yes, resizable=no, location=no, status=no';
				window.open(url,'newWindow',param);
			} else {
	...
	function doXccheckview(){	
		...
		if (data!=null&&data!='') {
				var task_id = '${context.task_id}';
				var cus_id = GrtMortList._obj.getSelectedData()[0].guaranty_no._getValue(); 
				var url = '<emp:url action="getPspSitecheckComGrAddPagedzyView.do"/>?task_id='+task_id+'&cus_id='+cus_id;
				url = EMPTools.encodeURI(url);
				var param = 'height=700, width=1200, top=80, left=80, toolbar=no, menubar=no, scrollbars=yes, resizable=no, location=no, status=no';
				window.open(url,'newWindow',param);
			} else {
	...
	function doBzrxccheckview(){
		...
		if (data!=null&&data!='') {
				var task_id = '${context.task_id}';
				var cus_id = GrtGuaranteeList._obj.getSelectedData()[0].cus_id._getValue();              
				var url = '<emp:url action="getPspSitecheckComGrAddPagedbqyView.do"/>?'+param+'&task_id='+task_id+'&cus_id='+cus_id;
				url = EMPTools.encodeURI(url);
				var param = 'height=700, width=1200, top=80, left=80, toolbar=no, menubar=no, scrollbars=yes, resizable=no, location=no, status=no';
				window.open(url,'newWindow',param);
			} else {
...
<body class="page_content" onload="doload()">
	...
	<div align="left">
		<%if(!op.equals("view")){ %>
		<emp:button id="getAddPspSitecheckComGrPage" label="新增" op="update"/>
		<emp:button id="getUpdatePspSitecheckComGrPage" label="修改" op="update"/>
		<emp:button id="deletePspSitecheckComGr" label="删除" op="update"/>
		<%} %>
		<emp:button id="viewPspSitecheckComGr" label="查看" op="view"/>
	</div>
	<emp:table icollName="PspSitecheckComGrList" pageMode="false" url="pagePspSitecheckComGrQuery.do">
		...
		<emp:text id="visit_type" label="现场检查类型" dictname="STD_ZB_VISITGR_TYPE"/>
	</emp:table>
	...
	<div class='emp_gridlayout_title'>担保人检查</div>
