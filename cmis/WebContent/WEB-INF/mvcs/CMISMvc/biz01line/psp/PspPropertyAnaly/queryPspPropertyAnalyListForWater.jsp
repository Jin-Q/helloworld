...
<script type="text/javascript">
	...
	function doload(){
		...
		/* added by yangzy 2014/11/07 贷后个人经营性业务常规检查参数改造 start */
		this_task_id = '${context.task_id}';
		/* added by yangzy 2014/11/07 贷后个人经营性业务常规检查参数改造 end */
	}
	...
	function doGetAddPspFeeDetailPage() {
		...
		var url = '<emp:url action="getPspFeeDetailAddPage.do"/>?task_id='+task_id+'&cus_id='+cus_id+'&dataFrom=indiv';
	...
	function doGetUpdatePspFeeDetailPage() {
		...
		if (paramStr != null) {
			var url = '<emp:url action="getPspFeeDetailUpdatePage.do"/>?'+paramStr+'&dataFrom=indiv';
			url = EMPTools.encodeURI(url);
			window.location = url;
		} else {
			alert('请先选择一条记录！');
			...
			if(confirm("是否确认要删除？")){
				var handleSuccess = function(o){
					if(o.responseText !== undefined) {
						...
						if(flag == "success"){
							alert("删除成功!");
							var url = '<emp:url action="queryPspPropertyAnalyListForWater.do"/>?task_id='+task_id+'&cus_id='+cus_id; 
							url = EMPTools.encodeURI(url);
							window.location = url; 
						}else {
							alert("删除异常!"); 
