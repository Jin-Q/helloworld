...
<script type="text/javascript">
	...
	function doSub(){
		...
		if(PspPropertyAnaly._checkAll()&PspPropertyStockright._checkAll()){
			PspPropertyAnaly._toForm(form);
			PspPropertyStockright._toForm(form);
...
<body class="page_content" >
	...
	<emp:form id="submitForm" action="updatePspPropertyAnalyRecord.do" method="POST">
		...
		<emp:gridLayout id="PspPropertyStockrightGroup" title="固定资产分析" maxColumn="2">
			...
			<emp:text id="PspPropertyStockright.property_id" label="资产编号" maxlength="32" required="true" hidden="true" />
			...
			<emp:text id="PspPropertyStockright.code_no" label="代码证书编号" maxlength="40" required="false" />
			<emp:text id="PspPropertyStockright.unit_price" label="每股面值" maxlength="16" required="false" dataType="Currency" />
			<emp:text id="PspPropertyStockright.hold_stock" label="持有股份" maxlength="8" required="false" dataType="Int" />
			<emp:date id="PspPropertyStockright.emit_date" label="发行时间" required="false" />
			<emp:text id="PspPropertyStockright.emit_comp" label="发行公司" maxlength="80" required="false" />
			<emp:select id="PspPropertyStockright.is_mrk" label="是否上市" required="false" dictname="STD_ZX_YES_NO" />
			<emp:text id="PspPropertyStockright.curt_price" label="当前股价" maxlength="16" required="false" dataType="Currency" />
		</emp:gridLayout>
