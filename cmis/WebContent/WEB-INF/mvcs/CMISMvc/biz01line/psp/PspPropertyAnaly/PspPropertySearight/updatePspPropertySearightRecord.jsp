...
<script type="text/javascript">
	...
	function doSub(){
		...
		if(PspPropertyAnaly._checkAll()&PspPropertySearight._checkAll()){
			PspPropertyAnaly._toForm(form);
			PspPropertySearight._toForm(form);
...
<body class="page_content" >
	...
	<emp:form id="submitForm" action="updatePspPropertyAnalyRecord.do" method="POST">
		<emp:gridLayout id="PspPropertySearightGroup" title="固定资产分析" maxColumn="2">
			...
			<emp:text id="PspPropertySearight.property_id" label="资产编号" maxlength="32" required="true" hidden="true" />
			...
			<emp:text id="PspPropertySearight.location" label="坐落" maxlength="100" required="false" />
			<emp:text id="PspPropertySearight.land_no" label="宗地号" maxlength="40" required="false" />
			<emp:text id="PspPropertySearight.land_area" label="面积" maxlength="8" required="false" dataType="Int" />
			<emp:text id="PspPropertySearight.use_term" label="使用期限" maxlength="3" required="false" dataType="Int" />
			<emp:date id="PspPropertySearight.end_date" label="终止日期" required="false" />
		</emp:gridLayout>
