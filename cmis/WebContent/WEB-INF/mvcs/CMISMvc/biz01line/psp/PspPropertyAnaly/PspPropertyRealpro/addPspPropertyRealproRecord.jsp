...
<script type="text/javascript">
	...
	function doSub(){
		...
		if(PspPropertyAnaly._checkAll()&PspPropertyRealpro._checkAll()){
			PspPropertyAnaly._toForm(form);
			PspPropertyRealpro._toForm(form);
...
<body class="page_content" >
	...
	<emp:form id="submitForm" action="addPspPropertyAllRecord.do" method="POST">
		...
		<emp:gridLayout id="PspPropertyRealproGroup" title="固定资产分析" maxColumn="2">
			...
			<emp:text id="PspPropertyAnaly.cus_id" label="客户码" maxlength="32" required="true" hidden="true" colSpan="2"/>
			...
			<emp:text id="PspPropertyRealpro.arch_squ" label="建筑面积" maxlength="14" required="false" dataType="Double"/>
			<emp:text id="PspPropertyRealpro.land_squ" label="土地面积" maxlength="14" required="false" dataType="Double"/>
			<emp:select id="PspPropertyRealpro.land_cha" label="土地性质" required="false" dictname="STD_ZB_LAND_CHA"/>
			<emp:text id="PspPropertyRealpro.use_agelimit" label="使用年限" maxlength="8" required="false" dataType="Int" />
			<emp:text id="PspPropertyRealpro.pur_price" label="购置价或造价" maxlength="14" required="false" dataType="Double" />
