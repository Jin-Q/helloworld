...
<script type="text/javascript">
	...
	function doViewPspZxqkLoan() {
		...
		if (paramStr != null) {
			var url = '<emp:url action="getPspZxqkLoanViewPageGr.do"/>?'+paramStr;
	...
	function doGetAddPspZxqkLoanPage() {
		...
		var url = '<emp:url action="getPspZxqkLoanAddPageGr.do"/>?task_id='+task_id+'&cus_id='+cus_id;
		url = EMPTools.encodeURI(url);
		window.location = url;
	};
	...
	function doDeletePspZxqkLoan() {
		...
		if (paramStr != null) {
			...
			if(confirm("是否确认要删除？")){
				...
				var url = '<emp:url action="deletePspZxqkLoanRecordGr.do"/>?'+paramStr;
				url = EMPTools.encodeURI(url);
	...
	function doGetUpdatePspZxqkLoanPage() {
		...
		if (paramStr != null) {
			...
			var url = '<emp:url action="getPspZxqkLoanUpdatePageGr.do"/>?'+paramStr;
			url = EMPTools.encodeURI(url);
			window.location = url;
		} else {
			alert('请先选择一条记录！');
	...
	function doBzrxccheck(){
		...
		if (data!=null&&data!='') {
				...
				var url = '<emp:url action="getPspZxqkLoanAddPagebzrGr.do"/>?task_id='+task_id+'&cus_id_bzr='+cus_id_bzr+'&cus_id='+cus_id;
				url = EMPTools.encodeURI(url);
...
<!--	<div class="emp_gridLayout_title">征信检查 </div>-->
<!--	<div align="left">-->
<!--		<%if(!op.equals("view")){ %>-->
...
<!--	</div>-->
<!--	<emp:table icollName="PspZxqkLoanList" pageMode="false" url="pagePspZxqkLoanQueryGr.do">-->
<!--		<emp:text id="pk_id" label="主键" hidden="true" />-->
<!--		<emp:text id="cus_id" label="客户码"  hidden="true"/>-->
<!--		<emp:text id="task_id" label="任务号" />-->
<!--		<emp:text id="dwrz_amt" label="对外融资总额（万元）" />-->
<!--		<emp:text id="dwdb_amt" label="对外担保总额（万元）" />-->
<!--		<emp:text id="bzr_name" label="保证人姓名" />-->
<!--		-->
<!--	</emp:table>-->
...
<div class='emp_gridlayout_title'>保证人征信检查</div>
	...
	<emp:gridLayout id="PspGroup" title="借款人" maxColumn="2">
		...
		<emp:text id="PspZxqkDetail.dwrz_jsqzje" label="较上期增减额"  required="true" dataType="Currency"/>
		<emp:radio id="PspZxqkDetail.dwrz_sfyqb" label="是否逾期、欠息或不良" required="true" dictname="STD_ZX_YES_NO" colSpan="2" layout="false" />
		<emp:text id="PspZxqkDetail.credit_bank_num" label="授信合作银行数量"  required="true" dataType="Int"  colSpan="2"/>
		<emp:text id="PspZxqkDetail.dwdb_amt" label="对外担保总额（万元）"  required="true" dataType="Currency"/>
		<emp:text id="PspZxqkDetail.dwdb_jsqzje" label="较上期增减额"  required="true" dataType="Currency"/>
		<emp:radio id="PspZxqkDetail.dwdb_sfyqb" label="是否逾期、欠息或不良" required="true" dictname="STD_ZX_YES_NO" colSpan="2" layout="false" />
		...
		<emp:textarea id="PspZxqkDetail.jkrzxbccdqs_sm" label="如有请说明：" maxlength="250" required="true" colSpan="2" hidden="true"/>
		...
		<emp:textarea id="PspZxqkDetail.sfjkryqtzwhdb_sm" label="如有请说明：" maxlength="250" required="true" colSpan="2" hidden="true"/> 
