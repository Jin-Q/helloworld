...
<script type="text/javascript">
	...
	function doSub(){
		...
		if(PspPropertyAnaly._checkAll()&PspPropertyLogo._checkAll()){
			PspPropertyAnaly._toForm(form);
			PspPropertyLogo._toForm(form);
...
<body class="page_content" >
	...
	<emp:form id="submitForm" action="updatePspPropertyAnalyRecord.do" method="POST">
		<emp:gridLayout id="PspPropertyLogoGroup" title="固定资产分析" maxColumn="2">
			...
			<emp:text id="PspPropertyLogo.property_id" label="资产编号" maxlength="32" required="true" hidden="true" />
			...
			<emp:date id="PspPropertyLogo.warrant_date" label="权利凭证期限" required="false" />
			<emp:text id="PspPropertyLogo.warrant_unit" label="权利凭证核发单位" maxlength="50" required="false" />
			<emp:date id="PspPropertyLogo.begin_date" label="权利凭证核发时间" required="false" />
		</emp:gridLayout>
