...
<script type="text/javascript">
	...
	function doload(){
		...
		var task_id = '<%=task_id%>';
		var urls = "&cus_id="+cus_id+"&task_id="+task_id;
		...
		PspCapUseMonitor.cus_id._setValue(cus_id);
		PspCapUseMonitor.task_id._setValue(task_id);
	}
	...
	function checkBillAmt(){
		...
		var url = '<emp:url action="checkAccLoanForPspCap.do"/>?cus_id='+cus_id+'&bill_no='+bill_no+'&disb_amt='+disb_amt;
...
<body class="page_content" onload="doload()">
	...
	<emp:form id="submitForm" action="addPspCapUseMonitorRecord.do?task_id=${context.task_id}" method="POST">
		
		<emp:gridLayout id="PspCapUseMonitorGroup" title="资金用途监控" maxColumn="2">
			<emp:pop id="PspCapUseMonitor.bill_no" label="借据编号" required="true" url="queryPspAccViewPop.do?returnMethod=selAccInfo" colSpan="2" />
			...
			<emp:select id="PspCapUseMonitor.is_exchang_setl" label="是否结汇"  dictname="STD_ZX_YES_NO" defvalue="2" onchange="changeIs_Exchang_Setl()"/>
			<emp:text id="PspCapUseMonitor.exchang_setl_amt" label="结汇金额" maxlength="16" required="true" dataType="Currency" defvalue="0"/>
		...
		<emp:gridLayout id="" title="登记信息" maxColumn="2">
			<emp:text id="PspCapUseMonitor.input_id_displayname" label="登记人" required="false" defvalue="${context.currentUserName}" readonly="true"/>
			<emp:text id="PspCapUseMonitor.input_br_id_displayname" label="登记机构" required="false" defvalue="${context.organName}" readonly="true"/>
			<emp:date id="PspCapUseMonitor.input_date" label="登记日期" required="false" defvalue="${context.OPENDAY}" readonly="true"/>
			
			<emp:text id="PspCapUseMonitor.input_id" label="登记人" maxlength="40" required="false" hidden="true" defvalue="${context.currentUserId}"/>
			<emp:text id="PspCapUseMonitor.input_br_id" label="登记机构" maxlength="20" required="false" hidden="true" defvalue="${context.organNo}"/>
		</emp:gridLayout>
