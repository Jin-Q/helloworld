...
<script type="text/javascript">
	...
	function doReturnMethod(){
		var lmt_type = '${context.lmt_type}';   //授信类型
		...
		if (data != null && data.length !=0) {  //判断如果选中的记录未空提示需选择记录
			...
			//实时查询剩余额度信息 （业务模块提供接口）(2014-06-27 bug3871 去除,getBusBalanceByArgno中判断去掉)	
	...
	function getBusBalanceByArgno(agr_no,lmt_type){
		var handleSuccess = function(o){
			if(o.responseText !== undefined) {
				...
				var msg = jsonstr.msg;
				if(flag == "success"){
					var lmt_amt = jsonstr.lmt_amt;   //台账已占用额度
					var crd_lmt;
					if("01"==lmt_type){
						crd_lmt = LmtAgrDetailsList._obj.getParamValue(['crd_amt']);  //授信额度（减去冻结金额）
					}else{
					...
					//	alert("该授信台账可用额度["+crd_lmt+"]，已用额度["+lmt_amt+"]，本次应占用["+outstnd_amt+"]，不能覆盖业务敞口["+outstnd_amt+"]！");
					//	return false;
					//}
	...
	function doViewAgrDetail(){
		var data = LmtAgrDetailsList._obj.getSelectedData();  //得到选中记录行
		var menuIdTab = "crd_ledger";
		if (data != null && data.length !=0) {
			if("01" == '<%=lmt_type%>'){
				var limit_code = LmtAgrDetailsList._obj.getSelectedData()[0].limit_code._getValue();
			}else if("02"== '<%=lmt_type%>'){
				var limit_code = LmtAgrDetailsList._obj.getSelectedData()[0].agr_no._getValue();
				menuIdTab = "LmtIntbankAcc";
			}
			var url = '<emp:url action="viewLmtAgrInfo.do"/>&agr_no='+limit_code+'&type=surp&menuIdTab='+menuIdTab+'&op=view&type=surp&showButton=N';
			url = EMPTools.encodeURI(url);
...
<body class="page_content">
	...
	<emp:table icollName="LmtAgrDetailsList" pageMode="true" 
		url="pageSelectLmtAgrDetails.do?limit_ind=${context.limit_ind}&lmt_type=${context.lmt_type}&cus_id=${context.cus_id}&outstnd_amt=${context.outstnd_amt}&guar_type=${context.guar_type}&prd_id=${context.prd_id}&selectType=${cotext.selectType}&limit_type=${context.limit_type}">
	...
	<% if("01".equals(lmt_type)){ %>
		...
		<emp:text id="limit_code" label="额度编码" />
		<emp:text id="limit_name" label="额度名称" />
		<emp:text id="limit_type" label="额度类型" dictname="STD_ZB_LIMIT_TYPE"/>
		<emp:text id="cur_type" label="币种" dictname="STD_ZX_CUR_TYPE"/>
		<emp:text id="total_crd_amt" label="授信金额" dataType="Currency"/>
		<emp:text id="crd_amt" label="台账可用金额(授信金额-冻结)" dataType="Currency"/>
