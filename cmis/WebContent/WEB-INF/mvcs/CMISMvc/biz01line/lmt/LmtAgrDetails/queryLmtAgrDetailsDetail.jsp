...
<% 
	...
	// added by yangzy 2014/08/01 授信台账查看，冻结金额根据角色控制 [授信冻结员_领导组] 累计总行风险部冻结的金额，其余岗位只展示支行冻结金额 start
	String roles = "";
	if(context.containsKey("roles")){
		roles = (String)context.getDataValue("roles");
	} 
	String flagHOrg = "";
	if(roles.contains("1045") ){
		flagHOrg = "1";
	}
	// added by yangzy 2014/08/01  授信台账查看，冻结金额根据角色控制 [授信冻结员_领导组] 累计总行风险部冻结的金额，其余岗位只展示支行冻结金额 end
...
<script type="text/javascript">
	...
	function doReturn() {
		...
		if("app"==type){
			...
			url = "<emp:url action='queryLmtAgrIndivDetailsList.do'/>&agr_no=${context.LmtAgrDetails.agr_no}&op=view&menuId=lmtAgrIndiv&subMenuId=crd_ledger";
		}else{
			url = "<emp:url action='queryLmtAgrDetailsList.do'/>&subConndition=${context.subConndition}&restrict_tab=false&op=view&menuId=${context.main_menuId}&subMenuId=crd_ledger";
		}
	...
	function doLoad(){
		...
		var type ="<%=type%>";
		//modified by yangzy 2015/04/13 额度协议查看错误改造 start
		if(menuId == 'indus_crd_change'||menuId == 'indus_crd_query' || menuId == "IqpMemMana" || type == "surp"){
		...
		//modified by yangzy 2015/04/13 额度协议查看错误改造 end
...
<body class="page_content" onload="doLoad()">
	<emp:tabGroup mainTab="main_tabs" id="main_tab">
		<emp:tab label="授信台账信息" id="main_tabs" needFlush="true" initial="true">
		<emp:gridLayout id="LmtAgrDetailsGroup" title="授信额度台账" maxColumn="2">
			...
			<emp:text id="LmtAgrDetails.cus_id_displayname" label="客户名称" cssElementClass="emp_field_text_long_readonly" colSpan="2" readonly="true"/>
			...
			<emp:select id="LmtAgrDetails.lrisk_type" label="低风险业务类型"  required="true" dictname="STD_ZB_LRISK_TYPE"/>
			...
			<emp:text id="LmtAgrDetails.froze_amt" label="冻结金额" maxlength="18" required="true" dataType="Currency" cssElementClass="emp_currency_text_readonly"/>
			<%if(flagHOrg.equals("1")){ //总行冻结金额%>
			<emp:text id="LmtAgrDetails.froze_amt_hq" label="总行冻结金额" maxlength="18" required="true" dataType="Currency" cssElementClass="emp_currency_text_readonly"/>
			<%}%>
			...
			<!-- modified by FCL 2015-6-23 需求编号：XD150715055 新增个人N+1显示  begin -->
			...
			<emp:pop id="LmtAgrDetails.green_indus_displayname" label="绿色产业类型" required="true" url="" returnMethod="greenIndusReturn" cssElementClass="emp_field_text_long_readonly" colSpan="2" />
			...
			<emp:select id="LmtAgrDetails.lmt_type" label="授信类别" required="false" dictname="STD_ZX_LMT_PRD" readonly="true" hidden="true"/>
			<emp:select id="LmtAgrDetails.lmt_status" label="额度状态" dictname="STD_LMT_STATUS" required="true" />
			<emp:text id="LmtAgrDetails.green_indus" label="绿色产业类型" hidden="true"/>
			<emp:select id="LmtAgrDetails.is_adj_term" label="是否调整期限" dictname="STD_ZX_YES_NO" required="true" defvalue="2" hidden="true"/>
	...
	<div align="center">
		<br>
		<input type="button" class="button100" id="btReturn" onclick="doReturn()" value="关闭">
