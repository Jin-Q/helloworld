		...
		var handleSuccess = function(o){
			if(o.responseText !== undefined) {
				...
				if(flag == "success"){
					...
					var crd_amt = LmtAgrDetailsList._obj.getParamValue(['crd_amt']);	//合作方授信总额
					//if(outstnd_amt>(crd_amt-lmt_amt)){  //合作方授信总额-占用金额
	...
	function doViewAgrDetail(){
		...
		if (data != null && data.length !=0) {
			var agr_no = LmtAgrDetailsList._obj.getSelectedData()[0].agr_no._getValue();
				var url = '<emp:url action="viewLmtAgrInfo.do"/>&agr_no='+agr_no+'&type=surp&showButton=N&menuIdTab=lmt_agr_coop';
				url = EMPTools.encodeURI(url);
...
<body class="page_content">
	...
	<emp:gridLayout id="LmtAgrDetailsGroup" title="输入查询条件" maxColumn="3">
		...
		<emp:text id="search_cus_id" label="合作方客户码" />
		<emp:select id="sub_type" label="合作方类型" dictname="STD_ZB_COOP_TYPE"/>
	</emp:gridLayout>
	...
	<emp:table icollName="LmtAgrDetailsList" pageMode="true" url="pageSelectLmtAgrDetails.do?lmt_type=${context.lmt_type}&cus_id=${context.cus_id}&outstnd_amt=${context.outstnd_amt}&guar_type=${context.guar_type}&prd_id=${context.prd_id}&selectType=1">
		<emp:text id="agr_no" label="授信协议编号" />
		<emp:text id="cus_id" label="合作方客户码" />
		<emp:text id="cus_id_displayname" label="合作方客户名称" />
		<emp:text id="sub_type" label="合作方类型" dictname="STD_ZB_COOP_TYPE"/>
