...
<%
	...
	//String lmt_type = (String)context.getDataValue("lmt_type");
...
<script type="text/javascript">
	...
	function doload(){
		...
		//if(lmt_type=="02"){  //公司条线
		//LmtAppDetails.prd_id._obj.config.url='<emp:url action="showPrdCheckTreeDetails.do"/>';
		//}else if(lmt_type=="01"){  //小微条线
			//LmtAppDetails.prd_id._obj.config.url='<emp:url action="showPrdCheckTreeDetails.do"/>&bizline=BL200';
		//}else if(lmt_type=="04"){  //个人条线
			//LmtAppDetails.prd_id._obj.config.url='<emp:url action="showPrdCheckTreeDetails.do"/>&bizline=BL300';
		//}
		...
		/**如果是授信变更则额度类型不允许修改 add by tangzf 2014.04.12**/
		var update_flag = LmtAppDetails.update_flag._getValue();
		if(update_flag=="02"){
		...
		var limit_name = LmtAppDetails.limit_name._getValue();
		if(limit_name=='100076'){//如果是泉水贷。默认授信期限为6个月，增加授信额度测算
		...
		/**added by wangj 2015-05-18 需求编号:XD141222087,法人账户透支需求变更  begin**/
		if(limit_name == '100051' || limit_name == '100088'){//如果是法人账户透支。默认授信类型为循环额度
			LmtAppDetails.limit_type._setValue('01');
			LmtAppDetails.limit_type._obj._renderReadonly(true);
			if(limit_name == '100051'){
				LmtAppDetails.prd_id._setValue(LmtAppDetails.limit_name._getValue());
				LmtAppDetails.prd_id_displayname._setValue(LmtAppDetails.limit_name_displayname._getValue());
				LmtAppDetails.prd_id._obj._renderReadonly(true);
				LmtAppDetails.prd_id_displayname._obj._renderReadonly(true);
			}
		}
		
		/**added by wangj 2015-05-18 需求编号:XD141222087,法人账户透支需求变更  end**/
		if(limit_name == '100088'){
			var options = LmtAppDetails.term_type._obj.element.options;
			//去除申请字典项为"日"
			for(var i=options.length-1;i>=0;i--){
				if(options[i].value == "003"){
				...
				var handleSuccess = function(o){
					if(o.responseText !== undefined) {
						...
						if(flag == "success"){
							...
							if(belgLine!=null&&belgLine=='BL300'){
								//如果条线为个人返回个人详情页面
								url = '<emp:url action="queryLmtAppIndivDetailsList.do"/>&serno=${context.serno}&cus_id=${context.cus_id}&app_type=${context.app_type}&op=${context.op}&subButtonId=${context.subButtonId}&lrisk_type=${context.lrisk_type}&menuId=LmtAppDetails';
								url = EMPTools.encodeURI(url);
								window.location = url;
							}else{
								//返回授信分项列表页面
								url = '<emp:url action="queryLmtAppDetailsList.do"/>&serno=${context.serno}&cus_id=${context.cus_id}&app_type=${context.app_type}&op=${context.op}&subButtonId=${context.subButtonId}&lrisk_type=${context.lrisk_type}&menuId=LmtAppDetails';
								url = EMPTools.encodeURI(url);
								window.location = url;
							}
						}else {
	...
	function setLmtName(data){
		...
		if(data.id=='100076'){//如果是泉水贷。默认授信期限为6个月，增加授信额度测算
			...
			/**add by lisj 2014-12-9  需求:XD140925064,生活贷需求开发  begin**/
			LmtAppDetails.term_type._setValue('${context.LmtAppDetails.term_type}');
			LmtAppDetails.term_type._obj._renderReadonly(false);
			LmtAppDetails.term._setValue('${context.LmtAppDetails.term}');
			LmtAppDetails.term._obj._renderReadonly(false);
			LmtAppDetails.crd_amt._setValue('${context.LmtAppDetails.crd_amt}');
			LmtAppDetails.crd_amt._obj._renderReadonly(false);

			LmtAppDetails.crd_amt._obj.removeOneButton('cal');
			/**add by lisj 2014-12-9  需求:XD140925064,生活贷需求开发  end**/
		}
		//增加泉水贷XD140715023  2014-09-01 zhaozq end
	...
	function returnCoreCus(data){
		...
		LmtAppDetails.core_corp_cus_id_displayname._setValue(data.cus_id_displayname._getValue());
	}
	/**modified by lisj 2015-6-23 需求编号：XD150407025 2015分支机构授权配置 begin**/
	//担保方式修改--不用了这个xjh 190522
...
<emp:tabGroup mainTab="main_tabs" id="main_tab">
	...
	<emp:form id="submitForm" action="updateLmtAppDetailsRecord.do" method="POST">
		...
		<emp:gridLayout id="LmtAppDetailsGroup" title="授信分项基本信息" maxColumn="2">
			...
			<emp:pop id="LmtAppDetails.core_corp_cus_id" label="核心企业客户码 " required="false" url="searchCoreConPop.do" returnMethod="returnCoreCus" hidden="true"/>
			...
			<emp:text id="LmtAppDetails.org_limit_code" label="授信额度编码" readonly="true"/>
			...
			<%} %>
			<!-- modified by lisj 2015-6-23 需求编号：XD150407025 2015分支机构授权配置 end -->
			<emp:select id="LmtAppDetails.is_pre_crd" label="是否预授信" required="true" readonly="true"  dictname="STD_ZX_YES_NO"  />
			...
			<%} %>
			<emp:select id="LmtAppDetails.is_adj_term" label="是否调整期限" required="false" dictname="STD_ZX_YES_NO" colSpan="2" onchange="adjTerm(this.value)" defvalue="2"/>
			...
			<emp:select id="LmtAppDetails.lmt_type" label="授信类别" required="false"  dictname="STD_ZX_LMT_PRD" readonly="true" hidden="true"/>
