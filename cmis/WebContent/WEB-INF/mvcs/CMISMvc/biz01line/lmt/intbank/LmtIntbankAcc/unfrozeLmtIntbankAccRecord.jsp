...
<script type="text/javascript">
	...
	function checkAmt()
	{
		...
		var unfroze_amt = LmtIntbankAcc.unfroze_amt._getValue();
		if(eval(froze_amt)<eval(unfroze_amt))
		{
			alert("解冻金额不能大于冻结额度！");
			LmtIntbankAcc.unfroze_amt._setValue("");
			return;
		}
	}
...
<body class="page_content">
	...
	<emp:form id="submitForm" action="unfrozeLmtIntbankAccRecord.do" method="POST">
		<emp:gridLayout id="LmtIntbankAccGroup" title="同业客户授信台帐" maxColumn="2">
			...
			<emp:text id="LmtIntbankAcc.cus_id_displayname" label="客户名称" required="false" readonly="true"/>
			...
			<emp:text id="LmtIntbankAcc.froze_amt" label="冻结额度(元)" maxlength="18" required="true" dataType="Currency" readonly="true" cssElementClass="emp_currency_text_readonly"/>
			<emp:text id="LmtIntbankAcc.unfroze_amt" label="解冻额度(元)" maxlength="18" required="true" dataType="Currency" onblur="checkAmt()"/>
			...
			<emp:select id="LmtIntbankAcc.lmt_status" label="额度状态" required="false" dictname="STD_LMT_STATUS" readonly="true"/>
			...
			<emp:text id="LmtIntbankAcc.odd_amt" label="可用额度(元)" maxlength="18" hidden="true" dataType="Currency"/>
			<emp:text id="LmtIntbankAcc.cus_id" label="客户码" maxlength="32" required="false" hidden="true"/>
		</emp:gridLayout>
	

		<div align="center">
			<emp:button id="unfroze" label="解冻"/>
