	...
	function doSubmitLmtAppJointCoop(){
		var handleSuccess = function(o) {
			...
			if (o.responseText !== undefined) {
				...
				if("Y" == flag){
					alert("保存成功！");
					var url = '<emp:url action="getLmtAppJointCoopUpdatePage.do"/>?serno='+serno+"&op=update";
					url = EMPTools.encodeURI(url);
					window.location = url;
				}else{
...
<body class="page_content" onload="onLoad()">
		<emp:form id="submitForm" action="addLmtAppJointCoopRecord.do" method="POST">
			<emp:gridLayout id="LmtAppJointCoopGroup" maxColumn="2" title="合作方授信申请">
				<emp:text id="LmtAppJointCoop.serno" label="业务编号" maxlength="40" required="false" readonly="true" cssElementClass="emp_field_text_readonly" hidden="true"/>
				...
				<emp:text id="LmtAppJointCoop.belg_org" label="所属机构码" cssElementClass="emp_field_text_long_readonly" required="false" hidden="true" colSpan="2"/>
				<emp:textarea id="LmtAppJointCoop.belg_org_displayname" label="所属机构名称"  required="false" readonly="true" hidden="true" colSpan="2"/> 
			...
			<emp:gridLayout id="CusBlkCheckinappGroup" title="登记信息" maxColumn="2">
				...
				<emp:text id="LmtAppJointCoop.manager_id" label="责任人" required="false" hidden="true"/>
				<emp:text id="LmtAppJointCoop.manager_br_id" label="责任机构" required="false" hidden="true"/>
				<emp:text id="LmtAppJointCoop.input_id_displayname" label="登记人"  required="true" defvalue="$currentUserName" readonly="true"/>
				<emp:text id="LmtAppJointCoop.input_br_id_displayname" label="登记机构"  required="true" defvalue="$organName" readonly="true" />
				<emp:text id="LmtAppJointCoop.input_id" label="登记人" maxlength="20" required="false" defvalue="$currentUserId" hidden="true"/>
				<emp:text id="LmtAppJointCoop.input_br_id" label="登记机构" maxlength="20" required="false" defvalue="$organNo" hidden="true" />
				<emp:date id="LmtAppJointCoop.input_date" label="登记日期" required="false" defvalue="$OPENDAY" readonly="true"/>
				<emp:select id="LmtAppJointCoop.approve_status" label="申请状态" required="false" dictname="WF_APP_STATUS" readonly="true" defvalue="000"/>
				<emp:date id="LmtAppJointCoop.app_date" label="申请日期" required="false" readonly="true" defvalue="$OPENDAY" hidden="true"/>
