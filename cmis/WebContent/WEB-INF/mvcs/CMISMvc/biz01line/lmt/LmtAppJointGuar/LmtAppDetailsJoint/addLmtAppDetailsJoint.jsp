...
<html>
<%
	String belg_line = request.getParameter("belg_line");
...
<script type="text/javascript">
	...
	function setProds(data){
		...
		LmtAppDetails.belg_line._setValue(data[2]);
		checkPrdConName('1');
	}
...
<body class="page_content" onload="doload()">
	...
	<emp:form id="submitForm" action="addLmtAppDetailsJointRecord.do" method="POST">
		
		<emp:gridLayout id="LmtAppDetailsGroup" title="额度分项信息" maxColumn="2">
			<emp:text id="LmtAppDetails.serno" label="业务编号" maxlength="40" required="false" readonly="true" defvalue="<%=serno%>" hidden="true"/>
			<emp:text id="LmtAppDetails.cus_id" label="客户码 " required="false" defvalue="<%=cus_id%>"  hidden="true"/>
			<emp:select id="LmtAppDetails.sub_type" label="分项类别" required="true" dictname="STD_LMT_PROJ_TYPE" readonly="true" defvalue="<%=type%>"/>
			<emp:select id="LmtAppDetails.limit_type" label="额度类型" required="true" dictname="STD_ZB_LIMIT_TYPE"/>
			<emp:text id="LmtAppDetails.limit_code" label="额度品种编号" maxlength="32" required="false" readonly="true" hidden="true"/>
			...
			<emp:select id="LmtAppDetails.guar_type" label="担保方式" required="true" dictname="STD_ZB_ASSURE_MEANS" defvalue="300" readonly="true"/> 
			<emp:select id="LmtAppDetails.is_pre_crd" label="是否预授信" required="true" dictname="STD_ZX_YES_NO" defvalue="1" readonly="true"/>
			...
			<emp:text id="LmtAppDetails.belg_line" label="所属条线" maxlength="16"  hidden="true"/>
		</emp:gridLayout>
