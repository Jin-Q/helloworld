...
<script type="text/javascript">
	...
	function doGetUpdateLmtAppFrozeUnfrozePage() {
		...
		if (paramStr != null) {
			if(approve_status == '000'){
				var url = '<emp:url action="getLmtAppFrozeUnfrozeUpdatePage.do"/>?'+paramStr+'&limit_code='+limit_code;
	...
	function doViewLmtAppFrozeUnfroze() {
		...
		var limit_code = LmtAppFrozeUnfrozeList._obj.getParamValue(['limit_code']);
		if (paramStr != null) {
			var url = '<emp:url action="getLmtAppFrozeUnfrozeViewPage.do"/>?'+paramStr+'&limit_code='+limit_code;
			url = EMPTools.encodeURI(url);
			window.location = url;
		} else {
			alert('请先选择一条记录！');
	...
	function doGetAddLmtAppFrozeUnfrozePage() {
		var url = '<emp:url action="getNextLmtAppFrozeUnfrozeAddPage.do"/>';
		url = EMPTools.encodeURI(url);
		window.location = url;
	};
	...
	function Change4Status() {
		...
		if (paramStr != null){
			if(confirm("是否确认要生效？")){
				var url = '<emp:url action="updateRecordStatus.do"/>?'+paramStr+'&approve_status='+approve_status+'&limit_code='+limit_code;
				url = EMPTools.encodeURI(url);
				var handleSuccess = function(o){
					if(o.responseText !== undefined) {
						try {
