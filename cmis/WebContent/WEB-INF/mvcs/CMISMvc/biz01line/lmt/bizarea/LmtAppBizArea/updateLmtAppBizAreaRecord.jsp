...
<script type="text/javascript">
	...
	function doOnload(){
		...
		var url_add = '<emp:url action="updateLmtAppBizAreaRecord.do"/>?biz_area_type=' + biz_area_type;
		url_add = EMPTools.encodeURI(url_add);
		document.getElementById("submitForm").action = url_add;

	/*	var options = document.getElementsByName("LmtAppBizArea.guar_type");
	...
	<emp:tabGroup mainTab="main_tabs" id="main_tab">
		...
		<emp:form id="submitForm" action="updateLmtAppBizAreaRecord.do" method="POST">
			<emp:gridLayout id="LmtAppBizAreaGroup" title="圈商准入额度申请" maxColumn="2">
				...
				<emp:date id="LmtAppBizArea.app_date" label="申请日期" required="false" dataType="Date" defvalue="${context.OPENDAY}" readonly="true"/>
				<emp:text id="LmtAppBizArea.over_date" label="办结日期" maxlength="10" required="false" dataType="Date" hidden="true"/>
			</emp:gridLayout>
		<%
		...
		<%}else if("1".equals(biz_area_type)){%>
			<emp:gridLayout id="LmtAppBizAreaCoreGroup" maxColumn="2" title="目标客户群（核心企业）">
				...
				<emp:text id="LmtAppBizAreaCore.coop_year" label="合作年限" maxlength="6" required="true" />
		...
		<%} %>
			<emp:gridLayout id="LmtAppBizAreaGroup" title="机构信息" maxColumn="2">
				<emp:text id="LmtAppBizArea.manager_id" label="责任人" hidden="true" required="true" maxlength="20"/>
				...
				<emp:text id="LmtAppBizArea.input_id_displayname" label="登记人" required="false" readonly="true"/>
				<emp:text id="LmtAppBizArea.input_br_id_displayname" label="登记机构" required="false" readonly="true"/>
				
				<emp:date id="LmtAppBizArea.input_date" label="登记日期" required="false" dataType="Date" readonly="true"/>
				<emp:select id="LmtAppBizArea.approve_status" label="申请状态" required="false" dictname="WF_APP_STATUS" readonly="true"/>
			</emp:gridLayout>
		</emp:form>
	</emp:tab>
