...
<%
	...
	String opType = context.getDataValue("op").toString();
%>
	...
	function doGetUpdateLmtAppDetailsPage() {
		...
		if (paramStr != null) {
			...
			var url = '<emp:url action="getLmtAppDetailsUpdatePage.do"/>?'+paramStr+'&serno='+serno+'&app_type=${context.app_type}&cus_id=${context.cus_id}&op=update&subButtonId=LmtAppDetails&menuIdTab=LmtAppDetails&grtOp=update&lrisk_type=${context.lrisk_type}';
	...
	function doAdjustDistinction(){
		...
		if (paramStr != null){
			...
			var serno =  '${context.serno}';
			if(update_flag == "02"){
				var url = '<emp:url action="getAdjustDistinctionPage.do"/>?'+paramStr+'&serno='+serno+'&app_type=${context.app_type}&cus_id=${context.cus_id}&op=${context.op}&subButtonId=${context.subButtonId}&type=indivApp&lrisk_type=${context.lrisk_type}';
				url = EMPTools.encodeURI(url);
				window.location = url;
			}else{
				alert("请选择申请类型为【变更】的数据！");
			}
		}else{
			alert("请先选择一条记录");
		}
	...
	function doViewLmtAppDetails() {
		...
		var serno =  '${context.serno}';
		if (paramStr != null) {
			var url = '<emp:url action="getLmtAppDetailsViewPage.do"/>?'+paramStr+'&serno='+serno+'&app_type=${context.app_type}&cus_id=${context.cus_id}&op=${context.op}&subButtonId=${context.subButtonId}&menuIdTab=LmtAppDetails&grtOp=view&lrisk_type=${context.lrisk_type}';
			url = EMPTools.encodeURI(url);
			window.location = url; 
		} else {
			alert('请先选择一条记录！');
	...
	function doDeleteLmtAppDetails(){
		...
		var paramStr = LmtAppDetailsList._obj.getParamStr(['limit_code','serno','org_limit_code']);
		if (paramStr != null) {
			...
			if(confirm("是否确认要删除此分项信息？将关联删除项下新建的担保合同。")){  
				var url = '<emp:url action="deleteLmtAppDetailsRecord.do"/>&'+paramStr+"&BelgLine=BL300";
				url = EMPTools.encodeURI(url);
	...
	function doViewLmtAgrDetails() {
		...
		if (paramStr != null) {
			var url = '<emp:url action="getLmtAgrDetailsViewPage.do"/>?'+paramStr+"&type=indivApp&serno=${context.serno}&app_type=${context.app_type}&cus_id=${context.cus_id}&op=${context.op}&subButtonId=${context.subButtonId}&lrisk_type=${context.lrisk_type}";
			url = EMPTools.encodeURI(url);
			window.location = url;
		} else {
			alert('请先选择一条记录！');
	...
	function doGetUpdateLmtAgrDetailsPage() {
		...
		var serno =  '${context.serno}';
		if (paramStr != null) {
			var url = '<emp:url action="getLmtAgrDetailsUpdatePage.do"/>?'+paramStr+'&serno='+serno+'&app_type=${context.app_type}&cus_id=${context.cus_id}&op=${context.op}&subButtonId=${context.subButtonId}&ogr=indiv';
			url = EMPTools.encodeURI(url);
			window.location = url;
		} else {
			alert('请先选择一条记录！');
	...
	//	var url = '<emp:url action="getLmtAppPage.do"/>&serno='+serno+'&cus_id='+cus_id+'&app_type=${context.app_type}&op=${context.op}&subButtonId=${context.subButtonId}';
		var url = '<emp:url action="getLmtAppDetailsAddPage.do"/>&BelgLine=BL300&serno='+ serno+'&cus_id='+cus_id+'&sub_type=01&app_type=${context.app_type}&op=${context.op}&subButtonId=${context.subButtonId}&lrisk_type='+lrisk_type;
		url = EMPTools.encodeURI(url);
		window.location = url;
	}
...
<body class="page_content">
	...
	<%if("update".equals(opType)){ %>
		<!--  <emp:button id="getUpdateLmtAgrDetailsPage" label="修改" op="update"/>  -->
	<%} %>
	...
	<div align="left">
	<%if("update".equals(opType)){ %>
		<emp:button id="getAddLmtAppDetailsPage" label="新增" op="add"/>
		<emp:button id="getUpdateLmtAppDetailsPage" label="修改" op="update"/>
		<emp:button id="deleteLmtAppDetails" label="删除" op="remove"/>
		<emp:button id="viewLmtAppDetails" label="查看" op="view"/>
		<emp:button id="adjustDistinction" label="调整对比" op="update"/>
	<%}else{ %>
