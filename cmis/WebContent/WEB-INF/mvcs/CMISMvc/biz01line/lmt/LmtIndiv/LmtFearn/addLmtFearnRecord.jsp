...
%>
	/*--user code begin--*/
	function doAddLmtFearn(){
		var form = document.getElementById("submitForm");
		LmtFearn._checkAll();
		if(LmtFearn._checkAll()){
	...
	function returnCus(data){
    	...
    	LmtFearn.cus_id_displayname._setValue(data.cus_name._getValue());//客户码
    	LmtFearn.cert_type._setValue(data.cert_type._getValue());//证件类型
    	LmtFearn.cert_code._setValue(data.cert_code._getValue());//证件号码
    	LmtFearn.cus_attr._setValue(data.cus_attr._getValue());//客户属性
    	checkCusId();
	}
		...
		var handleSuccess = function(o){
			if(o.responseText !== undefined) {
				...
				if(flag == "success"){
					...
					alert("此客户存在申请！");
					LmtFearn.cus_id._setValue("");
			    	LmtFearn.cus_id_displayname._setValue("");
			    	LmtFearn.cert_type._setValue("");
			    	LmtFearn.cus_attr._setValue("");
				}
			}
		};
			var handleFailure = function(o){
			...
			var cus_id = LmtFearn.cus_id._getValue();
			var serno = LmtFearn.serno._getValue();
			var url = '<emp:url action="checkCusIdAdd.do"/>&cus_id='+cus_id+"&serno="+serno;
	...
	function changePerc(){
		...
		if(sour == '01' || sour == '03' || sour == '04' || sour == '06' || sour == '07' || sour == '09'){
			...
			LmtFearn.ibank_mearn._setValue(''+Math.round(parseFloat(identy_perc)*10*parseFloat(mearn_score)*100)/1000+'');
			LmtFearn.ibank_yearn._setValue(''+Math.round(parseFloat(identy_perc)*10*parseFloat(yearn_score)*100)/1000+'');
	...
	function doload(){
		...
		LmtFearn.cus_id._obj.config.url=LmtFearn.cus_id._obj.config.url+urls;
 	}
	/*--user code end--*/
</script>
</head>
<body class="page_content" onload="doload()">
	<emp:form id="submitForm" action="addLmtFearnRecord.do" method="POST">
		<emp:gridLayout id="LmtFearnGroup" title="家庭收入" maxColumn="2">
		
			<emp:pop id="LmtFearn.cus_id" label="客户码 " url="queryRelaCusByLmtSernoPop.do?returnMethod=returnCus" 
						colSpan="2" popParam="width=700px,height=650px" required="true"/>
			<emp:text id="LmtFearn.cus_id_displayname" label="客户名称"  required="true" readonly="true" cssElementClass="emp_field_text_readonly" colSpan="2"/>
			...
			<emp:text id="LmtFearn.mearn_score" label="月收入原值" maxlength="18" required="true" dataType="Currency" defvalue="0" onchange="countMonth()" />
			<emp:text id="LmtFearn.ibank_mearn" label="我行认定月收入" maxlength="18" required="false" defvalue="0" readonly="true" dataType="Currency" />  
			<emp:text id="LmtFearn.yearn_score" label="年收入原值" maxlength="18" required="true" defvalue="0" dataType="Currency" onchange="countYear()" />
			<emp:text id="LmtFearn.ibank_yearn" label="我行认定年收入" maxlength="18" required="false" defvalue="0" dataType="Currency"  readonly="true" cssElementClass="emp_currency_text_readonly"/>
			<emp:textarea id="LmtFearn.memo" label="备注" maxlength="60" required="false" colSpan="2" />
			<emp:text id="LmtFearn.serno" label="流水号" maxlength="40" required="false" defvalue="<%=lmt_serno%>" hidden="true"/>
		</emp:gridLayout> 
		<div align="center">
			<br>
			<emp:button id="addLmtFearn" label="确定" />
