...
<script type="text/javascript">
	...
	function doSubmits(){
		url = 'doReturn';
		doPubAdd(url,ArpLawExecuteRecover);
	};

	function doReturn() {
		var url = '<emp:url action="queryArpLawExecuteRecoverList.do"/>?case_no=<%=case_no%>';
...
<body class="page_content">
	...
	<emp:form id="submitForm" action="addArpLawExecuteRecoverRecord.do" method="POST">
		...
		<emp:gridLayout id="ArpLawExecuteRecoverGroup" title="执行回收情况" maxColumn="2">
			...
			<emp:text id="ArpLawExecuteRecover.case_no" label="案件编号" maxlength="40" required="true" defvalue="<%=case_no%>" hidden="true"/>
		...
		<emp:gridLayout id="ArpLawExecuteRecoverGroup" maxColumn="2" title="登记信息">
			<emp:text id="ArpLawExecuteRecover.input_id_displayname" label="登记人" readonly="true" required="true" defvalue="$currentUserName" />
			<emp:text id="ArpLawExecuteRecover.input_br_id_displayname" label="登记机构" readonly="true" required="true" defvalue="$organName" />
			<emp:text id="ArpLawExecuteRecover.input_id" label="登记人" required="true"  defvalue="$currentUserId" hidden="true"/>
			<emp:text id="ArpLawExecuteRecover.input_br_id" label="登记机构" required="true" defvalue="$organNo" hidden="true" />
			<emp:date id="ArpLawExecuteRecover.input_date" label="登记日期" required="true"  defvalue="$OPENDAY" readonly="true" />
		</emp:gridLayout>
