...
<script type="text/javascript">
	...
	/*--user code begin--*/
	function doSubmits(){
		doPubUpdate(ArpLawExecuteInfo);
	};
	function doReturn() {
		var url = '<emp:url action="queryArpLawExecuteInfoList.do"/>?case_no='+ArpLawExecuteInfo.case_no._getValue();
...
<body class="page_content" onload="doLoad()">
	...
	<emp:form id="submitForm" action="updateArpLawExecuteInfoRecord.do" method="POST">
		<emp:gridLayout id="ArpLawExecuteInfoGroup" title="执行情况" maxColumn="2">
			...
			<emp:text id="ArpLawExecuteInfo.case_no" label="案件编号" maxlength="40" required="true" hidden="true"/>
		...
		<emp:gridLayout id="ArpLawExecuteInfoGroup" maxColumn="2" title="登记信息">
			<emp:text id="ArpLawExecuteInfo.input_id_displayname" label="登记人" readonly="true" required="true" />
			<emp:text id="ArpLawExecuteInfo.input_br_id_displayname" label="登记机构" readonly="true" required="true"  />
			<emp:text id="ArpLawExecuteInfo.input_id" label="登记人" required="true" hidden="true"/>
			<emp:text id="ArpLawExecuteInfo.input_br_id" label="登记机构" required="true" hidden="true" />
			<emp:date id="ArpLawExecuteInfo.input_date" label="登记日期" required="true"  readonly="true" />
		</emp:gridLayout>
