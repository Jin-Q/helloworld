...
<script type="text/javascript">
	...
	function reLoad(){
		var url = '<emp:url action="getArpLawLawsuitInfoUpdatePage.do"/>?case_no=${context.ArpLawLawsuitInfo.case_no}&op=update';
		url = EMPTools.encodeURI(url);
		window.location = url;
	};
	...
	function checkDate(obj){
		if(obj.value){
			var lawsuit_start_date = ArpLawLawsuitInfo.lawsuit_start_date._getValue();	//诉讼开始日期
			var lawsuit_end_date = ArpLawLawsuitInfo.lawsuit_end_date._getValue();	//诉讼结束日期

			if(obj.name == 'ArpLawLawsuitInfo.lawsuit_start_date'){
				if(lawsuit_end_date < lawsuit_start_date && lawsuit_end_date != ''){
					obj.value = '';
					alert("[诉讼开始日期]不能大于[诉讼结束日期]");
		   		}
			}
			if(obj.name == 'ArpLawLawsuitInfo.lawsuit_end_date'){
				if(lawsuit_start_date != '' && lawsuit_end_date < lawsuit_start_date ){
					obj.value = '';
					alert("[诉讼结束日期]不能小于[诉讼开始日期]");
		   		}
			}
		}		
	};
...
<body class="page_content" onload="doload()">
	...
	<emp:tab label="诉讼申请信息" id="main_tabs">
		...
		<emp:gridLayout id="ArpLawLawsuitInfoGroup" title="诉讼信息" maxColumn="2">
			...
			<emp:text id="ArpLawLawsuitInfo.other_fee" label="其他费用" maxlength="16" required="false" dataType="Currency" readonly="true"/>
			<emp:date id="ArpLawLawsuitInfo.lawsuit_start_date" label="诉讼开始日期" required="false"  onblur="checkDate(this)"/>
			<emp:date id="ArpLawLawsuitInfo.lawsuit_end_date" label="诉讼结束日期" required="false" onblur="checkDate(this)" />
			<emp:textarea id="ArpLawLawsuitInfo.executable_property_status" label="可执行财产状况" maxlength="250" required="false" colSpan="2" readonly="true"/>
			<emp:select id="ArpLawLawsuitInfo.law_disp_mode" label="法律处置方式" required="false" dictname="STD_ZB_LAW_DISP" readonly="true"/>
			<emp:textarea id="ArpLawLawsuitInfo.lawsuit_reason" label="起诉原因" maxlength="250" required="false" colSpan="2" readonly="true"/>
			<emp:select id="ArpLawLawsuitInfo.whether_invite_agt_lawyer" label="是否聘请代理律师" required="false" dictname="STD_ZX_YES_NO"  readonly="true"/>
			<emp:select id="ArpLawLawsuitInfo.agt_type" label="代理方式" required="false" dictname="STD_ZB_AGT_TYPE" readonly="true"/>
			<emp:textarea id="ArpLawLawsuitInfo.agt_agreed_indgt" label="代理约定事项" maxlength="250" required="false" colSpan="2" readonly="true" />
			<emp:text id="ArpLawLawsuitInfo.agcy" label="代理人" maxlength="100" required="false" readonly="true"/>
			<emp:text id="ArpLawLawsuitInfo.agcy_link_mode" label="代理人联系方式" maxlength="50" required="false" dataType="Phone" readonly="true"/>
			<emp:text id="ArpLawLawsuitInfo.belg_lawyer_office" label="所属律师事务所" maxlength="100" required="false" readonly="true"/>
			<emp:text id="ArpLawLawsuitInfo.agt_fee_pay_rate" label="代理费支付比例" maxlength="10" required="false" dataType="Percent" readonly="true" />
			<emp:text id="ArpLawLawsuitInfo.agt_fee_pay_amt" label="代理费支付金额" maxlength="16" required="false" dataType="Currency" readonly="true"/>
			<emp:textarea id="ArpLawLawsuitInfo.memo" label="备注" maxlength="250" required="false" colSpan="2" readonly="true"/>
			<emp:date id="ArpLawLawsuitInfo.app_date" label="申请日期" required="false" hidden="true" readonly="true" />
			<emp:date id="ArpLawLawsuitInfo.over_date" label="办结日期" required="false"  hidden="true" readonly="true"/>
		</emp:gridLayout>
		
		<emp:gridLayout id="ArpLawLawsuitInfoGroup" maxColumn="2" title="登记信息">
			<emp:text id="ArpLawLawsuitInfo.manager_id_displayname" label="管理人员" required="true"  readonly="true"/>
			<emp:text id="ArpLawLawsuitInfo.manager_br_id_displayname" label="管理机构"  required="true" cssElementClass="emp_pop_common_org" readonly="true"/>
