...
<script type="text/javascript">
	...
	function orgReceiver(data){
		...
		ArpBadassetHandoverApp.rcv_person_displayname._obj.config.url = EMPTools.encodeURI(url);
	};
	function doSubmits(){
		var url = '<emp:url action="getArpBadassetHandoverAppUpdatePage.do"/>?restrictUsed=false&action=update&serno=';
		doPubAdd(url,ArpBadassetHandoverApp);
	};
	function doReturn() {
...
<body class="page_content" onload="doLoad()">
	<emp:form id="submitForm" action="addArpBadassetHandoverAppRecord.do" method="POST">
		...
		<emp:gridLayout id="ArpBadassetHandoverAppGroup" maxColumn="2" title="移交信息">	
			<emp:text id="ArpBadassetHandoverApp.serno" label="业务编号" maxlength="40" hidden="true"  required="false"/>
			...
			url="querySOrgPop.do?restrictUsed=false" returnMethod="orgReceiver" required="true"/>
			...
			<emp:date id="ArpBadassetHandoverApp.app_date" label="申请日期" required="true" defvalue="$OPENDAY" hidden="true"/>
		...
		<emp:gridLayout id="ArpBadassetHandoverAppGroup" maxColumn="2" title="登记信息">			
			<emp:text id="ArpBadassetHandoverApp.input_id_displayname" label="登记人" readonly="true" required="true" defvalue="$currentUserName" />
			<emp:text id="ArpBadassetHandoverApp.input_br_id_displayname" label="登记机构" readonly="true" required="true" defvalue="$organName" />
			<emp:text id="ArpBadassetHandoverApp.input_id" label="登记人" required="true"  defvalue="$currentUserId" hidden="true"/>
			<emp:text id="ArpBadassetHandoverApp.input_br_id" label="登记机构" required="true" defvalue="$organNo" hidden="true" />
			<emp:date id="ArpBadassetHandoverApp.input_date" label="登记日期" required="true"  defvalue="$OPENDAY" readonly="true" />
			<emp:select id="ArpBadassetHandoverApp.approve_status" label="审批状态" required="false" dictname="WF_APP_STATUS"  defvalue="000" readonly="true" />
		</emp:gridLayout>
