...
//返回抵债资产编号及其名称
function setGuaranty(data){
	ArpCollDispApp.guaranty_no._setValue(data.guaranty_no._getValue());
	ArpCollDispApp.guaranty_no_displayname._setValue(data.guaranty_name._getValue());
	ArpCollDispApp.debt_in_amt._setValue(data.debt_in_amt._getValue());
}
function doNext(){
	...
	var handleSuccess = function(o){
		if(o.responseText !== undefined) {
			...
			if(flag == "success"){
				var url = '<emp:url action="getArpCollDispAppUpdatePage.do"/>?op=update&serno='+serno;
				url = EMPTools.encodeURI(url);
				window.location=url;
			}else {
				alert("新增失败!"); 
...
<body class="page_content">
	...
	<emp:form id="submitForm" action="addArpCollDispAppRecord.do" method="POST">
		
		<emp:gridLayout id="ArpCollDispAppGroup" title="抵债物处置申请" maxColumn="2">
			<emp:text id="ArpCollDispApp.serno" label="业务编号" maxlength="40" required="false" hidden="true" />
			<emp:date id="ArpCollDispApp.app_date" label="申请日期" required="false" hidden="true" defvalue="$OPENDAY"/>
			<emp:date id="ArpCollDispApp.end_date" label="办结日期" required="false" hidden="true"/>
			<emp:pop id="ArpCollDispApp.guaranty_no" label="抵债资产编号" required="true" url="queryArpCollDebtAccRePopList.do" returnMethod="setGuaranty"/>
			<emp:text id="ArpCollDispApp.guaranty_no_displayname" label="抵债资产名称"  required="false" readonly="true" colSpan="2"/>
			<emp:text id="ArpCollDispApp.debt_in_amt" label="抵入金额" maxlength="16" required="true" dataType="Currency" readonly="true"/>
			<emp:text id="ArpCollDispApp.disp_amt" label="处置金额" maxlength="16" required="true" dataType="Currency"/>
			<emp:select id="ArpCollDispApp.asset_disp_mode" label="资产处置方式" required="true" dictname="STD_ASSET_DISP_MODEL" />
