...
<script type="text/javascript">
	...
	function doSubmits(){
		url = 'doReturn';
		doPubAdd(url,ArpLawPreserveInfo);
	};

	function doReturn() {
		var url = '<emp:url action="queryArpLawPreserveInfoList.do"/>?case_no=<%=case_no%>';
...
<body class="page_content" onload="doLoad()">
	...
	<emp:form id="submitForm" action="addArpLawPreserveInfoRecord.do" method="POST">		
		<emp:gridLayout id="ArpLawPreserveInfoGroup" title="保全信息" maxColumn="2">
			...
			<emp:text id="ArpLawPreserveInfo.case_no" label="案件编号" maxlength="40" required="true" readonly="true" defvalue="<%=case_no%>"/>			
		...
		<emp:gridLayout id="ArpLawPreserveInfoGroup" maxColumn="2" title="登记信息">
			...
			<emp:text id="ArpLawPreserveInfo.input_id_displayname" label="登记人" readonly="true" required="true" defvalue="$currentUserName" />
			<emp:text id="ArpLawPreserveInfo.input_br_id_displayname" label="登记机构" readonly="true" required="true" defvalue="$organName" />
			<emp:text id="ArpLawPreserveInfo.input_id" label="登记人" required="true"  defvalue="$currentUserId" hidden="true"/>
			<emp:text id="ArpLawPreserveInfo.input_br_id" label="登记机构" required="true" defvalue="$organNo" hidden="true" />
			<emp:date id="ArpLawPreserveInfo.input_date" label="登记日期" required="true"  defvalue="$OPENDAY" readonly="true" />
		</emp:gridLayout>
