...
<script type="text/javascript">
	...
	function doGetUpdateCcrAppInfoPage() {
		...
		if (paramStr != null) {
			...
			var url = '<emp:url action="getCcrAppInBankUpdatePage.do"/>?'+paramStr;
	...
	function doViewCcrAppInfo() {
		var paramStr = CcrAppInfoList._obj.getParamStr(['serno']);
		if (paramStr != null) {
			var url = '<emp:url action="getCcrAppInBankViewPage.do"/>?'+paramStr;
			url = EMPTools.encodeURI(url);
			window.location = url;
		} else {
			alert('请先选择一条记录！');
	...
	function doGetAddCcrAppInfoPage() {
		var url = '<emp:url action="getCcrAppInBankAddPage.do"/>';
		url = EMPTools.encodeURI(url);
		window.location = url;
	};
	...
	function returnCus(data){
		...
		CcrAppInfo.cus_id_displayname._setValue(data.same_org_cnname._getValue());
    };
...
<body class="page_content">
	...
	<emp:gridLayout id="CcrAppInfoGroup" title="输入查询条件" maxColumn="2">
			...
			<emp:pop id="CcrAppInfo.cus_id_displayname" label="客户名称" url="getValueQueryUpOrgEstList.do" returnMethod="returnCus" />	
	...
	<emp:table icollName="CcrAppInfoList" pageMode="true" url="pageCcrAppInBankQuery.do">
