...
<script type="text/javascript">
	...
	function doViewCcrAppInfo() {
		var paramStr = CcrAppInfoList._obj.getParamStr(['serno']);
		if (paramStr != null) {
			var url = '<emp:url action="getCcrAppInfoViewPage.do"/>?'+paramStr;
			url = EMPTools.encodeURI(url);
			window.location = url;
		} else {
			alert('请先选择一条记录！');
	...
	function doGetAddCcrAppInfoPage() {
		var url = '<emp:url action="getCcrAppInfoAddPage.do"/>';
		url = EMPTools.encodeURI(url);
		window.location = url;
	};
...
<body class="page_content">
	...
	<emp:gridLayout id="CcrAppInfoGroup" title="输入查询条件" maxColumn="2">
			...
			<emp:select id="CcrAppInfo.approve_status" label="申请状态" dictname="WF_APP_STATUS"  />
	...
	<emp:table icollName="CcrAppInfoList" pageMode="true" url="pageCcrAppInfoQuery.do">
