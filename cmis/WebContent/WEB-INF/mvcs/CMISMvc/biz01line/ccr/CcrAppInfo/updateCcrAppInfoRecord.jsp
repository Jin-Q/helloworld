...
//		CcrAppInfo.model_no._obj.addOneButton('view11','查看',viewModelInfo);  //在评级模型后面新增 查看 按钮
	...
	function doReturnMethod(json, callback){
	   ...
	   window.location.reload();
		...
		//	ccrAppTab.tabs.ratingtab.refresh();
		}
	}
	//获取机评得分
...
//		}
		...
		WfiJoin.appl_type._setValue("670");  //流程申请类型，对应字典项[ZB_BIZ_CATE]，对应流程标识：ccr_app_info
...
<emp:tabGroup id="ccrAppTab" mainTab="appinf">
     <emp:tab label="申请信息" id="appinf"  needFlush="true" initial="true" >
		...
		<emp:gridLayout id="CcrAppInfoGroup" maxColumn="2" title="信用评级申请主表">
			...
			<emp:text id="CcrAppDetail.model_no_displayname" label="评级模型"  required="false" cssElementClass="emp_field_text_readonly" readonly="true" defvalue="${context.model_name}"/>
		...
		<emp:gridLayout id="CusComGroup" maxColumn="2" title="信用评级申请附表">
		    ...
		    <emp:text id="CcrAppInfo.flag" label="申请类型" maxlength="40"  hidden="true" defvalue="1"/>
		</emp:gridLayout>
	  ...
	  <emp:tab label="信用等级评估表" id="ratingtab"  needFlush="true" initial="true">
			...
			<emp:gridLayout id="scorePanel" maxColumn="2" title="信用评级得分">
				...
				<emp:select id="CcrAppDetail.auto_grade" label="机评等级"  required="false" disabled="true" dictname="STD_ZB_CREDIT_GRADE" colSpan="2" />
				<emp:select id="CcrAppDetail.limit_grade" label="限定等级" required="false" disabled="true" dictname="STD_ZB_CREDIT_GRADE" colSpan="2" />
				<emp:textarea id="CcrAppDetail.limit_reason" label="限定原因" maxlength="500" required="false" colSpan="2" readonly="true"/>
				<emp:select id="CcrAppDetail.adjusted_grade" label="客户经理调整等级" readonly="false" required="true" colSpan="2"  dictname="STD_ZB_CREDIT_GRADE" onblur="saveChange()"/>
