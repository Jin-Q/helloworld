...
<%
	...
	String lmt_serno = "";
	if(context.containsKey("CcrAppInfo.lmt_serno")){
		lmt_serno = (String)context.getDataValue("CcrAppInfo.lmt_serno");
	}
	String flow = "";//流程查看标识
...
function setconId(data){
	CcrAppInfo.manager_id._setValue(data.actorname._getValue());
}
//主管机构
...
//	var allScore = '${context.CcrAppInfo.all_score}';
//	defvalue="${context.CcrModelScore.model_name}"
	//对于个人客户经理调整得分必输，企业则客户经理调整等级必输
	/*if('M20091200009' == modelNo){
		CcrAppDetail.adjusted_score2._obj._renderRequired(true);
		CcrAppDetail.adjusted_score._obj._renderRequired(true);
	}else{
		CcrAppDetail.adjusted_grade2._obj._renderRequired(true);
		CcrAppDetail.adjusted_grade._obj._renderRequired(true);
	}
	
	if (cusType.substring(0,1) == '1') {
		ccrAppTab.tabs.fin_tab.renderHidden(true);
		ccrAppTab.tabs.san_tab.renderHidden(true);
		CcrAppDetail.com_fld_typ._obj._renderHidden(true);
		CcrAppDetail.com_opt_scale._obj._renderHidden(true);
	}	
	//集团客户类型
	//var grpCusType='${context.CcrAppDetail.com_grp_mode}';
	//if(grpCusType=='2'){//集团客户子公司
	//	CcrAppDetail.grp_cus_id._obj._renderHidden(false);
	//	CcrAppDetail.grp_cus_name._obj._renderHidden(false);
	//	CcrAppDetail.grp_crd_grade._obj._renderHidden(false);
	//	CcrAppDetail.grp_crd_date._obj._renderHidden(false);
	//	CcrAppDetail.grp_crd_end_date._obj._renderHidden(false);
	//}else{
	//	CcrAppDetail.grp_cus_id._obj._renderHidden(true);
	//	CcrAppDetail.grp_cus_name._obj._renderHidden(true);
	//	CcrAppDetail.grp_crd_grade._obj._renderHidden(true);
	//	CcrAppDetail.grp_crd_date._obj._renderHidden(true);
	//	CcrAppDetail.grp_crd_end_date._obj._renderHidden(true);
	//}
	*/
...
function doReturn() {
	...
	var menuId = '<%=menuId%>';
	var url = '';
	if(menuId=='IndivCcrApp'||menuId=='IndivCcrAppHistory'){
		url = '<emp:url action="queryCcrAppIndivInfoAllList.do"/>?menuId='+menuId;
	}else{
...
function doReturnMethod(json, callback){
	...
	var button = document.getElementById('button_creditRating');
	button.disabled="";	
	alert('${context.CcrAppDetail.auto_grade}');
	//如果调整得分不为空,则更新调整等级
	alert(CcrAppDetail.adjusted_grade._getValue()+'******************');
	if(CcrAppDetail.adjusted_grade._getValue()!=''){
		saveAdjuest_gradeOnly();
	}
	
	//CcrAppDetail.auto_grade._setValue("19");
...
function doSaveAdjuest_grade(){
	...
	var result = CcrAppInf._checkAll();
    if(result){
...
function saveAdjuest_gradeOnly(){
    ...
    if(result&&result1){
    	...
    	CcrAppDetail.all_score._toForm(form);
...
<body class="page_content" onload="doload()">
  ...
  <emp:tab label="申请信息" id="appinf"  needFlush="true" initial="true" >
		...
		<emp:gridLayout id="CcrAppInfoGroup" maxColumn="2" title="信用评级申请主表">
			<emp:text id="CcrAppInfo.serno" label="业务编号" maxlength="40" required="true" readonly="true"/>
			<emp:text id="CcrAppDetail.model_no_displayname" label="评级模型"  required="false" readonly="true" cssElementClass="emp_field_text_readonly" defvalue="${context.model_name}"/>
			...
			<emp:select id="CcrAppInfo.auto_grade1" label="机评信用等级"  required="false" readonly="true" dictname="STD_ZB_CREDIT_GRADE" defvalue="$CcrAppDetail.auto_grade"/>
			<emp:select id="CcrAppInfo.adj_auto_grade1" label="客户经理调整信用等级" required="false"  readonly="true" dictname="STD_ZB_CREDIT_GRADE" defvalue="$CcrAppDetail.adjusted_grade" />
	
		
		</emp:gridLayout>
		<emp:gridLayout id="CusComGroup" maxColumn="2" title="信用评级申请附表">
			<emp:select id="CcrAppDetail.last_adjusted_grade" label="上次信用等级" required="false"  readonly="true"  dictname="STD_ZB_CREDIT_GRADE"/>
		...
		<emp:gridLayout id="RegGroup" maxColumn="2" title="登记信息">
			...
			<emp:select id="CcrAppInfo.approve_status" label="申请状态" required="false" dictname="WF_APP_STATUS" defvalue="000" readonly="true" hidden="true"/>
  ...
  <emp:tab label="信用等级评估表" id="ratingtab"  needFlush="true" initial="true">
		...
		<emp:gridLayout id="scorePanel" maxColumn="2" title="信用评级得分">
			...
			<emp:select id="CcrAppDetail.limit_grade" label="限定等级" dictname="STD_ZB_CREDIT_GRADE" required="false" readonly="true" colSpan="2" />
			<emp:textarea id="CcrAppDetail.limit_reason" label="限定原因" maxlength="500" required="false" colSpan="2" />
			<emp:select id="CcrAppDetail.auto_grade" label="机评等级"  required="false"  readonly="true" dictname="STD_ZB_CREDIT_GRADE" colSpan="2" />
			<emp:select id="CcrAppDetail.adjusted_grade" label="客户经理调整等级" readonly="true" required="false" colSpan="2"  dictname="STD_ZB_CREDIT_GRADE" onblur="saveChange()"/>
