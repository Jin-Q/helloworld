...
function setconId(data){
	...
	<%if(isFlag.equals("1")){%>//是授信项下时，授信和评级一起走
...
function doload(){
	...
	function doCreditRating(){
		...
		var result1 = CcrAppDetail.bail_multi._checkAll();
		var result2 = CcrAppDetail.guar_type._checkAll();
	    if(result&&result1&&result2){
	    	...
	    	CcrAppDetail.bail_multi._toForm(form);
	    	CcrAppDetail.guar_type._toForm(form);
	    	var callback = {
...
function doSaveAdjuest_grade(){
	...
	var result = CcrAppInfo._checkAll();
 	...
 	<%if(isFlag.equals("1")){%>//是否授信项下，“是”时 ，需要同步对授信信息进行保存。
 	var result2 = LmtAppFinGuar._checkAll();
 	 if(result&&result1&&result2){
 		LmtAppFinGuar._toForm(form);
 		CcrAppDetail.is_authorize._toForm(form);
 	<%}else{%>
    if(result&&result1){
    <%}%>
    	CcrAppInfo.manager_id._toForm(form);
	    ...
	    if(result&&result1){
			if(isFlag=='1'){
				...
				if(result2){
					...
					window.ccrAppTab.tabs['lmt_app']._clickLink();
					alert("保存失败！\n请检查各标签页面中的必填信息是否遗漏！");
					return;
				}
			}
	...
	//只获取机评得分，不校验授信必输项，不进行提交和当前页面的刷新
	function getGradeOnlyNoFlush(){
		var isFlag = '<%=isFlag%>';
		var form = document.getElementById("updateGradeForm");
		var result = CcrAppInfo._checkAll();
	 	var result1 = CcrAppDetail._checkAll();
	...
	function doSubmitWF(){
		...
		WfiJoin.appl_type._setValue("650");  //流程申请类型，对应字典项[ZB_BIZ_CATE]，对应流程标识：ccr_app_info
		WfiJoin.cus_id._setValue(cus_id);
		WfiJoin.cus_name._setValue(cus_name);
		WfiJoin.prd_name._setValue("信用评级-融资性担保公司评级");
		initWFSubmit(false);
	}
	...
	function viewCusInfo(){
		var url = "<emp:url action='getCusViewPage.do'/>&cusId="+CcrAppInfo.cus_id._getValue();
      	url=encodeURI(url); 
	...
	function saveChange(){
		...
		<%if(isFlag.equals("1")){%>
		LmtAppFinGuar.eval_rst._setValue(CcrAppDetail.adjusted_grade._getValue());
		window.ccrAppTab.tabs['lmt_app']._clickLink();
		<%}%>
	...
	function doWrite(){
		<%if(isFlag.equals("1")){%>
		LmtAppFinGuar.guar_bail_multiple._setValue(CcrAppDetail.bail_multi._getValue());
		<%}%>
	}
	function doWriteTo(){
		<%if(isFlag.equals("1")){%>
		LmtAppFinGuar.guar_cls._setValue(CcrAppDetail.guar_type._getValue());
		<%}%>
	}
	function setCusInfo(){
		LmtAppFinGuar.serno._setValue(CcrAppInfo.serno._getValue());
		LmtAppFinGuar.cus_id._setValue(CcrAppInfo.cus_id._getValue());
		LmtAppFinGuar.cus_id_displayname._setValue(CcrAppInfo.cus_id_displayname._getValue());
    }
	function checkMoney(){
...
<emp:tabGroup id="ccrAppTab" mainTab="appinf">
  <emp:tab label="申请信息" id="appinf"  needFlush="true" initial="true" >
		...
		<emp:gridLayout id="CcrAppInfoGroup" maxColumn="2" title="信用评级基本信息">
			...
			<emp:text id="CcrAppDetail.bail_multi" label="担保放大倍数" maxlength="30" required="true" readonly="false" dataType="Currency" onchange="doWrite()"/>
			<emp:select id="CcrAppDetail.guar_type" label="担保类别" required="true"  readonly="false" dictname="STD_ZB_GUAR_TYPE" onchange="doWriteTo()"/>
  ...
  <emp:tab id="fin_tab" label="财务报表" url="queryFncStatLmtList.do" reqParams="cus_id=$CcrAppInfo.cus_id;" initial="false" needFlush="true"/>
	<% if(isFlag.equals("1")){//是授信项下值为，“是”时显示%>
   <emp:tab label="融资性担保公司额度申请" id="lmt_app"  needFlush="true" initial="true">
		...
		<emp:gridLayout id="LmtAppFinGuarGroup" title="融资额度信息" maxColumn="2">
			<emp:select id="LmtAppFinGuar.guar_cls" label="担保类别" required="true" dictname="STD_ZB_GUAR_TYPE" readonly="true" defvalue="$CcrAppDetail.guar_type"/>
			<emp:select id="LmtAppFinGuar.eval_rst" label="评级结果" required="true" dictname="STD_ZB_FINA_GRADE" readonly="true" defvalue="$CcrAppDetail.adjusted_grade"/>
			<emp:text id="LmtAppFinGuar.guar_bail_multiple" label="担保放大倍数" maxlength="10" required="true" readonly="true" defvalue="$CcrAppDetail.bail_multi"/>
			...
			<emp:pop id="LmtAppFinGuar.manager_id_displayname" label="责任人" url="getValueQuerySUserPopListOp.do?restrictUsed=false" returnMethod="setconId" required="false" hidden="true"/>
			<emp:pop id="LmtAppFinGuar.manager_br_id_displayname" label="责任机构" url="querySOrgPop.do?restrictUsed=false" returnMethod="getOrgID" required="false" hidden="true"/>
			<emp:text id="LmtAppFinGuar.input_id_displayname" label="登记人"  required="false" defvalue="$currentUserId" hidden="true"/>
			<emp:text id="LmtAppFinGuar.input_br_id_displayname" label="登记机构"  required="false" defvalue="$organNo" hidden="true"/>
			...
			<emp:text id="LmtAppFinGuar.input_date" label="登记日期" maxlength="10" required="false" defvalue="$OPENDAY" hidden="true"/>
