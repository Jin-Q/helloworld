	...
	function doGetUpdateCtrLimitAppPage() {
		...
		if (paramStr != null) {
		  if(appStatus == "000" || appStatus == "992" || appStatus == "993"){
			var url;
			if(appType == 01){
				url = '<emp:url action="getCtrLimitAppUpdatePage.do"/>?op=update&'+paramStr+'&approve_status='+appStatus;
			}else {
				url = '<emp:url action="updateCtrLimitContChangePage.do"/>?&op=update&'+paramStr+'&approve_status='+appStatus;
			}
...
//提交流程
	function doSubmitWF(){
		var paramStr = CtrLimitAppList._obj.getParamStr(['serno']);
		if (paramStr != null) {
			var serno = CtrLimitAppList._obj.getSelectedData()[0].serno._getValue();
			var cus_id = CtrLimitAppList._obj.getSelectedData()[0].cus_id._getValue();
			var cus_name = CtrLimitAppList._obj.getSelectedData()[0].cus_id_displayname._getValue();
			var approve_status = CtrLimitAppList._obj.getSelectedData()[0].approve_status._getValue();
			var app_amt = CtrLimitAppList._obj.getSelectedData()[0].app_amt._getValue();
