...
<script type="text/javascript">
	...
	CtrRpddscntCont._checkAll();

	if(CtrRpddscntCont._checkAll()){
		...
		var handleSuccess = function(o){
			if(o.responseText !== undefined) {
				...
				if(flag == "success"){
					...
					//var url = '<emp:url action="queryCtrRpddscntContList.do"/>';
...
<body class="page_content" onload="doOnLoad()">
	<emp:form id="submitForm" action="updateCtrRpddscntContRecord.do" method="POST">
	<emp:tabGroup mainTab="base_tab" id="mainTab" >
	 <emp:tab label="合同信息" id="base_tab" needFlush="true" initial="true" >
		<emp:gridLayout id="CtrRpddscntContGroup" title="转贴现申请信息" maxColumn="2">
					...
					<emp:pop id="CtrRpddscntCont.toorg_no" label="交易对手行号" url="getPrdBankInfoPopList.do?status=1" readonly="true" returnMethod="getToorgNo" required="true" buttonLabel="选择" />
					...
					<!--modified by wangj 需求编号：ED150612003 ODS系统取数需求  begin-->
					<emp:pop id="CtrRpddscntCont.tooorg_no" label="交易对手开户行行号" url="getPrdBankInfoPopList.do?status=1" returnMethod="getTooorgNo" readonly="true" required="false" hidden="true" buttonLabel="选择" />
					<!--modified by wangj 需求编号：ED150612003 ODS系统取数需求  end-->
					...
					<!--modified by wangj 需求编号：ED150612003 ODS系统取数需求  begin-->
					<emp:pop id="CtrRpddscntCont.acctsvcr_no" label="本行开户行行号" url="getPrdBankInfoPopList.do?status=1" readonly="true" returnMethod="getAcctsvcrNo" required="false" hidden="true" buttonLabel="选择" />
					<!--modified by wangj 需求编号：ED150612003 ODS系统取数需求  end-->
				...
				<emp:gridLayout id="CtrRpddscntContGroup" title="票据信息" maxColumn="2">
					...
					<emp:textarea id="CtrRpddscntCont.memo" label="备注" maxlength="250" required="false" colSpan="2" readonly="true"/>
				</emp:gridLayout>
				<emp:gridLayout id="" maxColumn="2" title="额度信息">
					<emp:select id="CtrRpddscntCont.limit_ind" label="授信额度使用标志" required="true" onchange="isUseLimt()" dictname="STD_INTBANK_LIMIT_IND" colSpan="2" readonly="true"/>	   
		    		<emp:pop id="CtrRpddscntCont.limit_acc_no" label="授信台账编号"  url="selectLmtAgrDetails.do" returnMethod="getLmtAmt" required="false" readonly="true" buttonLabel="选择" popParam="height=700, width=1200, top=80, left=80, toolbar=no, menubar=no, scrollbars=yes, resizable=yes, location=no, status=no"/>
