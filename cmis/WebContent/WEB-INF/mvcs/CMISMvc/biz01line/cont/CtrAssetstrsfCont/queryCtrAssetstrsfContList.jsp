...
<script type="text/javascript">
	...
	function doGetUpdateCtrAssetstrsfContPage() {
		...
		if (paramStr != null) {
			...
			var url = '<emp:url action="getCtrAssetstrsfContUpdatePage.do"/>?op=update&cont=cont&'+paramStr;
			...
			window.open(url,'newWindow_CtrAssetstrsfCont',param); 
	...
	function doViewCtrAssetstrsfCont() {
		...
		if (paramStr != null) {
		    /** modified by yangzy 20140928 合同查询模块查看及签订功能改为弹出框查看模式 begin **/
			var url = '<emp:url action="getCtrAssetstrsfContViewPage.do"/>?op=view&viewtype=out&cont=cont&'+paramStr;
			url = EMPTools.encodeURI(url);
			//window.location = url;
			var param = 'height=700, width=1200, top=80, left=80, toolbar=no, menubar=no, scrollbars=yes, resizable=no, location=no, status=no';
			window.open(url,'newWindow_CtrAssetstrsfCont',param); 
	...
	function doDeleteCtrAssetstrsfCont() {
		...
		if (paramStr != null) {
			if(confirm("是否确认要删除？")){
				var url = '<emp:url action="deleteCtrAssetstrsfContRecord.do"/>?'+paramStr;
				url = EMPTools.encodeURI(url);
				window.location = url;
			}
		} else {
    ...
    function doRemoveCtrLoanCont(){
    	...
    	if (paramStr != null) {
    		...
    		if(confirm("是否确认要作废？")){
    			...
    			var url = '<emp:url action="removeCtrAssetstrsfCont.do"/>?'+paramStr;	
    			url = EMPTools.encodeURI(url);
...
<body class="page_content">
	...
	<emp:gridLayout id="CtrAssetstrsfContGroup" title="输入查询条件" maxColumn="2">
	...
	<div align="left">
		<emp:button id="getUpdateCtrAssetstrsfContPage" label="签订" op="update"/>
	...
	<emp:table icollName="CtrAssetstrsfContList" pageMode="true" url="pageCtrAssetstrsfContQuery.do">
