...
function doSave(){
	if(CtrLimitLmtRelTemp._checkAll()){
		var serno = CtrLimitLmtRelTemp.limit_serno._getValue();
		...
		var handleSuccess = function(o){
			if(o.responseText !== undefined) {
				...
				if(flag == "success" && serno != null){
					var	url = '<emp:url action="queryCtrLimitLmtRelTempList.do"/>?serno='+serno;
...
<body class="page_content">
	...
	<emp:form id="submitForm" action="updateCtrLimitLmtRelTempRecord.do" method="POST">
		<emp:gridLayout id="CtrLimitLmtRelTempTempGroup" maxColumn="2" title="额度合同占用授信关联表">
			...
			<emp:pop id="CtrLimitLmtRelTemp.lmt_code_no" label="授信额度编号" url="queryLmtAgrDetailsPop.do?returnMethod=getLmtMsg&flag=2&cus_id=${context.cus_id}" returnMethod="getLmtMsg"  required="true" />
