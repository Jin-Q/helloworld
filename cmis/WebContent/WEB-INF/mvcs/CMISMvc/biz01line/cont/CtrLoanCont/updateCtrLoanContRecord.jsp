...
function doOnLoad(){ 
	/**added by wangj 2015/05/28  需求编号:XD141222087,法人账户透支需求变更  begin**/
	oldIsFreeze=CtrLoanCont.CtrLoanContSub.is_freeze._getValue();
	/**added by wangj 2015/05/28  需求编号:XD141222087,法人账户透支需求变更  end**/
	...
	*************************************************/
	...
	if(prd_id=="100051"){
		CtrLoanCont.CtrLoanContSub.overdrawn_term._obj._renderHidden(false);
		CtrLoanCont.CtrLoanContSub.overdrawn_term._obj._renderRequired(true);
		CtrLoanCont.CtrLoanContSub.overdrawn_type._obj._renderHidden(false);
		CtrLoanCont.CtrLoanContSub.overdrawn_type._obj._renderRequired(true);
		CtrLoanCont.CtrLoanContSub.overdrawn_type._setValue("003"); 
		CtrLoanCont.CtrLoanContSub.is_freeze._obj._renderHidden(false);
		CtrLoanCont.CtrLoanContSub.is_freeze._obj._renderRequired(true);
		if(oldIsFreeze==""){//是否冻结  为空默认为否
			CtrLoanCont.CtrLoanContSub.is_freeze._setValue("2"); 
		}
		...
		CtrLoanCont.in_acct_br_id._obj._renderRequired(true);
		...
		CtrLoanCont.CtrLoanContSub.overdrawn_type._obj._renderHidden(false);
		CtrLoanCont.CtrLoanContSub.overdrawn_type._obj._renderRequired(true);
		CtrLoanCont.CtrLoanContSub.overdrawn_type._obj._renderReadonly(false);
		//var options4ot = CtrLoanCont.CtrLoanContSub.overdrawn_type._obj.element.options;
		////去除申请字典项为"日"
		...
		CtrLoanCont.in_acct_br_id._obj._renderRequired(false);

	  }else{
		CtrLoanCont.in_acct_br_id_displayname._obj._renderHidden(true);
		CtrLoanCont.in_acct_br_id_displayname._obj._renderRequired(false);
		CtrLoanCont.in_acct_br_id._obj._renderRequired(false);
		CtrLoanCont.CtrLoanContSub.is_freeze._setValue("2"); //是否冻结 
	}
	...
	if(prd_id=="100051"){
		CtrLoanCont.CtrLoanContSub.belg_line._obj._renderHidden(false);
		CtrLoanCont.CtrLoanContSub.belg_line._obj._renderRequired(true);
	}
    ...
    if(prd_id =="100090" || ("${context.flg}" == "csgn" && prd_id =="100063") || prd_id =="400020" || prd_id =="400021"){
    	 CtrLoanCont.is_structured_fin._obj._renderRequired(true);
    	 CtrLoanCont.is_structured_fin._obj._renderHidden(false);
    	 CtrLoanCont.CtrLoanContSub.marketing_br_id._obj._renderHidden(false);
    	 CtrLoanCont.CtrLoanContSub.marketing_br_id._obj._renderRequired(true);
	 }else{
		 CtrLoanCont.is_structured_fin._obj._renderRequired(false);
	     CtrLoanCont.is_structured_fin._obj._renderHidden(true);
	     CtrLoanCont.CtrLoanContSub.marketing_br_id._obj._renderHidden(true);
	     CtrLoanCont.CtrLoanContSub.marketing_br_id._obj._renderRequired(false);
	 }
...
function isShow(){
    ...
    //风险拦截
    function interRisk(){
	   var serno = CtrLoanCont.serno._getValue();
	   var _applType="";
	   var _modelId="CtrLoanCont";
	   var _pkVal=serno;
	   var _preventIdLst="FFFA2794004CB3A4B7304EE5A7181F95";
   ...
   function doSub(){
	...
	CtrLoanCont.CtrLoanContSub._checkAll();
	if(!interRisk()){
		return false;
	}
	if(CtrLoanCont._checkAll() && CtrLoanCont.CtrLoanContSub._checkAll()){
		CtrLoanCont._toForm(form);
		CtrLoanCont.CtrLoanContSub._toForm(form);
		var handleSuccess = function(o){
			if(o.responseText !== undefined) {
				...
				if(flag == "success"){
					...
					//var url = '<emp:url action="queryCtrLoanContList.do"/>?menuId=${context.menuId}&biz_type='+${context.CtrLoanCont.biz_type};
...
/** added by yangzy 2015/10/28 需求：XD150407026， 贸易融资外币业务汇率，合同签订同步申请业务敞口及汇率 start **/
function doIqpRiskOpenAmt(){
	var form4iqpRisk = document.getElementById('submitForm');
	CtrLoanCont._toForm(form4iqpRisk);
	var handleSuccess = function(o) {
		if (o.responseText !== undefined) {
			try {
				var jsonstr = eval("(" + o.responseText + ")");
	...
	var url = '<emp:url action="updateIqpRiskOpenAmt4Cont.do"/>';
	url = EMPTools.encodeURI(url);
	var postData = YAHOO.util.Connect.setForm(form4iqpRisk);	 
	var obj1 = YAHOO.util.Connect.asyncRequest('POST',url,callback,postData);
}
/** added by yangzy 2015/10/28 需求：XD150407026， 贸易融资外币业务汇率，合同签订同步申请业务敞口及汇率 end **/
function doBack() {
    /** modified by yangzy 20140928 合同查询模块查看及签订功能改为弹出框查看模式 begin **/
    var url = '<emp:url action="queryCtrLoanContList.do"/>?biz_type=${context.CtrLoanCont.biz_type}&menuId=${context.menuId}'; 
	//url = EMPTools.encodeURI(url); 
	//window.location=url; 
    window.close();
    /** modified by yangzy 20140928 合同查询模块查看及签订功能改为弹出框查看模式 end **/
...
//业务合同签订前判断一般担保合同有没有签订
function doCheckGrtGuarCont(){
	...
	/**added by wangj 2015/05/28  需求编号:XD141222087,法人账户透支需求变更  end**/
	/**mofified by lisj 2015-6-30 需求编号：XD150123005 小微自助循环贷款改造 end**/
	var cont_no = CtrLoanCont.cont_no._getValue();
	var handleSuccess = function(o){
		if(o.responseText !== undefined) {
			try {
				var jsonstr = eval("("+o.responseText+")");
			...
			/**added by wangj 2015/05/12  需求编号:XD141222087,法人账户透支需求变更  begin**/	
				if(prd_id =="100051" || prd_id =="100088"){
					...
					/** modified by yangzy 2015/10/28 需求：XD150407026， 贸易融资外币业务汇率，合同签订同步申请业务敞口及汇率 start **/
					//doSub();
					doIqpRiskOpenAmt();
					/** modified by yangzy 2015/10/28 需求：XD150407026， 贸易融资外币业务汇率，合同签订同步申请业务敞口及汇率 end **/
				}
			/**added by wangj 2015/05/12  需求编号:XD141222087,法人账户透支需求变更  end**/
			/**mofified by lisj 2015-6-30 需求编号：XD150123005 小微自助循环贷款改造 end**/	
			}
	...
	var url = '<emp:url action="checkGrtGuarCont.do"/>?cont_no='+cont_no;
	url = EMPTools.encodeURI(url);   
...
function checkOverDrawn(){
	  var prd_id = CtrLoanCont.prd_id._getValue();
	  if(prd_id=="100051"){
		  var term=CtrLoanCont.CtrLoanContSub.overdrawn_term._getValue();
		  if(term == null || term == ""){
			 // alert("请录入透支期限！");
			  CtrLoanCont.CtrLoanContSub.overdrawn_term._obj.element.focus();
			  return;
	       }
		 if(parseInt(term)== 0||parseInt(term)>=90){
			 CtrLoanCont.CtrLoanContSub.overdrawn_term._setValue("");
			 alert("透支期限不能超过90天！");
			 CtrLoanCont.CtrLoanContSub.overdrawn_term._obj.element.focus();
			 return;
		  }
	  ...
	  if(prd_id=="100088"){
		  var term=CtrLoanCont.CtrLoanContSub.overdrawn_term._getValue();
		  var type=CtrLoanCont.CtrLoanContSub.overdrawn_type._getValue();
		  if(type == null || type == ""){
			  alert("请先选择透支期限！");
			  return;
		  }
...
function checkOverdrawnTerm(){
	/**modified by lisj 2015-7-1 需求编号：【XD150123005】小微自助循环贷款改造 begin**/
	var cont_no=CtrLoanCont.cont_no._getValue();
	var cont_start_date=CtrLoanCont.cont_start_date._getValue();
	var cont_end_date=CtrLoanCont.cont_end_date._getValue();
	var overdrawn_term=CtrLoanCont.CtrLoanContSub.overdrawn_term._getValue();
	var overdrawn_type = CtrLoanCont.CtrLoanContSub.overdrawn_type._getValue();
	var prd_id = CtrLoanCont.prd_id._getValue();
	var cont_term =CtrLoanCont.CtrLoanContSub.cont_term._getValue();
	var term_type  =CtrLoanCont.CtrLoanContSub.term_type._getValue();
	var handleSuccess = function(o){
		if(o.responseText !== undefined) {
			try {
				var jsonstr = eval("("+o.responseText+")");
			...
			if("isFreeze"==flag){
				...
				CtrLoanCont.CtrLoanContSub.overdrawn_term._obj.element.focus();
				return ;
			}else if(flag == "errorTerm"){
				alert("透支期限要大于或等于30天且不允许超过1年或12个月或365天！");
				CtrLoanCont.CtrLoanContSub.overdrawn_term._obj.element.focus();
			}else if(flag == "overTerm"){
				alert("透支期限不能大于合同期限！");
				CtrLoanCont.CtrLoanContSub.overdrawn_term._obj.element.focus();
			}else{
				/** modified by yangzy 2015/10/28 需求：XD150407026， 贸易融资外币业务汇率，合同签订同步申请业务敞口及汇率 start **/
	...
	var url = '<emp:url action="checkContOverdrawnTerm.do"/>?cont_no='+cont_no+'&cont_start_date='+cont_start_date+'&cont_end_date='+cont_end_date+'&overdrawn_term='+overdrawn_term+"&overdrawn_type="+overdrawn_type+"&prd_id="+prd_id+"&cont_term="+cont_term+"&term_type="+term_type;
	url = EMPTools.encodeURI(url);   
	var obj1 = YAHOO.util.Connect.asyncRequest('POST',url, callback,null);
	/**modified by lisj 2015-7-1 需求编号：【XD150123005】小微自助循环贷款改造 end**/	
};
function getAcctOrgID(data){
	CtrLoanCont.in_acct_br_id._setValue(data.organno._getValue());
	CtrLoanCont.in_acct_br_id_displayname._setValue(data.organname._getValue());
};
/**added by wangj 2015/05/12  需求编号:XD141222087,法人账户透支需求变更  end**/	
</script>
	...
	<emp:tabGroup mainTab="base_tab" id="mainTab" >
		 <emp:tab label="合同信息" id="base_tab" needFlush="true" initial="true" >  
          <emp:gridLayout id="CtrLoanContGroup" maxColumn="2" title="合同基本信息">  
			...
			<emp:date id="CtrLoanCont.ser_date" label="签订日期"  required="true" readonly="true" defvalue="${context.OPENDAY}" />
			...
			<emp:pop id="CtrLoanCont.CtrLoanContSub.repay_bill" label="偿还借据" url="queryAccPop.do?returnMethod=getBill&cus_id=${context.CtrLoanCont.cus_id}"  required="false" readonly="true" buttonLabel="选择" popParam="height=700, width=1200, top=80, left=80, toolbar=no, menubar=no, scrollbars=yes, resizable=no, location=no, status=no"/>
			...
			<emp:select id="CtrLoanCont.CtrLoanContSub.is_collect_stamp" label="是否收取印花税" required="true" dictname="STD_ZX_YES_NO" readonly="true" defvalue="1"/>
			...
			<!-- add by lisj 2015-10-13 需求编号：XD150409029 信贷保函及资产模块改造需求 begin -->
			<emp:select id="CtrLoanCont.is_structured_fin" label="是否结构化融资" dictname="STD_ZX_YES_NO" required="false" hidden="true" readonly="true"/>
			<!-- add by lisj 2015-10-13 需求编号：XD150409029 信贷保函及资产模块改造需求 end -->	
			<emp:select id="CtrLoanCont.is_limit_cont_pay" label="是否额度合同项下支用" onchange="isPay();" defvalue="2" required="false" hidden="true"  readonly="true" dictname="STD_ZX_YES_NO" />
		    ...
		    <emp:select id="CtrLoanCont.biz_type" label="业务模式" hidden="true" required="false" readonly="true" dictname="STD_BIZ_TYPE" />
			...
			<!--  /**added by wangj 2015/05/28  需求编号:XD141222087,法人账户透支需求变更  begin**/	-->
			<emp:select id="CtrLoanCont.CtrLoanContSub.is_freeze" label="是否冻结" dictname="STD_ZX_YES_NO" required="false" hidden="true" readonly="true"/>
		  	<!--  /**added by wangj 2015/05/28  需求编号:XD141222087,法人账户透支需求变更  end**/	-->
			<!--  /**added by wangj 2015/08/19  需求编号:XD150825064  源泉宝法人账户透支改造  begin**/	-->
			<emp:select id="CtrLoanCont.CtrLoanContSub.belg_line" label="业务条线" dictname="STD_ZB_BUSILINE" required="false" hidden="true" readonly="true"/>
		...
		<emp:gridLayout id="" maxColumn="2" title="期限信息">
		    <emp:date id="CtrLoanCont.cont_start_date" label="合同起始日期" required="true" defvalue="$OPENDAY"  onblur="caculateContEndDate()" />
		    ...
		    <!--  /**added by wangj 2015/05/12  需求编号:XD141222087,法人账户透支需求变更  begin**/	-->
		    <emp:text id="CtrLoanCont.CtrLoanContSub.overdrawn_term" label="透支期限" required="false" dataType="Int" onchange="checkOverDrawn()" hidden="true"/>					
		    <emp:select id="CtrLoanCont.CtrLoanContSub.overdrawn_type" label="透支类型" required="false" hidden="true" readonly="true" dictname="STD_ZB_TERM_TYPE" />		    
		     <!--  /**added by wangj 2015/05/12  需求编号:XD141222087,法人账户透支需求变更  end**/	-->
		...
		<emp:gridLayout id="" maxColumn="2" title="利率信息">    
			...
			<emp:text id="CtrLoanCont.CtrLoanContSub.default_rate_y" cssElementClass="emp_currency_text_readonly" label="违约利率（年）" hidden="true" maxlength="16" readonly="true" required="false" dataType="Rate"/>
        ...
        <emp:gridLayout id="" maxColumn="2" title="还款方式信息">    
			...
			<emp:select id="CtrLoanCont.CtrLoanContSub.repay_term" label="还款间隔周期" required="true" dictname="STD_BACK_CYCLE" readonly="true"/>
			<emp:text id="CtrLoanCont.CtrLoanContSub.repay_space" label="还款间隔" maxlength="10" required="true" dataType="Int" readonly="true" cssElementClass="emp_currency_text_readonly"/>
		...
		<emp:gridLayout id="" maxColumn="2" title="其他信息"> 
            ...
            <%
			   ...
			   String belg_line = "";
		     	...
		     	if("BL300".equals(belg_line)){%>
		    	    <emp:pop id="CtrLoanCont.CtrLoanContSub.ensure_project_loan_displayname" label="个人经营性贷款归属" url="showDicTree.do?dicTreeTypeId=STD_ZB_DKGS5" returnMethod="projectReturn" required="false" readonly="true" buttonLabel="选择" colSpan="2"  cssElementClass="emp_field_text_long_readonly"/>
		    <%
		     	}else{
			%>
		  ...
		  <emp:gridLayout id="" maxColumn="3" title="登记信息">
		    <!-- add by lisj 2015-10-13 需求编号：XD150409029 信贷保函及资产模块改造需求 begin -->
		    <emp:select id="CtrLoanCont.CtrLoanContSub.marketing_br_id" label="营销机构" dictname="STD_MARKETING_ORG_CODE" required="false" readonly="true"/>
		    <!-- add by lisj 2015-10-13 需求编号：XD150409029 信贷保函及资产模块改造需求 end -->   
		    <emp:text id="CtrLoanCont.manager_br_id_displayname" label="管理机构"  required="true" readonly="true"/>
		    <!-- /**added by wangj 2015/05/12  需求编号:XD141222087,法人账户透支需求变更  begin**/ -->
		    <emp:pop id="CtrLoanCont.in_acct_br_id_displayname" label="入账机构" required="false" url="querySOrgPop.do?restrictUsed=false" returnMethod="getAcctOrgID" reqParams="restrictUsed=false" />
		    <!-- /**added by wangj 2015/05/12  需求编号:XD141222087,法人账户透支需求变更  end**/ -->
		...
		<div align="center">
			<br>
			<emp:button id="checkGrtGuarCont" label="签订" op="update"/>
