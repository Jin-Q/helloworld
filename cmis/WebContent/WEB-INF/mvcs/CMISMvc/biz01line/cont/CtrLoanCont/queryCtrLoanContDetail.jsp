...
function doOnLoad(){ 
    ...
    ************************************************/
    ...
    if(prd_id == "200024"){
    	CtrLoanCont.CtrLoanContSub.actp_status._obj._renderHidden(false);
    }
	...
	isSelfLoans();
	/**added by yangzy 2015/04/02 需求编号:XD150318023,微贷平台零售自助贷款改造  end**/
	/**modified by lisj 2015-6-23  需求编号：XD150123005 小微自助循环贷款改造 begin**/
	...
	if(prd_id=="100051"){
		...
		var oldIsFreeze=CtrLoanCont.CtrLoanContSub.is_freeze._getValue();
	...
	}else if(prd_id=="100084"||prd_id=="100085"){
	      CtrLoanCont.CtrLoanContSub.is_freeze._obj._renderHidden(false);
	    //  CtrLoanCont.CtrLoanContSub.is_freeze._obj._renderRequired(true);
	}
	/**added by wangj 2015/05/12  需求编号:XD141222087,法人账户透支需求变更  end**/
	/**modified by lisj 2015-6-23  需求编号：XD150123005 小微自助循环贷款改造 end**/
   ...
   /**add by lisj 2015-10-13 需求编号：XD150409029 信贷保函及资产模块改造需求 end**/
	...
	//隐藏自助信息
		...
		if(prd_id=="100084"||prd_id=="100085"){
			$(".emp_field_label:eq(47)").text("自助贷款期限");
			$(".emp_field_label:eq(48)").text("自助贷款期限类型");
			CtrLoanCont.CtrLoanContSub.cus_card_code._obj._renderHidden(false);
			CtrLoanCont.CtrLoanContSub.cus_card_code._obj._renderRequired(true);
		}
	}
	/**added by yangzy 2015/04/02 需求编号:XD150318023,微贷平台零售自助贷款改造  end**/
function isShow(){
  ...
  <emp:tabGroup mainTab="base_tab" id="mainTab" >
	 <emp:tab label="合同信息" id="base_tab" needFlush="true" initial="true" >
          <emp:gridLayout id="CtrLoanContGroup" maxColumn="2" title="合同基本信息">
			...
			<!--added by yangzy 2015/04/02 需求编号:XD150318023,微贷平台零售自助贷款改造  begin-->
			<emp:text id="CtrLoanCont.CtrLoanContSub.cus_card_code" label="自助卡号" required="false" hidden="true" readonly="true"/>	
			<!--added by yangzy 2015/04/02 需求编号:XD150318023,微贷平台零售自助贷款改造  end-->
			...
			<!-- added by zhaoxp 2014/12/11 XD140812040, 银行承兑税票改造  start -->
			<emp:select id="CtrLoanCont.CtrLoanContSub.actp_status" label="税票提供状态" dictname="STD_ACTP_STATUS" required="true" readonly="false" defvalue="1" hidden="true"/>
		  	<!-- added by zhaoxp 2014/12/11 XD140812040, 银行承兑税票改造  end -->
		...
		<emp:gridLayout id="" maxColumn="2" title="期限信息">
		    ...
		    <emp:text id="CtrLoanCont.CtrLoanContSub.overdrawn_term" label="透支期限" required="false" dataType="Int"  hidden="true"/>					
		...
		if("trust".equals(flg)){%>
			<emp:tab label="费用信息" id="fee_info_tabs" url="getIqpTrustFeeInfoViewPage.do?serno=${context.CtrLoanCont.serno}"  initial="false" needFlush="true" />
		<% }%>
