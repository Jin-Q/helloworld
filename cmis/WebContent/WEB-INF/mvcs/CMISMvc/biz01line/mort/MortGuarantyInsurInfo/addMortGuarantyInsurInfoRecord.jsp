	...
	function doCheck(){
		var handleSuccess = function(o) {
			if (o.responseText !== undefined) {
				...
				if("false" == check){
					alert("此保险单编号已经存在，请重新录入!");
					MortGuarantyInsurInfo.insuarance_no._setValue("");
				}else{
				}
			}
		};
		...
		var insuarance_no = MortGuarantyInsurInfo.insuarance_no._getValue();
		var url = '<emp:url action="checkInsuaranceNo.do"/>?insuarance_no='+insuarance_no;
		url = EMPTools.encodeURI(url);
...
<body class="page_content">
	...
	<emp:form id="submitForm" action="addMortGuarantyInsurInfoRecord.do" method="POST">
		
		<emp:gridLayout id="MortGuarantyInsurInfoGroup" title="记录抵质押物保险信息" maxColumn="2">
			<emp:text id="MortGuarantyInsurInfo.guaranty_no" label="押品编号" maxlength="40" required="false" hidden="true" defvalue="<%=guaranty_no %>"/>
			...
			<emp:text id="MortGuarantyInsurInfo.insuarance_no" label="保险单编号" maxlength="40" required="true" onchange="doCheck()"/>
