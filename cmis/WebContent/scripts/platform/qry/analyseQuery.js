...
//选择显示字段和排序字段所需js
  ...
  function add2ListForOwn(toList,text,value){
	  var oOption = document.createElement("OPTION");
	  oOption.text = text ;
	  oOption.value = value;
	  document.getElementById(toList).options.add(oOption);
...
function doPopWindow(tablename,selectname,enname){
	...
	/*if(doPopWindow_modify){
		if(doPopWindow_modify(tablename,selectname,cnname)){
	...
	url=url+"&popReturnMethod=onReturn__"+enname.replace('.','')+"&returnMethod=onReturn__"+enname;
	//window.open(url,enname,'height=510, width=750, top=0, left=0, toolbar=no, menubar=no, scrollbars=yes, resizable=no, location=no, status=no');
	window.open(url);
}
...
function doSubmit8(){
	...
	//获取选择字段
	var ShowColumns = '|';
	$("#toListToSelect option").each(function(){
		ShowColumns += $(this).val()+'|';
		//alert('111'+ShowColumns);
	});
	if(ShowColumns == '|'){//如果没有条件
		ShowColumns = '';
	}
	...
	//获取排序字段
	var OrderByColumns='';
	$("#toListToOrder option").each(function(){
		OrderByColumns += $(this).val()+',';
		//alert('222'+OrderByColumns);
	});
...
$(function(){
	...
	//添加双击事件
	$(".buttonclass").on('dblclick',function(){
		$(this).removeClass("selectClass");
		if($(this).parent().attr("id") == "selectLeft"){
			$("#selectRight").append(this);
		}
		else if($(this).parent().attr("id") == "selectRight"){
			$("#selectLeft").append(this);
		}
		else if($(this).parent().attr("id") == "orderLeft"){
			var id = $(this).attr('id');
			if(id.indexOf('desc') == -1 && $("[id='"+id+" desc']").parent("#orderRight").length>0){
				//alert('该字段的降序排列已选择');
			}
			else if(id.indexOf('desc') != -1 && $("[id='"+id.substring(0,id.length-5)+"']").parent("#orderRight").length>0){
				//alert('该字段的升序排序已选择');
			}else{
				$("#orderRight").append(this);
			}
		}
		else if($(this).parent().attr("id") == "orderRight"){
			$("#orderLeft").append(this);
		}
	});
	//添加单机事件
	$("body").on('click',"#selectRight>.buttonclass,#orderRight>.buttonclass",function(){
		$("#selectRight>.buttonclass,#orderRight>.buttonclass").removeClass("selectClass");
		$(this).toggleClass("selectClass");
	});
	
});

function move(type){//Sprev  Snext  Oprev Onext
	try{
		var _this = $(".selectClass").eq(0);
		if(type == 'Sprev' || type == 'Oprev'){//前移
			_this.prev().before(_this);
		}else if(type == 'Snext' || type == 'Onext'){//后移
