   ...
   for(i=0;i<42;i++) {
      ...
      if(sD>-1 && sD<cld.length) { 
		...
		for(j in arrArtdate){
         ...
         if(isart){
			 sObj.innerHTML = (sD+1)+" <img src='scripts/workflow/freeDate/close.gif' style='cursor:hand' alt='点击设置为工作日' onClick=\"doSetWorkDate("+disYMD+")\">";
			 sObj.color = '#CDBC71' ;
			 tdObj.className="freeDateTD";
		 }
		 else{
			 sObj.innerHTML = (sD+1)+" <img src='scripts/workflow/freeDate/dot.gif' style='cursor:hand' alt='点击设置为非工作日' onClick=\"doSetFreeDate("+disYMD+")\">";
		 }
   ...
   switch (K){
      ...
      case 'MU' :
		 ...
		 if(mMonth<0){
			 mMonth=11;
			 mYear--;
			 document.submitForm.action = "echaincommonservlet?method=echainflowmanage&actionType=freedate&subType=getFreeDate";
