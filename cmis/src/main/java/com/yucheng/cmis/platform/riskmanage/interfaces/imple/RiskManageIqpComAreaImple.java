...
public class RiskManageIqpComAreaImple implements RiskManageInterface{

	private final String modelId = "IqpLoanApp";//业务申请表
	private static final Logger logger = Logger.getLogger(RiskManageIqpComAreaImple.class);
	/*圈商授信额度及限额检查
	 * 1.检查当前业务风险敞口金额 <= 圈商额度单户限额；
	 * 2.检查当前业务风险敞口金额+圈商额度下所关联的存量业务风险敞口金额 <= 授信总额。
     * 
	 */
		...
		try {
			...
			if(limit_ind!=null&&!limit_ind.equals("")){
				if(limit_ind.equals("4")||limit_ind.equals("5")||limit_ind.equals("6")){
					...
					BigDecimal area_lmt_amt = new BigDecimal(lmtKColl.getDataValue("lmt_amt")+"");//授信占用
					...
					IndexedCollection iqpContIColl =  SqlClient.queryList4IColl("queryIqpContByAreaCusId", kColl4Query, conn);//存量业务（已包含了当前申请）
					BigDecimal areacus_lmt_amt = new BigDecimal("0");//单户的存量业务风险敞口金额
					for(int i=0;i<iqpContIColl.size();i++){
						...
						if(cont_no==null){
							...
							areacus_lmt_amt = areacus_lmt_amt.add(iqp_amt);
						}else{
					...
					inMap.put("IN_AREA_LMT_AMT", area_lmt_amt+"");
					inMap.put("IN_AREACUS_LMT_AMT", areacus_lmt_amt+"");
					Map<String, String> outMap = new HashMap<String, String>();
					outMap=shuffleService.fireTargetRule("IQPCOMRULE", "IQPCOMRULE_6", inMap);
			...
			logger.error("圈商额度及限额检查失败！"+e.getMessage());
