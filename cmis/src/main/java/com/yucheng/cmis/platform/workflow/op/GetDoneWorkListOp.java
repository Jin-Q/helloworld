 ...
 *
 */
public class GetDoneWorkListOp extends CMISOperation {
	...
	public String doExecute(Context context) throws EMPException {
		...
		try {
			...
			//非拟稿||已经办理  && 流转中 ||预结束||过期办理  && 所有流程参与人LIKE
			//(此处可能存在一个bug（一般不会），当出现用户号如800001、80000等情况时，所以尽量避免存在诸如此类的用户ID)
			String condition2 = "(ALLPROCESSOR='"+currentUserId+"' OR " +
					"ALLPROCESSOR = '"+currentUserId+";' OR " +
					"ALLPROCESSOR LIKE '"+currentUserId+";%' OR " +
					"ALLPROCESSOR LIKE '%;"+currentUserId+";%' OR " +
					"ALLPROCESSOR LIKE '%;"+currentUserId+"' OR " +
					"ALLPROCESSOR LIKE '%;"+currentUserId+";') AND " +
					"CURRENTNODEUSER NOT LIKE '%"+currentUserId+"%' AND" +
					" (BDRAFT='0' OR ISPROCESSED='1') AND (WFSTATUS='0' OR WFSTATUS='4' OR WFSTATUS='5') ";
			...
			EMPLog.log("GetDoneWorkListOp", EMPLog.ERROR, EMPLog.ERROR, "获取已办事项列表出错！异常信息："+e.getMessage(), e);
