...
public class RiskManageIqpComIndusImple implements RiskManageInterface{

	private final String modelId = "IqpLoanApp";//业务申请表
	private static final Logger logger = Logger.getLogger(RiskManageIqpComIndusImple.class);
	/*行业授信额度及限额检查
	 * 1.如果当前业务”管理机构+业务担保方式”在行业授权管理中有设置授信额，则检查：当前业务风险敞口金额 <= 单户授权金额；
     * 2.检查当前业务风险敞口金额 <= 行业授信单户限额
     * 3.检查当前业务风险敞口金额 + 行业授信名单下所有客户的存量业务风险敞口金额 <= 行业授信总额。
	 */
		...
		try {
			...
			if(limit_ind!=null&&!limit_ind.equals("")){
				if(limit_ind.equals("4")||limit_ind.equals("5")||limit_ind.equals("6")){
					...
					BigDecimal indus_lmt_amt = new BigDecimal(lmtKColl.getDataValue("lmt_amt")+"");//授信占用
					...
					IndexedCollection iqpContIColl =  SqlClient.queryList4IColl("queryIqpContByIndusCusId", kColl4Query, conn);//存量业务（已包含了当前申请）
					BigDecimal induscus_lmt_amt = new BigDecimal("0");//单户的存量业务风险敞口金额
					for(int i=0;i<iqpContIColl.size();i++){
						...
						if(cont_no==null){
							...
							induscus_lmt_amt = induscus_lmt_amt.add(iqp_amt);
						}else{
					...
					inMap.put("IN_INDUS_LMT_AMT", indus_lmt_amt+"");
					inMap.put("IN_INDUSCUS_LMT_AMT", induscus_lmt_amt+"");
					Map<String, String> outMap = new HashMap<String, String>();
					outMap=shuffleService.fireTargetRule("IQPCOMRULE", "IQPCOMRULE_5", inMap);
			...
			logger.error("行业授信额度及限额检查失败！"+e.getMessage());
