...
public class RiskManageIqpSigleImple implements RiskManageInterface{
		...
		try {
	    		...
	    		if(limit_code!=null && !"".equals(limit_code)){
					...
					//查询授信启用金额-冻结金额
						...
						if(iColl.size()>0 ){
							...
							/**  总部风险部、总行行领导可对授信做临时冻结、解冻     需求编号：XD140630018     2014-08-01    唐顺岩 	 */
	    	...
	    	if(lmt_canAmt_total.compareTo(lmt_amt_total)>=0){
		    	...
		    	if("2".equals(limit_ind) || "3".equals(limit_ind) || "5".equals(limit_ind) || "6".equals(limit_ind)){
					...
					//limit_acc_no = (String)kColl.getDataValue("limit_acc_no");
