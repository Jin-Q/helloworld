 ...
 *
 */
public class GetInstanceInfoOp extends CMISOperation {
	...
	public String doExecute(Context context) throws EMPException {
		...
		try {
			...
			String wfSign = instanceVO.getWfSign();//流程标识
			...
			if(instanceVO.getCommentList()!=null&&instanceVO.getCommentList().size()>0&&instanceVO.getCommentList().get(0).getCommentContent()!=null){
				String commentid = wfiComponent1.getCurrentCommId(instanceId, currentUserId, connection, nodeId);
				//String commentid1 = instanceVO.getCommentList().get(0).getCommentContent();
			...
			context.put("commentContent", commentcontent);
...
//			WfiWorkflow2biz wf2bizConf = wfiComponent.getWf2bizConf(applType, nodeId, sceneScope);
			WfiWorkflow2biz wf2bizConf = wfiComponent.getWf2bizConfBySign(applType, wfSign, nodeId, sceneScope);
			...
			if(bizUrl!=null && bizUrl.trim().length()>0) {
				...
				if(icoll.size()>0){
					...
					condition = "WHERE INSTANCEID='"+instanceId+"' AND COMMENTID IN (SELECT COMMENTID FROM (SELECT COMMENTID FROM WFI_BIZ_VAR_RECORD WHERE INSTANCEID='"+instanceId+"' ORDER BY OP_TIME DESC) WHERE rownum=1)";
					icoll = dao.queryList("WfiBizVarRecord", condition, connection);
					for(int i=0;i<icoll.size();i++){
						KeyedCollection kCollTmp = (KeyedCollection)icoll.get(i);
						String varKey = (String)kCollTmp.getDataValue("var_key");
						varRecordKcoll.put(varKey+"_old", kCollTmp.getDataValue("var_value"));
					}
				}
				
//				String condition = "WHERE INSTANCEID='"+instanceId+"' AND NODEID IN (SELECT NODEID FROM (SELECT NODEID FROM WFI_BIZ_VAR_RECORD WHERE INSTANCEID='"+instanceId+"' ORDER BY OP_TIME DESC) WHERE rownum=1)";
//				IndexedCollection icoll = dao.queryList("WfiBizVarRecord", condition, connection);
//				KeyedCollection varRecordKcoll = new KeyedCollection("WfiBizVarRecord");
//				for(int i=0;i<icoll.size();i++){
//					KeyedCollection kCollTmp = (KeyedCollection)icoll.get(i);
...
//					varRecordKcoll.put(varKey+"_old", kCollTmp.getDataValue("var_value"));
//				}
//				condition = "WHERE INSTANCEID='"+instanceId+"' AND NODEID='"+nodeId+"' AND INPUT_ID='"+currentUserId+"'";
//				if(icoll.size()>0){
//					for(int i=0; i<icoll.size(); i++) {
//						KeyedCollection kcoll = (KeyedCollection) icoll.get(i);
//						String varKey = (String) kcoll.getDataValue("var_key");
...
//			preventList = wf2bizConf.getPreventList(); //风险拦截列表
			...
			/**从风险拦截配置中查找当前节点是否有配置风险拦截 add by tangzf 2013.11.30 start*/
			String conditionBiz = " where APPL_TYPE='"+applType+"' and WFSIGN='"+instanceVO.getWfSign()+"'";
			IndexedCollection iCollBiz = dao.queryList("WfiWorkflow2biz", conditionBiz, connection);
			if(iCollBiz.size()>0){
				KeyedCollection kCollBiz = (KeyedCollection)iCollBiz.get(0);
				String preventListTmp = (String)kCollBiz.getDataValue("prevent_list");//流程关联业务中配置风险拦截方案
				String conditionStr = " where SCENE_ID='"+nodeId+"' and WFID='"+instanceVO.getWfSign()+"' and PREVENT_ID='"+preventListTmp+"'";
				IndexedCollection iColl = dao.queryList("PrdPvRiskScene", conditionStr, connection);
				if(iColl.size()>0){//在风险拦截配置中能查到记录
					preventList = preventListTmp;
				}
			}
			/**从风险拦截配置中查找当前节点是否有配置风险拦截 add by tangzf 2013.11.30 end*/
			...
			context.put("wfiStatus", wfiStatus);
			...
			//审批中变更特殊处理标志（单一法人授信（申请、复议）、个人授信（申请、复议））
			if("003".equals(applType)||"3281".equals(applType)||"0061".equals(applType)||"0062".equals(applType)){
				context.put("isSpecial", "y");
			}
			//判断是否是发起节点
			String preNodeId = instanceVO.getPreNodeId();
			if(preNodeId!=null&&!"".equals(preNodeId)){
				boolean isFirstNode = (Boolean) wfi.getWFNodeProperty(preNodeId, "isFirstNode");
				context.put("isFirstNode", isFirstNode);
			}
		} catch (Exception e) {
