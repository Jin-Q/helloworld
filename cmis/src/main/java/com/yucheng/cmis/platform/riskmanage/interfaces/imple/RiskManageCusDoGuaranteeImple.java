...
*@author lisj
*@time 2014-12-24
*@description 需求编号：【XD141029074】增加授信企业关联担保提醒事项（风险拦截类）
*@version v1.0
*
 */
public class RiskManageCusDoGuaranteeImple implements RiskManageInterface {
	   ...
	   // String conditionStr = "";
		...
		try {
			...
			//  .getComponentInstance(AppConstant.IQPLOANAPPCOMPONENT, context, conn);
			 KeyedCollection kCollApp = dao.queryDetail(tableName, serno, connection);
			 String cus_id = (String)kCollApp.getDataValue("cus_id");
			 //获取该授信企业为他人担保的担保合同编号列表
			 IndexedCollection GuarContNo4CusDoList = SqlClient.queryList4IColl("queryGuarContNo4CusDoByCusId", cus_id, conn);
			 ...
			 if(GuarContNo4CusDoList!=null && GuarContNo4CusDoList.size()>0){
			  for(int i=0; i < GuarContNo4CusDoList.size(); i++){
				  ...
				  guar_cont_no = (String) temp.getDataValue("guar_cont_no");//该企业为他人担保的担保合同编号
				...
				IndexedCollection iColl =  dao.queryList("GrtLoanRGur", conditionStr, conn);
				 ...
				 if(used_amt!=null && guar_cont_state !=null &&  guar_cont_state.equals("01")){
					 returnFlag ="不通过";
					 returnInfo += "该企业已在我行为["+borrower_name+"]提供担保,担保金额["+cur_type+used_amt+"];";
			  ...
			  if("".equals(returnFlag)){
		    	  returnFlag ="通过";
		    	  returnInfo ="该企业在我行未存在为他人担保情况，检查通过！";
			  }
			 }else{
			    	 returnFlag ="通过";
			    	 returnInfo ="该企业在我行未存在为他人担保情况，检查通过！";
