...
public class RiskManageIqpChangeImple implements RiskManageInterface{

	/*信用证/保函修改校验 担保合同金额能否覆盖敞口金额
	 */
				...
				if("100".equals(new_assure_main) || "200".equals(new_assure_main) || "210".equals(new_assure_main) || "220".equals(new_assure_main) || "300".equals(new_assure_main)){
					...
					if(res_value.size()>0){
						...
						if("IqpCreditChangeApp".equals(tableName)){
							BigDecimal new_apply_amt = BigDecimalUtil.replaceNull(kColl.getDataValue("new_apply_amt"));//修改后信用证金额
							BigDecimal new_floodact_perc = BigDecimalUtil.replaceNull(kColl.getDataValue("new_floodact_perc"));//修改后溢装比例
							BigDecimal new_security_rate = BigDecimalUtil.replaceNull(kColl.getDataValue("new_security_rate"));//修改后保证金比例
							...
							//risk_open_amt = ((new_apply_amt.multiply(new BigDecimal(1).add(new_floodact_perc)).multiply(new BigDecimal(1).subtract(new_security_rate))).multiply(exchange_rate));
							BigDecimal securityAmt = new_apply_amt.multiply(new BigDecimal(1).add(new_floodact_perc)).multiply(new_security_rate).multiply(exchange_rate).divide(exchange_rate_security,2,BigDecimal.ROUND_HALF_EVEN);
							...
							risk_open_amt = (new_apply_amt.multiply(new BigDecimal(1).add(new_floodact_perc)).multiply(exchange_rate).subtract(securityAmt.multiply(exchange_rate_security)));
							
							risk_open_amt = iqpLoanAppComponent.caculateLimitSpac(oldSerno, risk_open_amt);
						}else if("IqpGuarantChangeApp".equals(tableName)){
							BigDecimal new_cont_amt = BigDecimalUtil.replaceNull(kColl.getDataValue("new_cont_amt"));//修改后保函金额
							BigDecimal new_security_rate = BigDecimalUtil.replaceNull(kColl.getDataValue("new_security_rate"));//修改后保证金比例
							String cont_cur_type = (String)kColl.getDataValue("cont_cur_type");//币种
							//获取实时汇率  start
							String security_cur_type = (String) kColl.getDataValue("security_cur_type");//保证金币种
							...
							BigDecimal securityAmt = new_cont_amt.multiply(new_security_rate).multiply(exchange_rate).divide(exchange_rate_security,2,BigDecimal.ROUND_HALF_EVEN);
							java.text.NumberFormat nf = java.text.NumberFormat.getInstance();
							nf.setGroupingUsed(false);
							String caculateAmt = String.valueOf(securityAmt);
							securityAmt = BigDecimalUtil.replaceNull(Math.ceil(Double.valueOf(caculateAmt)/100)*100);
							String changeAmt = nf.format(securityAmt);
							securityAmt = BigDecimalUtil.replaceNull(changeAmt);
							risk_open_amt = ((new_cont_amt.multiply(exchange_rate).subtract(securityAmt.multiply(exchange_rate_security))));
							risk_open_amt = iqpLoanAppComponent.caculateLimitSpac(oldSerno, risk_open_amt);
						}
