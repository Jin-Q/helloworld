...
public class GetPrdPvRiskSceneChangeUpdatePageOp extends CMISOperation {
	...
	public String doExecute(Context context) throws EMPException {
		...
		try{
			...
			if(wfid == null || wfid.trim().length() == 0){
				throw new EMPException("获取流程编号失败！");
			}
			/** 将流程字典转换为流程标识 */
			...
			String conndition = " where prevent_id='"+prevent_id+"' and wfid='"+wfSign+"' ";
			String scene_id = "";
			if(context.containsKey("scene_id")){
				scene_id = (String)context.getDataValue("scene_id");
			}
			if(scene_id != null && scene_id.trim().length() > 0){
				/** 存在流程节点ID，则为点击节点出发的操作 */
				conndition += " and scene_id = '"+scene_id+"'";
			}else {
				/** 取出配置的流程节点，任意取出一条节点为例 */
				conndition += " and scene_id is null ";
			}
			...
			retrunKColl.addDataField("scene_id", scene_id);
