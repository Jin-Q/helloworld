...
public class RiskManageIqpChangeLmtCheckImple implements RiskManageInterface{
	...
	public Map<String, String> getResultMap(String tableName, String serno,Context context,Connection connection) throws Exception {
		...
		try {
			...
			KeyedCollection kCollRBusLmt = dao.queryFirst("RBusLmtInfo",null," WHERE CONT_NO='"+kCollCtr.getDataValue("cont_no")+"'" , connection);
			limit_acc_no = (String)kCollRBusLmt.getDataValue("agr_no");   //获取授信额度编号-自有
			/** END */
						
			if(limit_acc_no != null && !"".equals(limit_acc_no)){
				String aaa = "'"+limit_acc_no+"'";
	    	...
	    	//扣减修改前业务的授信占用，加上修改后的授信占用
	    	if("2".equals(limit_ind) || "3".equals(limit_ind) || "5".equals(limit_ind) || "6".equals(limit_ind)){
	    		lmt_amt_total = lmt_amt_total.subtract(lmt_amt_old).add(lmt_amt_new);
			}
