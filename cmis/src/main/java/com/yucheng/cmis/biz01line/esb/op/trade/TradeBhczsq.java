 ...
 *
 */
public class TradeBhczsq extends TranClient {
	...
	public CompositeData doExecute(Context context, Connection connection)
		...
		try{
			...
			/** 系统头 */
			reqCD.addStruct("SYS_HEAD", esbInterfacesImple.getSysHeadCD(serviceCode, TradeConstance.SERVICE_SCENE_BHFFSQ, TradeConstance.CONSUMERID, serno,(String)context.getDataValue(CMISConstance.ATTR_CURRENTUSERID),(String)context.getDataValue(CMISConstance.ATTR_CURRENTUSERNAME), (String)context.getDataValue(CMISConstance.ATTR_ORGID), TradeConstance.TRADE_SYSTEM, context));
			/** 应用头 */
...
//			String lmPrdId = service.getPrdBasicCLPM2LM(authKColl.getDataValue("cont_no").toString(), authKColl.getDataValue("prd_id").toString(), context, connection);
//			/** 核算与信贷业务品种映射 END */
			...
			bodyCD.addField("GT_NAME", TagUtil.getEMPField(reflectKColl.getDataValue("GT_NAME"), FieldType.FIELD_STRING, 60, 0));
			...
			bodyCD.addField("GT_KIND", TagUtil.getEMPField(reflectKColl.getDataValue("GT_KIND"), FieldType.FIELD_STRING, 10, 0));
			bodyCD.addField("OPEN_TYPE", TagUtil.getEMPField(reflectKColl.getDataValue("OPEN_TYPE"), FieldType.FIELD_STRING, 5, 0));
			...
			bodyCD.addField("OPEN_DATE", TagUtil.getEMPField(TagUtil.replaceNull4Int(reflectKColl.getDataValue("OPEN_DATE")), FieldType.FIELD_STRING, 8, 0));
			bodyCD.addField("EXPIRY_DATE", TagUtil.getEMPField(TagUtil.replaceNull4String(reflectKColl.getDataValue("EXPIRY_DATE")), FieldType.FIELD_STRING, 8, 0));
			bodyCD.addField("BENEFIT_ORG_NAME", TagUtil.getEMPField(TagUtil.replaceNull4String(reflectKColl.getDataValue("BENEFIT_ORG_NAME")), FieldType.FIELD_STRING, 150, 0));
