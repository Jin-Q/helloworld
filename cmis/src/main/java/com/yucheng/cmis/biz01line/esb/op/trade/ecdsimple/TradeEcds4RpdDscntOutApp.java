 ...
 * @author yangzy
 * 说明：
 * 接收电票系统发送的转贴现卖出票据明细，生成转贴现卖出业务明细
 * 
 */
public class TradeEcds4RpdDscntOutApp extends TranService {
	...
	public KeyedCollection doExecute(CompositeData CD, Connection connection)
		...
		try {
			...
			String check_RAN = "2";
			//3003：买入返售到期转贴现
			if(PRODUCT_TYPE!=null&&"3001".equals(PRODUCT_TYPE)){//转贴现卖断
				biz_type = "03";//卖出卖断
				prd_id = "300023";//外部转贴现300023
			}else if(PRODUCT_TYPE!=null&&"3002".equals(PRODUCT_TYPE)){//卖出回购式转贴现
				biz_type = "04";//卖出回购
				prd_id = "300023";//外部转贴现300023
				check_RAN = "1";
			}else if(PRODUCT_TYPE!=null&&"3003".equals(PRODUCT_TYPE)){//3003：买入返售到期转贴现
				biz_type = "02";//买入返售
				prd_id = "300023";//外部转贴现300023
			}else if(PRODUCT_TYPE!=null&&"4001".equals(PRODUCT_TYPE)){//卖断式再贴现
				biz_type = "06";//再贴现
				prd_id = "300024";//再贴现300024
			}else if(PRODUCT_TYPE!=null&&"4002".equals(PRODUCT_TYPE)){//卖出回购式再贴现
				biz_type = "06";//再贴现
				prd_id = "300024";//再贴现300024
			}
			String bill_type = "";
			...
			if("success".equals(flag)){
				if(bill_array!=null&&bill_array.size()>0){
					...
					if("success".equals(flag)){
						...
						EMPLog.log("TradeEcds4RpdDscntOutApp", EMPLog.INFO, 0, "【电子商业汇票转贴现卖出申请】交易处理完成...", null);
					}else{
						if("error1".equals(flag)){
							retKColl.setDataValue("ret_code", "9999");
							retKColl.setDataValue("ret_msg", "【电子商业汇票转贴现卖出申请】,业务处理失败,票据批次币种为空/不一致！");
						}else if("error2".equals(flag)){
							retKColl.setDataValue("ret_code", "9999");
							retKColl.setDataValue("ret_msg", "【电子商业汇票转贴现卖出申请】,业务处理失败,票据贴现赎回开放日为空/不一致！");
						}else if("error3".equals(flag)){
							retKColl.setDataValue("ret_code", "9999");
							retKColl.setDataValue("ret_msg", "【电子商业汇票转贴现卖出申请】,业务处理失败,票据批次转贴现利率为空/不一致！");
						}
					}
				}
			}else{
				retKColl.setDataValue("ret_code", "9999");
				retKColl.setDataValue("ret_msg", "【电子商业汇票转贴现卖出申请】,业务处理失败,责任人不在责任机构下！");
			}
		} catch (Exception e) {
			retKColl.setDataValue("ret_code", "9999");
			retKColl.setDataValue("ret_msg", "【电子商业汇票转贴现卖出申请】,业务处理失败！");
			e.printStackTrace();
		}
		return retKColl;
	}
