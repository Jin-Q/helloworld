 ...
 * @create 2013-07-10
 */
public class PrdPvpMapPopOp extends CMISOperation {
	private static final String APPFORMDIC = "PRD_APP_DIC_TYPE";//构造申请表单字典项
	private static final String CONTFORMDIC = "PRD_PVP_DIC_TYPE";//构造出账表单字典项 
	@Override
	public String doExecute(Context context) throws EMPException {
		...
		try {
			...
			pvpform = (String)context.getDataValue("pvpform");
			...
			TableModel contFormModel = modelLoader.getTableModel(pvpform);
			if(appFormModel == null){
				throw new EMPException("表模型"+appForm+"未配置！");
			}
			if(contFormModel == null){
				throw new EMPException("表模型"+pvpform+"未配置！");
			}
			/** 构造申请表单字典项 */
			IndexedCollection appDicIColl = new IndexedCollection(APPFORMDIC); 
			this.addModelFiledDic(appForm, appDicIColl, context);
			/** 构造出账表单字典项 */
			IndexedCollection pvpDicIColl = new IndexedCollection(CONTFORMDIC);
			this.addModelFiledDic(pvpform, pvpDicIColl, context);
			
			this.putDataElement2Context(appDicIColl, context);
			this.putDataElement2Context(pvpDicIColl, context);
			...
			pvpmapping = (String)kColl.getDataValue("pvpmapping");
			
			if(pvpmapping != null && pvpmapping.trim().length() != 0){
				IndexedCollection iColMap = this.changeMAPXML2Option(pvpmapping, appDicIColl, pvpDicIColl, iColSysVar);
	...
	public void addModelFiledDic(String modelId,IndexedCollection dicIColl,Context context) throws Exception{
		...
		/**
		 * 获取主表下子表的所有字段，放入dicIColl，子表显示模式为【字段名+【从】】用于标识主子、主从表
		 */
		...
		while(iterator.hasNext()){
			...
			while(itSon.hasNext()){
				...
				kFiled.addDataField("cnname", field.getCnname()+"[从"+sonFlag+"]");
				dicIColl.addDataElement(kFiled);
			}
			sonFlag++;
		}
