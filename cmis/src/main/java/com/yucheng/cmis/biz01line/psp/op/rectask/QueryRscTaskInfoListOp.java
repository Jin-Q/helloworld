...
public class QueryRscTaskInfoListOp  extends CMISOperation {
	...
	public String doExecute(Context context) throws EMPException {
		...
		try {
			...
			String  inputBrId = "";
			try {
				inputBrId = (String) queryData.getDataValue("input_br_id");
			} catch (Exception e) {
			}
			context.put("input_br_id", inputBrId);
			//定义查询参数map
			...
			if(NewStringUtils.isBlank(usr_cde)){
				usr_cde = (String)context.getDataValue(CMISConstance.ATTR_CURRENTUSERID);
			}
            if(NewStringUtils.isBlank(bch_cde)){
            	bch_cde = (String)context.getDataValue(CMISConstance.ATTR_ORGID);
			}
            if(NewStringUtils.isNotBlank(inputBrId)){
            	paramMap.put("input_br_id", inputBrId);
			}
