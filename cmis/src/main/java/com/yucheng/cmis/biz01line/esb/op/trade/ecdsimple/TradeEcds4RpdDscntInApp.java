 ...
 * @author yangzy
 * 说明：
 * 接收电票系统发送的转贴现买入票据明细，生成转贴现买入业务明细
 * 
 */
public class TradeEcds4RpdDscntInApp extends TranService {
	...
	public KeyedCollection doExecute(CompositeData CD, Connection connection)
		...
		try {
			...
			String DISCOUNTER_BANK_NAME = reqBody.getDataValue("DISCOUNTER_BANK_NAME").toString().trim();                            //贴出行名称      
			String DISCOUNTER_BANK_BRANCH_ID = reqBody.getDataValue("DISCOUNTER_BANK_BRANCH_ID").toString().trim();                  //贴出行行号      
			String DUTY_PERSON = reqBody.getDataValue("DUTY_PERSON").toString().trim();                                              //责任人          
			String PRODUCT_TYPE = reqBody.getDataValue("PRODUCT_TYPE").toString().trim();                                            //产品类型        
			Double DISCOUNT_RAN_RATE = Double.valueOf(reqBody.getDataValue("DISCOUNT_RAN_RATE").toString().trim());                  //贴现赎回利率 
			...
			//2003：卖出回购到期转贴现
			//5001：卖出回购到期再贴现
			if(PRODUCT_TYPE!=null&&"2001".equals(PRODUCT_TYPE)){//转贴现买断
				biz_type = "01";//买入买断
				prd_id = "300023";//外部转贴现300023
			}else if(PRODUCT_TYPE!=null&&"2002".equals(PRODUCT_TYPE)){//买入返售式转贴现
				biz_type = "02";//买入返售
				prd_id = "300023";//外部转贴现300023
				check_RAN = "1";
			}else if(PRODUCT_TYPE!=null&&"2003".equals(PRODUCT_TYPE)){//卖出回购到期转贴现
				biz_type = "04";//卖出回购
				prd_id = "300023";//外部转贴现300023
				check_RAN = "1";
			}else if(PRODUCT_TYPE!=null&&"5001".equals(PRODUCT_TYPE)){//卖出回购到期再贴现
				biz_type = "06";//再贴现
				prd_id = "300024";//再贴现300024
			}else if(PRODUCT_TYPE!=null&&"5002".equals(PRODUCT_TYPE)){//买入央行卖出
				biz_type = "06";//再贴现
				prd_id = "300024";//再贴现300024
			}
			String bill_type = "";
			...
			if("success".equals(flag)){
				if(bill_array!=null&&bill_array.size()>0){
					...
					for (int j=0;j<bill_array.size();j++){
						...
						if(kColl_bill_array_j.getDataValue("DISCOUNT_RATE")==null || "".equals(kColl_bill_array_j.getDataValue("DISCOUNT_RATE").toString().trim()) || BigDecimal.valueOf(Double.valueOf(RATE)).compareTo(BigDecimal.valueOf(Double.valueOf(kColl_bill_array_j.getDataValue("DISCOUNT_RATE").toString().trim())))>0){
							flag = "error3";
						}
						if("1".equals(check_RAN)){
							if("".equals(DISCOUNT_RAN_DATE)){
								DISCOUNT_RAN_DATE = kColl_bill_array_j.getDataValue("DISCOUNT_RAN_OPEN_DATE").toString().trim();
								DISCOUNT_END_DATE = kColl_bill_array_j.getDataValue("DISCOUNT_RAN_END_DATE").toString().trim();
							}
							if(kColl_bill_array_j.getDataValue("DISCOUNT_RAN_OPEN_DATE")==null||"".equals(kColl_bill_array_j.getDataValue("DISCOUNT_RAN_OPEN_DATE").toString().trim()) || !DISCOUNT_RAN_DATE.equals(kColl_bill_array_j.getDataValue("DISCOUNT_RAN_OPEN_DATE").toString().trim())){
								flag = "error2";
							}
						}
					}
					if("success".equals(flag)){
						for (int i=0;i<bill_array.size();i++){
							...
							if("1".equals(check_RAN)){
								kColl_bill_income.put("fore_rebuy_date", DISCOUNT_RAN_END_DATE);//预计回购日期 
								SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
								Date expiryDateHelp = sdf.parse(BILL_EXPIRY_DATE);
								Date rebuyDateHelp = sdf.parse(DISCOUNT_RAN_OPEN_DATE);
								long rebuyNum = (expiryDateHelp.getTime()-rebuyDateHelp.getTime())/(24L*60L*60L*1000L);
								if(rebuyNum <= 0){
									rebuyNum = 0;
								}
								//kColl_bill_income.put("rebuy_days", rebuyNum);                    //回购天数(回购日-贴现日)
								double rebuyInt = BILL_AMT*Double.parseDouble(String.valueOf(rebuyNum))*DISCOUNT_RAN_RATE/360;
								rebuy_int += rebuyInt;
								//kColl_bill_income.put("rebuy_int", rebuyInt);                     //回购利息
							}
						...
						if("1".equals(check_RAN)){
							kColl_batch_mng.put("rebuy_rate", DISCOUNT_RAN_RATE);	     //回购利率
							kColl_batch_mng.put("rebuy_int", rebuy_int);	             //回购利息		
							kColl_batch_mng.put("rebuy_date", DISCOUNT_END_DATE);	     //回购日期	
						}
						...
						kColl_app.put("rebuy_date", DISCOUNT_RAN_DATE);         //回购日期
						kColl_app.put("rebuy_rate", DISCOUNT_RAN_RATE);         //回购利率
						kColl_app.put("rebuy_int", rebuy_int);                  //总回购利息
						...
						EMPLog.log("TradeEcds4RpdDscntInApp", EMPLog.INFO, 0, "【电子商业汇票转贴现买入申请】交易处理完成...", null);
					}else{
						if("error1".equals(flag)){
							retKColl.setDataValue("ret_code", "9999");
							retKColl.setDataValue("ret_msg", "【电子商业汇票转贴现买入申请】,业务处理失败,票据批次币种为空/不一致！");
						}else if("error2".equals(flag)){
							retKColl.setDataValue("ret_code", "9999");
							retKColl.setDataValue("ret_msg", "【电子商业汇票转贴现买入申请】,业务处理失败,票据贴现赎回开放日为空/不一致！");
						}else if("error3".equals(flag)){
							retKColl.setDataValue("ret_code", "9999");
							retKColl.setDataValue("ret_msg", "【电子商业汇票转贴现买入申请】,业务处理失败,票据批次转贴现利率为空/不一致！");
						}
					}
				}
			}else{
				retKColl.setDataValue("ret_code", "9999");
				retKColl.setDataValue("ret_msg", "【电子商业汇票转贴现买入申请】,业务处理失败,责任人不在责任机构下！");
			}
		} catch (Exception e) {
			retKColl.setDataValue("ret_code", "9999");
			retKColl.setDataValue("ret_msg", "【电子商业汇票转贴现买入申请】,业务处理失败！");
			e.printStackTrace();
		}
		return retKColl;
	}
