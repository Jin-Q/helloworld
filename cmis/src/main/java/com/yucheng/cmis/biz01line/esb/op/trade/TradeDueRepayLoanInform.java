 ...
 */
public class TradeDueRepayLoanInform extends TranService {
	...
	public KeyedCollection doExecute(CompositeData CD, Connection connection) throws Exception {
		...
		try {
			...
			EMPLog.log("TradeDueRepayLoanInform", EMPLog.INFO, 0, "【贷款逾期还款通知】交易处理完成...", null);
		} catch (Exception e) {
			retKColl.setDataValue("ret_code", "999999");
			retKColl.setDataValue("ret_msg", "贷款逾期还款通知 ,业务处理失败！借据号为："+DUEBILL_NO);
			e.printStackTrace();
			EMPLog.log("TradeDueRepayLoanInform", EMPLog.ERROR, 0, "【贷款逾期还款通知】交易处理失败，借据号为："+DUEBILL_NO+"异常信息为："+e.getMessage(), null);
		}
		return retKColl;
	}
