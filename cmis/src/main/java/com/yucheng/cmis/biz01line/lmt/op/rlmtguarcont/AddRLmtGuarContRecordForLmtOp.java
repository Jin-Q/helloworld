...
public class AddRLmtGuarContRecordForLmtOp extends CMISOperation {
	...
	public String doExecute(Context context) throws EMPException {
		...
		try{
			...
			//校验是否已被引入
			//String condition_j  = "where guar_cont_no='"+kColl.getDataValue("guar_cont_no")+"' and corre_rel <> '3' "; 
			//IndexedCollection iColl_j = (IndexedCollection)dao.queryList(modelId,condition_j, connection);
			//if(iColl_j!=null&&iColl_j.size()>0){
			//	flag = "该担保合同已被引用!";
			//}
			/**担保等级默认赋值，大小为已有等级+1   --Start--*/ 
			if("success".equals(flag)){
				String condition  = "where limit_code='"+kColl.getDataValue("limit_code")+"' and serno = '"+kColl.getDataValue("serno")+"'"; 
