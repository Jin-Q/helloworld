...
public class QueryArpLawLawsuitInfoListOp extends CMISOperation {
	...
	public String doExecute(Context context) throws EMPException {
		...
		try{
			...
			if(conditionStr.indexOf("WHERE") != -1){				
				conditionStr = conditionStr + conditionMember + " order by case_no desc";
			}else{
				conditionStr = "where 1=1 " + conditionMember + " order by case_no desc";
			}
	...
	public void addNameList(IndexedCollection iColl,TableModelDAO dao ,Connection connection ,Context context ,String type) throws EMPException {
		String case_no = "",Member_Name="" ,Member_Condition ="";
		int i = 0,j = 0;
		for (i = 0; i < iColl.size(); i++) {
			KeyedCollection kColl = (KeyedCollection) iColl.get(i);
			case_no = kColl.getDataValue("case_no").toString();
			
			/*** 取最先录入的诉讼人员，作为首页展示 ***/
			if(type.equals("001")){	//被告人
				Member_Condition  = " where member_type = '001' and  case_no = '"+case_no+"'  and rownum = 1 order by pk_serno asc";
			}else{	//债务人
				Member_Condition  = " where member_type = '002' and  case_no = '"+case_no+"'  and rownum = 1 order by pk_serno asc";
			}
			
			List<String> list = new ArrayList<String>();
			list.add("cus_id");
			list.add("pk_serno");
			IndexedCollection Member_iColl = dao.queryList("ArpLawMemberMana",list ,Member_Condition,connection);
			...
			for  (j = 0; j < Member_iColl.size(); j++) {
				...
				Member_Name = Member_kColl.getDataValue("cus_name").toString();
			}
	...
	public String queryNameList(KeyedCollection kColl,TableModelDAO dao ,Connection connection ,Context context,String type) throws EMPException {
		String case_no = "", condition = "", value = "";
		if(kColl != null){
			if(type.equals("001")){
			...
			if(!value.equals("")){
				...
				IndexedCollection defendant_iColl = dao.queryList("ArpLawMemberMana", condition, connection);				
				for( int j = 0; j < defendant_iColl.size(); j++) {
					KeyedCollection Member_kColl = (KeyedCollection) defendant_iColl.get(j);
					case_no = case_no +Member_kColl.getDataValue("case_no").toString()+ "','";
				}			
				case_no = "and case_no in ("+case_no +"')";
			}
		}
		return case_no;
	}
