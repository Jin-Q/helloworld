...
public class QueryIqpNetMagInfoPopOp extends CMISOperation {
		...
		try{
			...
			if(context.containsKey("cus_id")){
				...
				if("".equals(net_agr_no_str)){
					...
					net_agr_no_str = net_agr_no_str.substring(0,net_agr_no_str.length()-1);
				}
				...
				if("".equals(conditionStr)){
					conditionStr = "where status='1' order by net_agr_no desc";
				}else{
