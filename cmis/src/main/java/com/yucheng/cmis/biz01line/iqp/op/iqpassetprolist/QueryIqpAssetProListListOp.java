...
public class QueryIqpAssetProListListOp extends CMISOperation {
	...
	public String doExecute(Context context) throws EMPException {
		...
		try{
			...
			for(int i=0;i<iColl.size();i++){
				...
				/*从贷款台账表中获取贷款余额、到期日期 ，   2014-08-20  王青 start*/
				String loan_balance = (String)kCollIqpAsset.getDataValue("loan_balance");
				String end_date = (String)kCollIqpAsset.getDataValue("end_date");
				kColl.put("cus_id", cus_id);
				kColl.put("loan_balance", loan_balance);
				kColl.put("end_date", end_date);
				/*从贷款台账表中获取贷款余额、到期日期 ，   2014-08-20  王青 end*/
