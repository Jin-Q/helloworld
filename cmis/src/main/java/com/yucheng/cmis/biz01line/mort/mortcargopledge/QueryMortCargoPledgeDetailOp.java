...
public class QueryMortCargoPledgeDetailOp  extends CMISOperation {
...
//			}
			...
			String storage_date = null;
			String exware_date = null;
			try {
			...
			if(context.containsKey("exware_date")){
				exware_date = (String)context.getDataValue("exware_date");
			}
			...
			if(context.containsKey("action")){
				...
				if("zh".equals(action)){
					...
					storage_date = (String)kColl.getDataValue("storage_date");
					kColl.put("exware_date", exware_date);
				}
			}else{
				kColl = dao.queryDetail(modelId, cargo_id_value, connection);
				storage_date = (String)kColl.getDataValue("storage_date");
			...
			if(context.containsKey("serno")){
				...
				kColl.put("storage_date", storage_date);
				kColl.put("exware_date", exware_date);
			}
			//翻译产地、销售区域、押品所属目录
