...
public class QueryLmtIntbankPop4CusOp extends CMISOperation {
	...
	public String doExecute(Context context) throws EMPException {
		...
		try{
			...
			try {
				...
				throw new Exception("传入的客户码为空，请联系后台管理员!");
			}
			...
			for(int i=0;i<iColl.size();i++){
					...
					String cus_id_select = (String)kColl.getDataValue("cus_id");					
					CMISModualServiceFactory serviceJndi = CMISModualServiceFactory.getInstance();
					CusServiceInterface service = (CusServiceInterface)serviceJndi.getModualServiceById("cusServices", "cus");
					KeyedCollection kColl_cus =service.getCusSameOrgKcoll(cus_id_select, context, connection);
