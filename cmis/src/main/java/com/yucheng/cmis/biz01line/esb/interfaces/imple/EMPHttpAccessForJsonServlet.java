...
import com.yucheng.cmis.biz01line.esb.op.trade.TradeOnlineCredit;
...
public class EMPHttpAccessForJsonServlet extends HttpServlet {
	private static final Logger logger = Logger.getLogger(EMPHttpAccessForJsonServlet.class);
	...
	public void service(HttpServletRequest request, HttpServletResponse response)
  {
    ...
    try
    {
      ...
      this.httpRequestHandler.parseRequest(request); //解析请求获取reqData
      ...
      String retMsg = deal.exec(jsonObj); //调用实际处理类，生成响应报文

      this.httpRequestHandler.handleResponse(request, response, retMsg, 
        reqURI, sessionId); //返回响应报文
    }
    catch (Exception e)
    {
