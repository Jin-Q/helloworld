...
public class CheckIqpAssetRegiAppOp extends CMISOperation {

	private final String modelId = "IqpAssetRegiApp";
	...
	public String doExecute(Context context) throws EMPException {
		...
		try{
			...
			if(iColl.size()==0){
				KeyedCollection kColl = dao.queryDetail("AccLoan", bill_no, connection);
				...
				context.addDataField("flag", "error");
				context.addDataField("msg", "该笔借据已做过信贷资产登记!");
			}
			
		}catch (EMPException ee) {
			context.addDataField("flag", "error");
			context.addDataField("msg", "该笔借据已做过信贷资产登记，失败原因："+ee.getMessage());
			throw ee;
		} catch(Exception e){
