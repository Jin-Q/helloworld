...
public class GetDthLoanDirectionMainPageOp extends CMISOperation {	
	...
	public String doExecute(Context context) throws EMPException {
		...
		try{
			...
			for(i = 0;i < iColl_data.size() ; i++){
				...
				String multiXML = delLoanDirectionChart(context, kColl_data.getDataValue("enname"), kColl_data.getDataValue("cnname"));
	 ...
	 * 贷款投向数据处理
	 */
	public String delLoanDirectionChart(Context context,Object date_enname,Object date_cnname) throws Exception {
		DthPubAction cmisOp = new DthPubAction();
		KeyedCollection styles = new KeyedCollection();
		/*** flash样式设置 ***/
		styles.addDataField("Caption", date_cnname+"各贷款投向下贷款余额");
		...
		styles.addDataField("jsName", "getLoanDirectionDetails");
		
		KeyedCollection kColl = new KeyedCollection();
		kColl.addDataField("submitType", "getLoanDirectionChart");
