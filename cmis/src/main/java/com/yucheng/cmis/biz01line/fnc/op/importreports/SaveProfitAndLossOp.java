...
public class SaveProfitAndLossOp extends CMISOperation {
		...
		try{
			...
			if(dataList != null){
				for(int i=0;i<dataList.size();++i){
					...
					tempKcoll.setName("IqpMeFncPl");
					dao.update(tempKcoll, connection);
				}
			}
			if(tableInfo != null){
				tableInfo.setName("IqpMeFncPl");
				dao.update(tableInfo, connection);
			}
			context.put("flag", "success");
		}catch (EMPException ee) {
			EMPLog.log(this.getClass().getName(), EMPLog.ERROR, 0, "保存损益表失败！");
			throw ee;
		} catch(Exception e){
			EMPLog.log(this.getClass().getName(), EMPLog.ERROR, 0, "保存损益表失败！");
