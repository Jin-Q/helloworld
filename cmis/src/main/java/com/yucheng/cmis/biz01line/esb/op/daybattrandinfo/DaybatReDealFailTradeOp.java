 ...
 *
 */
public class DaybatReDealFailTradeOp extends CMISOperation {
	...
	public String doExecute(Context context) throws EMPException {
		...
		try {
			...
			try{
				pk1 = context.getDataValue("pk1").toString();
			}catch(Exception e){}
			if (pk1 == null || pk1.length() == 0)
				throw new EMPJDBCException("The value of param[" + pk1
						+ "] cannot be null!");
			TableModelDAO dao = this.getTableModelDAO(context);
			KeyedCollection kColl = (KeyedCollection) dao.queryDetail("DaybatTrandInfo", pk1, connection);
			if(kColl!=null && kColl.getDataValue("pk1")!=null&&!"".equals(kColl.getDataValue("pk1"))){
