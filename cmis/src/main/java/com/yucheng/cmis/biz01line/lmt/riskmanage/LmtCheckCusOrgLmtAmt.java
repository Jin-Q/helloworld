 ...
 *
 */
public class LmtCheckCusOrgLmtAmt implements RiskManageInterface {
	...
	public Map<String, String> getResultMap(String tableName, String serno, Context context, Connection connection) throws Exception {
		...
		parm.put("managerBrId", manager_br_id);
		parm.put("cusId", cus_id);
		IndexedCollection ic = SqlClient.queryList4IColl("getIndivLmtAmtByOrgId", parm, connection);
		if(ic.size()>0){
			KeyedCollection kCollTmp = (KeyedCollection)ic.get(0);
			sigAmtAllTmp = (BigDecimal)kCollTmp.getDataValue("amt_tol");//机构其他金额汇总
		}
		parm.clear();
		parm.put("managerBrId", manager_br_id);
		parm.put("serno", serno);
		//查询在途授信金额
		IndexedCollection icOnWay = SqlClient.queryList4IColl("getIndivLmtWayAmtByOrgId", parm, connection);
		if(icOnWay.size()>0){
		...
		String condition = "WHERE CODE_ID='"+manager_br_id+"'";
		...
		if(iCollQuo.size()>0){
			...
			if(sigAmtAllTmp.compareTo(sigAmtAll)<=0){
				...
				outMap.put("OUT_提示信息", "没有超过该机构授信限额");
			}else{
				outMap.put("OUT_是否通过", "不通过");
				outMap.put("OUT_提示信息", "超过该机构授信限额");
			}
			...
			if("通过".equals(outMap.get("OUT_是否通过"))){
				...
				if(iCollApp.size()>0){
					...
					if(AmtThis.compareTo(sigAmt)>0){
						outMap.put("OUT_是否通过", "不通过");
						outMap.put("OUT_提示信息", "超过该机构单户授信限额");
					}else{
						outMap.put("OUT_是否通过", "通过");
						outMap.put("OUT_提示信息", "没有超过该机构单户授信限额");
					}
				}else{
					//查询该客户原有有效授信协议
			...
			outMap.put("OUT_是否通过", "通过");
			outMap.put("OUT_提示信息", "该机构未设置授信限额");
		}
