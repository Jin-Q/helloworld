...
public class IqpBillDetailListToExcel extends CMISOperation {
	...
	public String doExecute(Context context) throws EMPException {
		...
		try {
			...
			try {
				queryData = (KeyedCollection) context.getDataElement(this.billModel);
			} catch (Exception e) {
			}

			String contditionStr = TableModelUtil.getQueryCondition_bak(billModel, queryData, context, false, true, false);
			RecordRestrict recordRestrict = this.getRecordRestrict(context);
			contditionStr = recordRestrict.judgeQueryRestrict(this.billModel,contditionStr, context, connection);
			...
			IndexedCollection iColl = TableModelUtil.buildPageData(pageInfo,dataSource, sql_select);// dao.queryList(billModel,list
