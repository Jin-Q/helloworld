...
public class LmtGrpCheckGuarContDataValidity implements RiskManageInterface {
	/* add by wangj 2015-6-12 需求编号：XD150407025_2015 分支机构授权配置 begin*/
	...
	/* add by wangj 2015-6-12 需求编号：XD150407025_2015 分支机构授权配置 end*/
	public Map<String, String> getResultMap(String tableName, String serno, Context context, Connection connection) throws Exception {
		...
		IndexedCollection ladIColl = dao.queryList(ladModelId," where SERNO IN (SELECT SERNO FROM LMT_APPLY WHERE GRP_SERNO='" + serno + "' ) and guar_type in ('100','200')  and nvl(crd_amt,0)>0  ", connection);
		if (ladIColl != null && ladIColl.size() > 0) {
			for (int i = 0; i < ladIColl.size(); i++) {
				KeyedCollection kColl = (KeyedCollection) ladIColl.get(i);
				String ladSerno=(String) kColl.getDataValue("serno");
				...
				if ("100".equals(guar_type)) {
					condition = " where limit_code = '" + limit_code+ "' and serno = '" + ladSerno + "' and corre_rel not in ('3','5') and exists (select 1 from grt_guar_cont where grt_guar_cont.guar_cont_no = r_lmt_app_guar_cont.guar_cont_no and grt_guar_cont.guar_way = '00' and grt_guar_cont.guar_cont_state in ('00','01')) ";
				} else {
					condition = " where limit_code = '" + limit_code+ "' and serno = '" + ladSerno + "' and corre_rel not in ('3','5') and exists (select 1 from grt_guar_cont where grt_guar_cont.guar_cont_no = r_lmt_app_guar_cont.guar_cont_no and grt_guar_cont.guar_way = '01' and grt_guar_cont.guar_cont_state in ('00','01')) ";
				}
				...
				if (rlagcIColl != null && rlagcIColl.size() > 0) {
					...
					for (int j = 0; j < rlagcIColl.size(); j++) {
						...
						if (count.compareTo(new BigDecimal(1))<0) {
							returnFlag = "不通过";
							returnInfo = "该授信申请中业务流水号["+ladSerno+"]的授信分项[" + limit_code+ "]的担保合同["+guar_cont_no+"]不存在押品类别为" + showStr+ "的【担保品】,请检查！";
							break;
						}
					}
				} else {
					returnFlag = "不通过";
					returnInfo = "该授信申请中业务流水号["+ladSerno+"]的授信分项[" + limit_code+ "]不存在相应的【担保合同信息】,请检查！";
				}
		...
		String contdionStr = " WHERE LIMIT_CODE IN(SELECT ORG_LIMIT_CODE FROM LMT_APP_DETAILS WHERE SERNO IN (SELECT SERNO FROM LMT_APPLY WHERE GRP_SERNO='"+serno+"'))";
		//需关联流水号并且担保合同与授信关系不为解除   2013-12-03  唐顺岩
		contdionStr += " AND SERNO IN (SELECT SERNO FROM LMT_APPLY WHERE GRP_SERNO='"+serno+"') AND CORRE_REL<>'3'";
		IndexedCollection icoll = dao.queryList("RLmtAppGuarCont", contdionStr, connection);
		for (Iterator iterator = icoll.iterator(); iterator.hasNext();) {
			...
			IndexedCollection iColl = dao.queryList(ladModelId, " where org_limit_code = '" + limit_code+ "' and serno = '" + ladSerno + "'", connection);
