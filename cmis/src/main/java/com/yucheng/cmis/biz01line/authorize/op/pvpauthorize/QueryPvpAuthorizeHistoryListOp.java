...
public class QueryPvpAuthorizeHistoryListOp extends CMISOperation {
	...
	public String doExecute(Context context) throws EMPException {
		...
		try{
			...
			if(conditionStr !=null && !(conditionStr.equals(""))){
				//conditionStr += " and status <> '00' and status <> '01' and serno in (select b.serno from pvp_loan_app b where b.cont_no in (select c.cont_no from ctr_loan_cont c union all select c.cont_no from ctr_rpddscnt_cont c)) order by serno,cont_no desc";
				conditionStr += " order by tran_date desc,substr(serno,-14) desc";
			}else{
				//conditionStr = "where status not in('00','01') and serno in (select b.serno from pvp_loan_app b where b.cont_no in (select c.cont_no from ctr_loan_cont c union all select c.cont_no from ctr_rpddscnt_cont c)) order by serno,cont_no desc";
				 conditionStr = " order by tran_date desc,substr(serno,-14) desc";  
			}    
