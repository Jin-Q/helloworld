...
public class QueryPvpAssetProHistoryListOp extends CMISOperation {

	private final String modelId = "PvpLoanApp";
	private final String modelIdCont = "CtrAssetProCont";
		...
		try{
			...
			if(iColl!=null && iColl.size()>0){
				for(Iterator<KeyedCollection> iterator = iColl.iterator();iterator.hasNext();){
					...
					KeyedCollection capc = dao.queryDetail(modelIdCont, temp.getDataValue("cont_no").toString(), connection);
