...
public class DelImageScanActionOp extends CMISOperation {
	...
	public String doExecute(Context context) throws EMPException {
		...
		try{
			...
			if(kColl.getDataValue("cus_type").equals("BL200")){	//给影像系统时小微视为对公客户
				kColl.setDataValue("cus_type", "BL100");
			}
			
			/*** 根据影像接口调用类型，调用不同接口 ***/
			if(image_action.equals("Scan22")){	//2.2.	客户资料扫描（抵质押预扫）接口
				url = cmisOp.delSecondBasicUrl(url, kColl, context, 0);
			}else if(image_action.equals("Scan24")){	//2.4.	业务资料扫描接口
				url = cmisOp.delSecondBusUrl(url, kColl, context,0);
			}else if(image_action.equals("Scan26")){	//2.6.	贷后资料扫描接口
				url = cmisOp.delSecondPostUrl(url, kColl, context, 0);
			}
