...
public class submitLmtAppGrp  extends CMISOperation {
	
	private final String modelIdAppAgr = "LmtAppGrp";
	...
	public String doExecute(Context context) throws EMPException {
		...
		try{
			...
			KeyedCollection kCollAppAgr = dao.queryDetail(modelIdAppAgr, serno_value, connection);
			
			kCollAppAgr.setDataValue("over_date", context.getDataValue("OPENDAY"));
			kCollAppAgr.setDataValue("approve_status", "997");
			//修改集团申请表的办结日期、申请状态
			dao.update(kCollAppAgr, connection);
			
			String grp_agr_no = "";
			if(!"".equals(kCollAppAgr.getDataValue("app_type")) && "01".equals(kCollAppAgr.getDataValue("app_type"))){  //新增
				//生成授信协议编号
			...
			}else if(!"".equals(kCollAppAgr.getDataValue("app_type")) && "02".equals(kCollAppAgr.getDataValue("app_type"))){   //变更
			...
			IndexedCollection iCollApp = dao.queryList(modelIdApp, condition, connection);
			for(int i=0;i<iCollApp.size();i++){
				...
				kCollApp.setDataValue("over_date", context.getDataValue("OPENDAY"));
				//修改单一授信申请表的办结日期、申请状态
				...
				kCollApp.addDataField("grp_agr_no", grp_agr_no);
				...
				for (Iterator<KeyedCollection> iterator = iColl.iterator(); iterator.hasNext();) {
					...
					kColl_details.addDataField("agr_no", app_agr_no);  //授信协议编号
