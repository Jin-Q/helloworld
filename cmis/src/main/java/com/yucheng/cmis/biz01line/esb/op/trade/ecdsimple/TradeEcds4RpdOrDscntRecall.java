 ...
 * 
 */
public class TradeEcds4RpdOrDscntRecall extends TranService {
	...
	public KeyedCollection doExecute(CompositeData CD, Connection connection)
		...
		try {
			...
			String BUSS_TYPE = reqBody.getDataValue("BUSS_TYPE").toString().trim();//业务类型
			KeyedCollection approveInfoKColl = new KeyedCollection();
			String appTableModel = "";
			String sqlName4App = "";
			String sqlName4Cont = "";
			if(BUSS_TYPE!=null&&!"".equals(BUSS_TYPE)){
				if("00".equals(BUSS_TYPE)){//贴现业务
					approveInfoKColl = (KeyedCollection)SqlClient.queryFirst("getDscntApproveInfo4Ecds", BATCH_NO, null, connection);
					appTableModel = "IqpLoanApp";
					sqlName4App = "updateIqpLoanAppStatus4Ecds";
					sqlName4Cont = "updateCtrloanContStatus4Ecds";
				}else{//转贴现业务
					approveInfoKColl = (KeyedCollection)SqlClient.queryFirst("getRpdDscntApproveInfo4Ecds", BATCH_NO, null, connection);
					appTableModel = "IqpRpddscnt";
					sqlName4App = "updateIqpRpddscntStatus4Ecds";
					sqlName4Cont = "updateCtrRpddscntContStatus4Ecds";
				}
				if(approveInfoKColl!=null&&approveInfoKColl.size()>0){
					...
					if(flag!=null&&"1".equals(flag)){
						...
						SqlClient.delete("deleteAccDrftForECDSCancel", cont_no, connection);
					}else if(flag!=null&&"5".equals(flag)){
						retKColl.setDataValue("ret_code", "9999");
						retKColl.setDataValue("ret_msg", "【电子商业汇票相关贴现业务撤回】,交易失败！");
						retKColl.setDataValue("cancel_result", "01");
					}
				}
			}else{
				retKColl.setDataValue("ret_code", "9999");
				retKColl.setDataValue("ret_msg", "【电子商业汇票相关贴现撤回】,业务类型为空,交易失败！");
				retKColl.setDataValue("cancel_result", "01");
			}
			retKColl.setDataValue("cancel_result", "00");
			EMPLog.log("TradeEcds4RpdOrDscntRecall", EMPLog.INFO, 0, "【电子商业汇票相关贴现业务撤回】交易完成...", null);
		} catch (Exception e) {
			retKColl.setDataValue("ret_code", "9999");
			retKColl.setDataValue("ret_msg", "【电子商业汇票相关贴现业务撤回】,交易失败！");
			retKColl.setDataValue("cancel_result", "01");
