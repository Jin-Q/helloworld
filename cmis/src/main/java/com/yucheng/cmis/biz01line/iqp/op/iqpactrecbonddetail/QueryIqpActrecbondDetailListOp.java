...
public class QueryIqpActrecbondDetailListOp extends CMISOperation {
	...
	public String doExecute(Context context) throws EMPException {
		...
		try {
			...
			String conditionStr = TableModelUtil.getQueryCondition_bak(
					this.modelId, queryData, context, false, true, false);
			if (conditionStr == null || "".equals(conditionStr)) {
				conditionStr = "where po_no ='" + poNo + "' order by po_no desc";
			} else {
				conditionStr = conditionStr + "and po_no ='" + poNo + "' order by po_no desc";
			}
			int size = 10;
