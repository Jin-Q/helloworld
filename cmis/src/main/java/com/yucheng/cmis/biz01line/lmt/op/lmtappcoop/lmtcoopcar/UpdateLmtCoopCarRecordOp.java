...
public class UpdateLmtCoopCarRecordOp extends CMISOperation {
	...
	public String doExecute(Context context) throws EMPException {
		...
		try{
			...
			if(kColl == null || kColl.size() == 0){
				context.addDataField("msg", "未获取到合作方汽车项目信息！");
				context.addDataField("flag","N");
				throw new Exception("未获取到合作方汽车项目信息！");
			}
			
			TableModelDAO dao = this.getTableModelDAO(context);
			int count=dao.update(kColl, connection);
			if(count!=1){
				context.addDataField("msg", "合作方汽车项目信息更新失败！");
				context.addDataField("flag","N");
				throw new Exception("合作方汽车项目信息更新失败！");
			}
