...
public class AddPubDocumentInfoRecordOp extends CMISOperation{
	...
	public String doExecute(Context context) throws EMPException {
		...
		try{
			...
			if(context.containsKey("file_type")){
				fileType = context.getDataValue("file_type").toString();
			}
			if(context.containsKey("serno")){
				serno = context.getDataValue("serno").toString();
			}
			if(context.containsKey("file_memo")){
				fileMemo = (String)context.getDataValue("file_memo");
			}
			kColl.addDataField("file_type", fileType);
			kColl.addDataField("serno", serno);
			kColl.addDataField("file_memo",fileMemo);
