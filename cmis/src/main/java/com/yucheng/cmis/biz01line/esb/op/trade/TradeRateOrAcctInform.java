 ...
 * 利率调整、账户调整通知
 * @author Pansq
 * 1.判断还款账号是否为空,不为空则根据借据号和账户属性为01(一般账户)更新账户信息表的账号
 * 2.判断利率是否为空,不为空则根据利率类型（正常利率，逾期利率，垫款利率，违约利率）更新合同子表中相应的利率字段
 * 
 */
public class TradeRateOrAcctInform extends TranService {
...
//					SqlClient.update("updateAccRateInfo", DUEBILL_NO, value, null, connection);//利率放在日终同步 2014-07-04
			...
			EMPLog.log("TradeRateOrAcctInform", EMPLog.INFO, 0, "【利率调整、账户调整通知】交易处理完成...", null);
		} catch (Exception e) {
			retKColl.setDataValue("ret_code", "999999");
			retKColl.setDataValue("ret_msg", "【利率调整、账户调整通知】 ,业务处理失败！借据号为："+DUEBILL_NO);
			e.printStackTrace();
			EMPLog.log("TradeRateOrAcctInform", EMPLog.ERROR, 0, "【利率调整、账户调整通知】交易处理失败，借据号为："+DUEBILL_NO+"异常信息为："+e.getMessage(), null);
		}
		return retKColl;
	}
