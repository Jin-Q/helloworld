...
public class DeleteRLmtGuarContZGEOp extends CMISOperation {

	private final String modelIdRel = "RLmtAppGuarCont"; 
	...
	public String doExecute(Context context) throws EMPException {
		...
		try{
			...
			LmtGuarComponent cmisComponent = (LmtGuarComponent)CMISComponentFactory.getComponentFactoryInstance().getComponentInstance("LmtRGuar", context, connection);
         	int rels = cmisComponent.updateRLmtGuarLvlZGE(updateMap, connection);
			if(rels < 0){ 
			...
			map.put("serno", serno);   //新加流水号 做组合主键
			map.put("guar_cont_no", guar_cont_no);
			int countRel = dao.deleteByPks(modelIdRel, map, connection); 
