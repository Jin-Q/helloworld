 ...
 * 测试DBUtil.java
 */
public class Trade0200300000505 extends TranService {
	...
	public KeyedCollection doExecute(CompositeData CD, Connection connection) throws Exception {
		...
		String client_no ="",gt_status,start_date,end_date;
		try {
			...
			gt_status = (String)reqBody.getDataValue("GT_STATUS");
			...
			/** 
			 * 参数：客户码，保函状态，起始日期，到期日期
			 * */
			String sql = "select a.loan_amt, a.distr_date, a.end_date, b.assure_main, a.acc_status,b.security_rate " +
					" from acc_loan a, ctr_loan_cont b" +
					" where a.cont_no = b.cont_no and a.prd_id = '400021' and a.acc_status != '0' ";

			boolean flag = true;
			if(gt_status!=null&&!gt_status.equals("")){
				sql = sql + " and a.acc_status = '"+gt_status+"' ";
				flag = false;
			}
			...
			/** 组装返回报文 */
			retKColl.setDataValue("ret_msg", "【保函业务查询】交易处理完成");
			retKColl.put("client_no", client_no);
			retKColl.put("IColl", IColl);
			
			EMPLog.log("outReport", EMPLog.INFO, 0, "【保函业务查询】交易处理完成...", null);
		} catch (Exception e) {
			retKColl.setDataValue("ret_code", "999999");
			retKColl.setDataValue("ret_msg", "【保函业务查询】业务处理失败！客户号号为："+client_no);
			e.printStackTrace();
			EMPLog.log("outReport", EMPLog.ERROR, 0, "【保函业务查询】交易处理失败，客户号为："+client_no+"异常信息为："+e.getMessage(), null);
		}
		return retKColl;
	}
}
