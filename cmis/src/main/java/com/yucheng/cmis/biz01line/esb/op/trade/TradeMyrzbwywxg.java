 ...
 * 
 */
public class TradeMyrzbwywxg extends TranClient {
	...
	public CompositeData doExecute(Context context, Connection connection)
		...
		try{
			...
			bodyCD.addField("ADD_OR_REDUCE_AMT", TagUtil.getEMPField(reflectKColl.getDataValue("ADD_OR_REDUCE_AMT"), FieldType.FIELD_DOUBLE, 20, 2));
			bodyCD.addField("BUSS_CCY", TagUtil.getEMPField(reflectKColl.getDataValue("BUSS_CCY"), FieldType.FIELD_STRING, 3, 0));
			...
			bodyCD.addField("EXE_EXPI_DATE", TagUtil.getEMPField(reflectKColl.getDataValue("EXE_EXPI_DATE"), FieldType.FIELD_STRING, 8, 0));
			...
			bodyCD.addField("BAK_FIELD", TagUtil.getEMPField("", FieldType.FIELD_STRING, 300, 0));
			bodyCD.addField("BAK_FIELD2", TagUtil.getEMPField("", FieldType.FIELD_STRING, 300, 0));
			bodyCD.addField("GT_AMOUNT", TagUtil.getEMPField(reflectKColl.getDataValue("GT_AMOUNT"), FieldType.FIELD_DOUBLE, 20, 2));
			bodyCD.addField("GT_HIGHEST_AMT", TagUtil.getEMPField(reflectKColl.getDataValue("GT_HIGHEST_AMT"), FieldType.FIELD_DOUBLE, 20, 2));
			
			reqCD.addStruct("BODY", bodyCD);
			
			EMPLog.log("outReport", EMPLog.INFO, 0,  new String(PackUtil.pack(reqCD), "UTF-8"));
		}catch (Exception e) {
