...
public class QueryIqpFreedomPayInfoListOp extends CMISOperation {
		...
		try{
			...
			if(modiflg!=null && !"".equals(modiflg) && "yes".equals(modiflg)){
				if(conditionStr.indexOf("WHERE") != -1){
					conditionStr = conditionStr + " and modify_rel_serno = '"+modify_rel_serno+"' "  + " order by to_number(dateno) asc";
				}else{
					conditionStr = "where 1=1 " + " and modify_rel_serno = '"+modify_rel_serno+"' "  + " order by to_number(dateno) asc";
				}
			}else{
				if(conditionStr.indexOf("WHERE") != -1){
					conditionStr = conditionStr + " and serno = '"+serno+"' "  + " order by to_number(dateno) asc";
				}else{
					conditionStr = "where 1=1 " + " and serno = '"+serno+"' "  + " order by to_number(dateno) asc";
				}
			}
			...
			if(modiflg!=null && !"".equals(modiflg) && "yes".equals(modiflg)){
				iColl = dao.queryList("IqpFreedomPayInfoTmp",null ,conditionStr,connection);
				iColl.setName(modelId);
			}else{
				iColl = dao.queryList(modelId,null ,conditionStr,connection);
			}
			
			//增加还款计划初始化 add by zhaozq 20141014 start
			if(iColl==null||iColl.size()==0){
				KeyedCollection iqpkc =new KeyedCollection();
				KeyedCollection subkc =new KeyedCollection();
				String apply_date ="";
				...
				String term ="";
				if(modiflg!=null && !"".equals(modiflg) && "yes".equals(modiflg)){
					iqpkc = dao.queryDetail("CtrLoanContTmp", modify_rel_serno, connection);
					apply_date = (String) iqpkc.getDataValue("cont_start_date");
					subkc = dao.queryDetail("CtrLoanContSubTmp", modify_rel_serno, connection);
					...
					term = (String) subkc.getDataValue("cont_term");
				}else{
					iqpkc = dao.queryDetail("IqpLoanApp", serno, connection);
					apply_date = (String) iqpkc.getDataValue("apply_date");
					subkc = dao.queryDetail("IqpLoanAppSub", serno, connection);
					interest_term = (String) subkc.getDataValue("interest_term");
				    repay_date = (String) subkc.getDataValue("repay_date");
					termType = (String) subkc.getDataValue("term_type");
			...
			//增加还款计划初始化 add by zhaozq 20141014 end
